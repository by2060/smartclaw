...
def build_session_dynamic_rex_prompt(
    *,
    ...
    """Build Rex's prompt using session-scoped capability visibility."""
...
def _filter_session_tools(
    *,
    ...
    allowed = {
        str(name).strip()
        for name in (getattr(rex_agent_info, "tools", None) or [])
        if str(name).strip()
    }
    return [
        tool
...
def _filter_session_skills(
    *,
    ...
    allowed = _normalize_name_set(getattr(rex_agent_info, "skills", None) or [])
    if not allowed:
        return []
    return [
        skill
