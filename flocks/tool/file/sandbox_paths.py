...
def _map_workspace_plugin_container_path(
    ...
    """Map /workspace/.flocks/plugins to the project plugin directory."""
    ...
    rel: str | None = None
    for root in dict.fromkeys(container_roots):
        plugin_prefix = f"{root.rstrip('/')}/.flocks/plugins"
        if raw == plugin_prefix:
            rel = ".flocks/plugins"
            break
        if raw.startswith(plugin_prefix + "/"):
            rel = ".flocks/plugins/" + raw[len(plugin_prefix + "/"):]
            break
    if rel is None:
        return None
...
def _map_session_output_path(
    ...
    user-facing Flocks outputs directory. Those files still need to be readable
...
def is_project_plugin_path(ctx: ToolContext, filepath: str) -> bool:
    ...
    try:
        ...
        if Instance.get_directory():
            roots.append(Path(str(Instance.get_directory())) / ".flocks" / "plugins")
    except Exception:
        pass
    roots.append(Path.cwd() / ".flocks" / "plugins")
