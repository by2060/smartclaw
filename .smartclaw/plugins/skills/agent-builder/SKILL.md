...
name: agent-builder
category: system
description: Create new sub-agents (subagents) by generating YAML config and prompt files in the project-level .smartclaw/plugins/agents/ directory. The created agent can be delegated to by Titan via delegate_task. Use when the user asks to create, add, or generate a new agent.
description_cn: 创建新的子智能体，通过在项目级 .smartclaw/plugins/agents/ 目录下生成 YAML 配置和 prompt 文件。创建的智能体可被 Titan 通过 delegate_task 委派调用。当用户要求创建、添加或生成新智能体时使用。
---

# Agent Builder

Create a new sub-agent from user requirements. Produces a YAML config file + prompt file under `<project>/.smartclaw/plugins/agents/<name>/`, loadable by the system without restart.

**Required directory layout:**
```
<project>/.smartclaw/plugins/agents/
...
Create `<project>/.smartclaw/plugins/agents/{name}/prompt.md` with the following structure:
...
Create `<project>/.smartclaw/plugins/agents/{name}/agent.yaml`. Prefer an explicit `tools` allowlist; use `permission` only for advanced wildcard matching or deny/allow patterns that cannot be expressed as a simple list.

```yaml
# Required
name: {name}                    # kebab-case, globally unique
description: >                  # One-sentence description; Titan uses this to decide when to delegate
...
# Delegation metadata (tells Titan when to delegate to this agent)
prompt_metadata:
...
| `plan_and_execute` | Complex tasks requiring planning before execution | All tools | titan, hephaestus |
...
1. **YAML syntax**: run `python3 -c "from pathlib import Path; import yaml; root=Path.cwd(); yaml.safe_load((root/'.smartclaw/plugins/agents/{name}/agent.yaml').read_text(encoding='utf-8'))"`
2. **Prompt file exists**: confirm `<project>/.smartclaw/plugins/agents/{name}/prompt.md` has been created
3. **Directory structure**: ensure files are inside `<project>/.smartclaw/plugins/agents/{name}/`, NOT as flat files like `agents/{name}.yaml`
4. **Name uniqueness**: ensure no collision with built-in agents (reserved names: titan, hephaestus, oracle, librarian, explore, general, metis, momus, multimodal-looker, titan-junior, build, plan, compaction, title, summary)
...
6. **Trigger reload**: call the refresh API so Titan recognizes the new agent immediately — **no restart needed**:
   ```bash
...
- File paths created (e.g. `<project>/.smartclaw/plugins/agents/{name}/agent.yaml` and `prompt.md`)
- Agent name and role
...
- **Files MUST be written to `<project>/.smartclaw/plugins/agents/<name>/` subdirectory** — flat files like `agents/<name>.yaml` are legacy and should NOT be created
