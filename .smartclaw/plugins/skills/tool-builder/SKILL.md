...
category: system
description: >-
  Creates reusable SmartClaw tools and API integrations. Supports YAML-HTTP for REST APIs and Python for local utilities, with mandatory verification and smoke testing. All output under the project-level .smartclaw/plugins/tools/ directory. When to use: creating or adding a new SmartClaw tool, building local utilities such as base64 encode-decode, URL encode-decode, JSON formatting, parsing, hashing, text or file transformation, or integrating an external REST API as a reusable tool. Example requests: "Create a base64 encode/decode tool", "Build a URL encode/decode utility", "Add a JSON formatter tool", "Integrate a REST API as a SmartClaw tool".
description_cn: >-
  创建可复用的 SmartClaw 工具和 API 集成。支持 YAML-HTTP 用于 REST API 和 Python 用于本地工具，必须验证和冒烟测试。所有输出在项目级 .smartclaw/plugins/tools/ 目录下。当用户要求创建或添加新 SmartClaw 工具、构建本地工具（如 base64 编解码、URL 编解码、JSON 格式化、解析、哈希、文本或文件转换）或集成外部 REST API 作为可复用工具时使用。
---
...
**ALL generated artifacts MUST be placed under `<project>/.smartclaw/plugins/tools/`.**

```
<project>/.smartclaw/plugins/tools/
...
| Mode | Output Path |
|------|------------|
| YAML-HTTP tool | `<project>/.smartclaw/plugins/tools/api/{name}.yaml` |
| YAML-HTTP tool (Provider) | `<project>/.smartclaw/plugins/tools/api/{provider}/{name}.yaml` |
| YAML script handler | `<project>/.smartclaw/plugins/tools/api/{provider}/{name}.handler.py` |
| Provider config | `<project>/.smartclaw/plugins/tools/api/{provider}/_provider.yaml` |
| Python tool | `<project>/.smartclaw/plugins/tools/python/{name}.py` |

**NEVER** write to `smartclaw/tool/generated/`, `smartclaw/tool/`, or any other project source path.
...
Add the secret to `~/.smartclaw/config/.secret.json`:
```python
...
3. **Check if a Provider exists** — Look for `<project>/.smartclaw/plugins/tools/api/{provider}/_provider.yaml`.
...
6. **Write tool YAMLs for all in-scope endpoints** — Create `<project>/.smartclaw/plugins/tools/api/{name}.yaml` (or `api/{provider}/{name}.yaml`) until the inventory is exhausted, not just the first few endpoints.
...
```yaml
# <project>/.smartclaw/plugins/tools/api/threatbook-cn/_provider.yaml
name: threatbook-cn
...
```yaml
# <project>/.smartclaw/plugins/tools/api/threatbook-cn/threatbook_cn_file_report.yaml
...
Script (`<project>/.smartclaw/plugins/tools/api/threatbook-cn/threatbook_cn.handler.py`):
```python
...
3. **Generate tool code** — Create `<project>/.smartclaw/plugins/tools/python/{name}.py` with `@ToolRegistry.register_function`.
4. **Run Verification Protocol** (see below) — MANDATORY.

### Python Tool Format

File: `<project>/.smartclaw/plugins/tools/python/{name}.py`
```python
...
```bash
PROJECT_SKILL_DIR=".smartclaw/plugins/skills/tool-builder"
USER_SKILL_DIR="$HOME/.smartclaw/plugins/skills/tool-builder"
...
- The resolved script file exists under an allowed plugins root:
  `<project>/.smartclaw/plugins/` or `~/.smartclaw/plugins/`
- For newly created project tools, place the handler next to the YAML file under
  `<project>/.smartclaw/plugins/tools/api/{provider}/`
...
**Python tools (Mode B)**
- `from smartclaw.tool.registry import ...` is present
...
from smartclaw.tool.tool_loader import yaml_to_tool, _read_yaml_raw
...
- **Wrong output path**: STOP immediately, move files to `<project>/.smartclaw/plugins/tools/{type}/`

## Pre-flight Checklist (mental, before writing any file)

1. Output path is under `<project>/.smartclaw/plugins/tools/{type}/`
