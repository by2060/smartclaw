...
name: onboarding
category: system
description: Guide new users through the complete SmartClaw setup process via conversation — covering ThreatBook MCP/API, security tool APIs, IM channels, and scenario demos. Also runs system health inspection for returning users. Trigger when the user sends "请启动新手引导流程", "start onboarding", or any similar request to begin setup/configuration.
description_cn: 通过对话引导新用户完成 SmartClaw 完整设置流程，包括 ThreatBook MCP/API、安全工具 API、IM 渠道和场景演示。对老用户运行系统健康检查。当用户发送“请启动新手引导流程”、“start onboarding”或类似请求时触发。
---

# Onboarding — New User Setup Guide

When this skill is triggered, walk the user through SmartClaw initial configuration step by step via natural conversation.
...
# IMPORTANT: use the unified user config under ~/.smartclaw/config, not the repo's example .smartclaw directory.
from smartclaw.skill.onboarding_status import print_onboarding_preflight_status
...
Open with a brief friendly greeting that introduces Titan. Then immediately present the **Status Report** (from "Status Report Format" above) based on the pre-flight check results.
...
Tell the user: 微步 LLM 是 SmartClaw 默认的推理引擎。中国区请前往 https://x.threatbook.cn 获取 API Key，国际区请前往 https://x.threatbook.io 获取 API Key。默认先配置中国区 `threatbook-cn-llm`。
...
# 2. Register provider in smartclaw.json so apply_config() can resolve the secret ref
...
print('ThreatBook CN LLM configured (secret + smartclaw.json).')
```
...
> | 类型 | 用途 | 接入方式 |
> |---|---|---|
> | **微步 MCP 工具** | Titan 直接调用威胁情报工具（IP/域名/Hash 查询等），通过 MCP 协议 | `smartclaw mcp add` CLI 命令 |
...
### Step 2a — 微步 MCP 工具（通过 `smartclaw_mcp` 工具添加）

Tell the user:

> 微步 MCP 工具让 Titan 直接调用微步在线威胁情报能力（IP/域名/Hash 查询等），推荐优先配置。
> 中国区请前往 [https://x.threatbook.com/smartclaw/activate](https://x.threatbook.com/smartclaw/activate) 获取 MCP API Key，国际区请前往 [https://threatbook.io/smartclaw/activate](https://threatbook.io/smartclaw/activate) 获取 MCP API Key，然后把 Key 告诉我。
...
**After receiving the MCP key, call `smartclaw_mcp` tool to register the MCP server:**
...
```bash
# 触发连接，让 MCP 工具立即注册
# 默认端口 8000；若用户自定义了端口，先运行 `smartclaw serve --help` 查看，或让用户告知
...
> 这些服务都按 **API service** 处理，不要使用 `smartclaw mcp add`、`ConfigWriter.add_mcp_server(...)`，也不要为了这些服务额外安装或连接 MCP。
...
Configure each channel by writing to `smartclaw.json` via `ConfigWriter`.
...
Tell the user: "配置完成！来看看 SmartClaw 能帮你做哪些事。"
...
**触发方式：** 粘贴邮件内容/EML 文件/可疑 URL，Titan 自动分析。
...
2. **Secret file writes**: Write secrets to `Config.get_secret_file()` (normally `~/.smartclaw/config/.secret.json`) via `json.load/dump` (load existing → merge new key → write back). `SecretManager` also works, but using `Config.get_secret_file()` keeps onboarding scripts aligned with the runtime config directory.
3. **Key naming conventions**:
   ...
   - IM channels: 敏感凭据存入 `.secret.json`，命名约定：`channel_feishu_appSecret`、`channel_wecom_secret`、`channel_dingtalk_clientSecret`、`channel_telegram_botToken`；`smartclaw.json` 中对应字段写 `{secret:<key>}` 引用。非敏感字段（如 `appId`、`botId`、`clientId`）直接写入 `smartclaw.json` 明文。配置结构为**扁平结构**，字段直接放在 `channels.<name>` 下，不使用 `accounts.default` 嵌套。
