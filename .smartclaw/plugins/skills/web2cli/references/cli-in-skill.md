...
- skill 目录：`$HOME/.smartclaw/plugins/skills/<name>-use/`
- CLI 主脚本：`$HOME/.smartclaw/plugins/skills/<name>-use/scripts/<name>_cli.py`
- 默认认证状态：`~/.smartclaw/browser/<name>/auth-state.json`
...
```bash
SKILL_ROOT="$HOME/.smartclaw/plugins/skills/<name>-use"

mkdir -p "$SKILL_ROOT/scripts"
mkdir -p "$HOME/.smartclaw/browser/<name>"

cp "$CAPTURE_ROOT/<normalized_capture_name>_cli.py" \
  "$SKILL_ROOT/scripts/<name>_cli.py"

cp "$CAPTURE_ROOT/auth-state.json" \
  "$HOME/.smartclaw/browser/<name>/auth-state.json"
```

然后补齐这几项：

1. 在 `scripts/config.py` 中把认证状态默认值指向 `~/.smartclaw/browser/<name>/auth-state.json`
...
AUTH_STATE_FILE = Path(
    os.getenv(
        "<NAME>_AUTH_STATE",
        Path.home() / ".smartclaw" / "browser" / "<name>" / "auth-state.json",
    )
)
```
...
```text
$HOME/.smartclaw/plugins/skills/<name>-use/
├── SKILL.md
├── scripts/
│   ├── <name>_cli.py
│   └── config.py
...
其中 `SKILL.md` 必须遵守 SmartClaw 的标准 skill 格式：
...
新 skill 的原则也一样：先把生成的 CLI 改成稳定文件名，再把临时 `auth-state.json` 切换到全局默认位置 `~/.smartclaw/browser/<name>/auth-state.json`。
...
- 默认 state 路径，例如 `~/.smartclaw/browser/<name>/auth-state.json`
- 认证失效识别与恢复步骤
...
```bash
~/.smartclaw/browser/<name>/auth-state.json
```
...
# agent-browser 模式
agent-browser state save "$HOME/.smartclaw/browser/<name>/auth-state.json"

# 或 cdp-direct / smartclaw browser 模式
smartclaw browser state save "$HOME/.smartclaw/browser/<name>/auth-state.json"
```
...
`web2cli` 产出的 `<capture_name>_cli.py` 是临时结果；真正沉淀到 skill 时，要改成稳定产品名脚本，并把认证状态统一落到 `~/.smartclaw/browser/<name>/auth-state.json`。
