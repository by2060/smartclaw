...
适用于需要复用用户 Chromium 系浏览器登录态、通过 `browser-use` 的 `smartclaw browser` 内核直连 CDP 的场景。
...
用户完成后，不要立刻再次运行 `smartclaw browser --doctor`；先执行一次 `smartclaw browser --setup`，或直接执行 `smartclaw browser -c 'print(page_info())'` 触发 attach，再用 `--doctor` 做只读确认。

## 输出目录约定

捕获产生的文件统一落到 `~/.smartclaw/workspace/outputs/web2cli/<name>/`。
...
CAPTURE_NAME="<name>"
CAPTURE_ROOT="$HOME/.smartclaw/workspace/outputs/web2cli/$CAPTURE_NAME"
WEB2CLI_SKILL=".smartclaw/plugins/skills/web2cli"
mkdir -p "$CAPTURE_ROOT/captures"
```

补充说明：

- `smartclaw browser -c '...'` 会把代码直接交给 Python `exec()`，表达式不会像 REPL 一样自动回显；需要输出时必须显式 `print(...)`。
...
agent-browser --session-name "$CAPTURE_NAME" eval --stdin < .smartclaw/plugins/skills/web2cli/scripts/inject-hook-base.js
```
...
```bash
(
  TARGET_ID="$TARGET_ID" smartclaw browser -c '
import os
...
(
  TARGET_ID="$TARGET_ID" CAPTURE_OUT="$CAPTURE_OUT" smartclaw browser -c '
import json
import os
...
if target_id:
    attach_tab(target_id)
'
  && smartclaw browser state save "$CAPTURE_ROOT/auth-state.json"
)
```
...
```bash
uv run python .smartclaw/plugins/skills/web2cli/scripts/generate-spec.py \
...
```bash
uv run python .smartclaw/plugins/skills/web2cli/scripts/generate-cli.py \
