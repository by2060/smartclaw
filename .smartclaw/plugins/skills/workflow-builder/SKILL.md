...
category: system
description: 根据自然语言描述生成 smartclaw 内置工作流（workflow.md, workflow.json）。当用户提出创建/设计/生成/搭建工作流或任何多步骤流程（如告警调查、事件响应、SOP/Runbook 自动化）时使用本 skill。
description_cn: 根据自然语言描述生成 smartclaw 内置工作流（workflow.md, workflow.json）。当用户提出创建/设计/生成/搭建工作流或任何多步骤流程（如告警调查、事件响应、SOP/Runbook 自动化）时使用。
---
...
- **强制读取** `smartclaw/tool/registry.py`（`ToolInfo.parameters` 是参数 schema 的权威来源）。
...
1. 将简化 JSON 写入当前项目规范目录下的 `workflow.json`（**必须使用绝对路径**，见第 6 节）：`<workspace>/.smartclaw/plugins/workflows/<id>/`。用户级 `~/.smartclaw/plugins/workflows/<id>/` 仅允许作为历史兼容扫描路径，不允许作为新建或修改目标。
...
1. 先用 `write` 工具将 `workflow.md` **写入文件**（路径与第 6 节一致，例如 `.../plugins/workflows/<id>/workflow.md`）。
   - **⚠️ 路径必须使用绝对路径**：先解析 workspace（从 cwd 向上第一个含 `.smartclaw` 的目录），再拼接 `/.smartclaw/plugins/workflows/<id>`。
   - **严禁**使用未展开的相对路径（如 `.smartclaw/plugins/workflows/<id>/` 相对仓库根随手写入错误位置），否则 WebUI 可能无法从实际扫描目录读到文件。
...
**Node 约束**（对应 `smartclaw/workflow/models.py`）：
...
- **项目级（当前 workspace）**：`<workspace>/.smartclaw/plugins/workflows/<slug-or-folder>/`（`workflow.json`、`workflow.md`、`meta.json` 由 API 写入时可能同目录）
  - ⚠️ 任务输出（报告、artifacts）**不**写入此目录，统一写入 `~/.smartclaw/workspace/outputs/<YYYY-MM-DD>/<session_id>/`（见全局文件输出约定）
...
**项目（workspace 下）**的`<workspace>/.smartclaw/plugins/workflows/`，**项目规范路径（新建或修改工作流必须落地于此）**
...
```bash
python3 -c "from pathlib import Path; p=Path.cwd(); ws=next((x for x in [p,*p.parents] if (x/'.smartclaw').is_dir()), p); print(ws/'.smartclaw/plugins/workflows/<folder>')"
```

**正确示例**：
- `<workspace>/.smartclaw/plugins/workflows/alert_triage/workflow.json` ✅（项目级）

**错误示例**：
- `.smartclaw/plugins/workflows/alert_triage/workflow.json` ❌（未展开相对路径，易写错磁盘位置）
- `~/.smartclaw/plugins/workflows/alert_triage/workflow.json` ❌（用户级路径仅允许历史兼容扫描，不允许作为新建或修改目标）
- 仅因习惯写入 `~/.smartclaw/workflow/...` 作为**新**工作流首选 ❌（仍可被扫描，但与当前规范及项目级落盘不一致）
