# CDP 直连（smartclaw browser 内核）

本文件是 `browser-use` 的 CDP 模式参考文档。只有当 `browser-use/SKILL.md` 判定应使用 `CDP 直连` 时，才读取并遵循本文件；不要和 `references/agent-browser.md` 同时加载。

## 使用入口

`smartclaw browser` 是 SmartClaw 内置的薄 CDP harness：agent 直接控制真实浏览器，helpers 预加载，daemon 自动启动。通过 `smartclaw browser ...` 调用。
...
先确认 `smartclaw browser` 可用：

```bash
smartclaw browser --doctor
```
...
`smartclaw browser -c` 内部可直接使用预加载 helpers；daemon 会自动启动并连接已运行的 Chromium 系浏览器。

注意：

- `smartclaw browser -c '...'` 执行的是一段 Python 代码，不是交互式 REPL；如果希望看到结果，必须显式 `print(...)`。
...
`smartclaw browser` 的基础循环是：打开页面 -> 读取结构化状态 -> 执行动作 -> 重新读取状态验证。

1. 如果是具体网站任务，先搜索已存在的站点经验。优先看 `~/.smartclaw/workspace/domain-skills/`；如果没有，再自己探索。
...
```bash
smartclaw browser --debug-clicks -c '
click_at_xy(420, 315)
'
```
...
`smartclaw browser` 复用用户当前浏览器 profile。cookies、localStorage、sessionStorage 会自然保留；用户完成登录后，同一 profile 下的新 tab 通常自动带登录态。
...
- 明确需要导出或保存登录状态时，可以用 smartclaw browser state save auth-state.json
...
如果 `smartclaw browser` 不可用或连接失败：

1. 先运行 `smartclaw browser --doctor` 看版本、安装模式、daemon 和浏览器状态。
2. 首次安装或冷启动优先运行 `smartclaw browser --setup`。
...
5. 用户刚开启 remote debugging 时，不要立刻再次运行 `smartclaw browser --doctor`；先执行一次 `smartclaw browser --setup`，或直接执行 `smartclaw browser -c 'print(page_info())'` 触发 daemon attach，再用 `--doctor` 做只读确认。
6. `connection refused`、`DevTools not live yet`、`/json/version` 404 通常是浏览器正在启动，轮询等待，不要重启。
7. stale websocket / stale socket 时执行一次：

```bash
smartclaw browser -c 'restart_daemon()'
```

## 沉淀可复用经验

针对特定网站的多次操作，如果有可复用信息，优先沉淀到 `~/.smartclaw/workspace/domain-skills/<site>/`：
...
> 终极模式（非必要不进入）
`smartclaw/browser/` 是当前 CDP 直连模式使用的内核实现：
