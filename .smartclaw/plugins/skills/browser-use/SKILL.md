...
该命令会检查 `smartclaw browser` 的 daemon 是否可用、Chrome/Chromium/Edge 是否运行，以及当前是否有可用的浏览器连接。
...
然后等待用户进一步指示。如果用户确认已开启后，不要立刻重跑 `smartclaw browser --doctor`；先执行一次 `smartclaw browser --setup`，或直接执行 `smartclaw browser -c 'print(page_info())'` 触发 attach，再运行 `smartclaw browser --doctor` 做只读确认。
...
#### 结果 C：`smartclaw browser --doctor` 失败，或当前机器没有可用 Chrome/Chromium/Edge
...
- `references/cdp-direct.md`：以 `smartclaw browser` 作为 CDP 直连入口的启动方式、API、页面探索策略、错误处理
