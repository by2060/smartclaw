"""
Tests for the unified plugin system (smartclaw.plugin).
"""
...
class TestProjectLevelPlugins:
    """Tests for project-level .smartclaw/plugins/ scanning added in load_all()."""
    ...
    def test_load_all_scans_project_plugins(self, tmp_path: Path):
        """load_all() should pick up plugins from <project>/.smartclaw/plugins/ in addition
        to user-level ~/.smartclaw/plugins/."""
        # User-level plugin root (empty — nothing installed globally)
        user_root = tmp_path / "user_smartclaw" / "plugins"

        # Project-level plugin: tmp_path is the project dir
        project_tools_dir = tmp_path / ".smartclaw" / "plugins" / "tools"
    ...
    def test_load_all_skips_project_if_same_as_user_root(self, tmp_path: Path):
        ...
        # project_dir's .smartclaw/plugins == shared_root would only match if
        # tmp_path/.smartclaw/plugins == shared_root — which it doesn't here,
