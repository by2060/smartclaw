"""
Tests for SessionRunner internals in smartclaw/session/runner.py
...
import smartclaw.session.runner as runner_mod
...
def _make_agent(name="titan", tools=None):
    agent = MagicMock()
    agent.name = name
    agent.tools = tools
    return agent
...
async def test_direct_session_shell_blocks_high_risk_command(monkeypatch):
    ...
    monkeypatch.setattr("smartclaw.tool.code.bash_blacklist.check_bash_blacklist", fake_check_blacklist)
...
class TestAgentDeclaresTool:
    def test_agent_with_explicit_tools_allows_declared_tools(self):
        runner = _make_runner()
        agent = _make_agent(name="titan", tools=["bash", "read"])
    ...
    def test_non_titan_agent_defaults_to_deny(self):
        runner = _make_runner()
...
class TestBuildTools:
    ...
    async def test_excludes_invalid_tool(self):
        ...
        with patch(
            "smartclaw.session.runner.ToolRegistry.list_tools",
    ...
    async def test_tool_format_is_function_type(self):
        runner = _make_runner()
        agent = _make_agent(name="titan", tools=["bash"])
        ...
        with patch(
            "smartclaw.session.runner.SessionRunner._list_callable_tool_infos_for_turn",
    ...
    async def test_list_callable_tools_uses_current_session_id(self):
        ...
        with patch(
            "smartclaw.session.runner.list_session_callable_tool_infos",
            selector_mock,
        ):
    ...
    async def test_build_tools_rewrites_skill_description(self):
        ...
        with patch.object(
            ...
            "smartclaw.tool.system.skill.Skill.all",
            AsyncMock(return_value=[mock_skill]),
        ), patch(
            "smartclaw.tool.system.skill.build_description",
...
class TestBuildSystemPrompts:
    ...
    async def test_build_system_prompts_reuses_loop_static_cache(self):
        ...
        with patch("smartclaw.session.runner.SystemPrompt.provider", return_value=["provider prompt"]), \
             patch("smartclaw.session.runner.SystemPrompt.environment", env_mock), \
             patch("smartclaw.session.runner.SystemPrompt.custom", custom_mock), \
    ...
    async def test_build_system_prompts_rebuilds_when_tool_revision_changes(self):
        ...
        with patch("smartclaw.session.runner.ToolRegistry.revision", side_effect=[1, 2]), \
             patch("smartclaw.session.runner.SystemPrompt.provider", return_value=["provider prompt"]), \
             patch("smartclaw.session.runner.SystemPrompt.environment", env_mock), \
             patch("smartclaw.session.runner.SystemPrompt.custom", custom_mock), \
             patch.object(SessionRunner, "_build_sandbox_prompt", sandbox_mock), \
             patch.object(SessionRunner, "_build_channel_context_prompt", channel_mock), \
    ...
    async def test_build_system_prompts_labels_memory_as_human_user_memory(self):
        ...
        with patch("smartclaw.session.runner.SystemPrompt.provider", return_value=[]), \
             patch("smartclaw.session.runner.SystemPrompt.environment", AsyncMock(return_value=[])), \
             patch("smartclaw.session.runner.SystemPrompt.custom", AsyncMock(return_value=[])), \
    ...
    @pytest.mark.asyncio
    async def test_titan_system_prompt_cache_key_includes_allowed_subagents(self):
        shared_cache = {}
        session = _make_session("ses_prompts_titan_scope")
        session.user_context = {"allowedSubagents": ["alpha"]}
        runner = SessionRunner(session=session, static_cache=shared_cache)
        agent = AgentInfo(name="titan", mode="primary", prompt="fallback prompt")

        with patch("smartclaw.session.runner.ToolRegistry.revision", return_value=1), \
             patch("smartclaw.session.runner.SystemPrompt.provider", return_value=["provider prompt"]), \
             patch("smartclaw.session.runner.SystemPrompt.environment", AsyncMock(return_value=["env prompt"])), \
             patch("smartclaw.session.runner.SystemPrompt.custom", AsyncMock(return_value=["custom prompt"])), \
             patch.object(SessionRunner, "_build_sandbox_prompt", AsyncMock(return_value="")), \
             patch.object(SessionRunner, "_build_channel_context_prompt", AsyncMock(return_value="")), \
             patch.object(SessionRunner, "_get_tool_instructions", return_value="tool instructions"), \
    ...
    def test_build_tool_catalog_prompt_for_titan(self):
        runner = _make_runner()
        agent = _make_agent(name="titan")
        agent.mode = "primary"

        with patch(
            "smartclaw.session.runner.SessionRunner._list_catalog_tool_infos",
            ...
            "smartclaw.tool.system.slash_command.format_tools_catalog_summary",
    ...
    def test_list_catalog_tool_infos_filters_titan_to_declared_and_always_load(self):
        runner = _make_runner()
        agent = _make_agent(name="titan", tools=["read", "tool_search"])
        agent.mode = "primary"
        shell_tool = ToolInfo(
            name="bash",
        ...
        with patch(
            "smartclaw.session.runner.list_tool_catalog_infos",
    ...
    def test_list_catalog_tool_infos_filters_subagent_boundaries(self):
        ...
        with patch("smartclaw.session.runner.list_tool_catalog_infos", return_value=tool_infos):
            infos = runner._list_catalog_tool_infos(agent)
...
async def test_process_step_creates_assistant_message_with_provider_and_model(monkeypatch):
    ...
    agent = SimpleNamespace(name="titan", steps=None, permission=None)
    provider = MagicMock()
...
async def test_process_step_records_usage_after_success(monkeypatch):
    ...
    agent = SimpleNamespace(name="titan", steps=None, mode="primary", prompt="", tools=[])
    provider = MagicMock()
    provider.is_configured.return_value = True
...
async def test_record_usage_if_available_swallows_import_error():
    ...
    with patch.dict("sys.modules", {"smartclaw.provider.usage_service": None}):
...
async def test_record_usage_if_available_passes_message_id():
    ...
    with patch.dict("sys.modules", {"smartclaw.provider.usage_service": fake_module}):
