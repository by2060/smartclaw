...
* hard timeout on the background flush (a stuck provider call cannot pin
  references forever);
* opt-out via ``SMARTCLAW_COMPACTION_FLUSH_BACKGROUND=0`` (legacy inline mode);
...
from smartclaw.session.lifecycle.compaction import compaction as compaction_mod
from smartclaw.session.lifecycle.compaction import summary as summary_mod
...
async def _reset_flush_state(monkeypatch):
    ...
    await _hard_reset()
    monkeypatch.delenv("SMARTCLAW_COMPACTION_FLUSH_BACKGROUND", raising=False)
    monkeypatch.delenv("SMARTCLAW_COMPACTION_FLUSH_TIMEOUT", raising=False)
    yield
    await _hard_reset()
...
async def test_inline_mode_when_env_disables_background(monkeypatch):
    """``SMARTCLAW_COMPACTION_FLUSH_BACKGROUND=0`` falls back to await."""
    monkeypatch.setenv("SMARTCLAW_COMPACTION_FLUSH_BACKGROUND", "0")
...
async def test_background_flush_times_out(monkeypatch):
    ...
    monkeypatch.setenv("SMARTCLAW_COMPACTION_FLUSH_TIMEOUT", "0.05")
...
async def test_flush_timeout_parsing(monkeypatch, raw, expected):
    ...
    if raw is None:
        ...
        monkeypatch.setenv("SMARTCLAW_COMPACTION_FLUSH_TIMEOUT", raw)
...
async def test_summarize_chunked_runs_chunks_in_parallel(monkeypatch):
    ...
    monkeypatch.setenv("SMARTCLAW_COMPACTION_CHUNK_CONCURRENCY", "4")
...
async def test_summarize_chunked_concurrency_limit_respected(monkeypatch):
    ...
    monkeypatch.setenv("SMARTCLAW_COMPACTION_CHUNK_CONCURRENCY", "2")
