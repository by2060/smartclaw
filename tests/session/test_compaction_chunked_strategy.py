...
def test_strategy_chunk_size_floor_respected(monkeypatch):
    ...
    """
    monkeypatch.setenv("SMARTCLAW_COMPACTION_PREEMPTIVE_CHUNK_RATIO", "0.5")
    use_chunked, chunk_size, decision = compaction_mod._decide_chunked_strategy(
...
def test_strategy_env_overrides_take_effect(monkeypatch):
    ...
    monkeypatch.setenv("SMARTCLAW_COMPACTION_PREEMPTIVE_CHUNK_RATIO", "0.5")
    monkeypatch.setenv("SMARTCLAW_COMPACTION_TARGET_PARALLEL_CHUNKS", "5")
    monkeypatch.setenv("SMARTCLAW_COMPACTION_MIN_CHUNK_CHARS", "1000")
...
def test_strategy_invalid_env_falls_back_to_default(monkeypatch):
    ...
    entirely.
    """
    monkeypatch.setenv("SMARTCLAW_COMPACTION_PREEMPTIVE_CHUNK_RATIO", "not-a-number")
    monkeypatch.setenv("SMARTCLAW_COMPACTION_TARGET_PARALLEL_CHUNKS", "-1")
    monkeypatch.setenv("SMARTCLAW_COMPACTION_MIN_CHUNK_CHARS", "0")
