...
import pytest
from smartclaw.auth.context import AuthUser, reset_current_auth_user, set_current_auth_user
from smartclaw.session.session import Session
from smartclaw.session.message import Message, MessageInfo, MessageRole, TokenUsage
from smartclaw.session.callable_state import add_session_callable_tools, get_session_callable_tools
...
async def test_session_create_initializes_callable_tools_from_declared_agent_tools(
    ...
    monkeypatch.setattr(
        "smartclaw.session.callable_state.initialize_session_callable_tools",
        initialize_mock,
    )
    monkeypatch.setattr(
        "smartclaw.tool.catalog.get_always_load_tool_names",
...
async def test_message_create():
    ...
    assert message.sessionID == "test_session"  # SmartClaw uses sessionID
    assert message.role == "user"  # role is a string in SmartClaw
...
async def test_agent_list():
    ...
    assert len(agents) >= 3
    assert any(a.name == "titan" for a in agents)
...
async def test_agent_system_prompt():
    ...
    # plan agent doesn't have a custom prompt (matches SmartClaw)
