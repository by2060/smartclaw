"""
Tests for smartclaw/session/streaming/stream_processor.py
...
from smartclaw.tool.registry import ToolResult
from smartclaw.session.streaming.stream_processor import StreamProcessor, ToolCallState
...
from smartclaw.session.message import MessageRole
...
class TestTextAccumulation:
    ...
    async def test_text_end_finalizes_part(self):
        proc = _make_processor()
        with patch("smartclaw.session.streaming.stream_processor.Message.store_part", new=AsyncMock()):
            await proc.process_event(TextStartEvent())
...
"""))
    ...
    async def test_text_end_parses_minimax_tool_call_xml(self):
        ...
        with (
            patch("smartclaw.session.streaming.stream_processor.Message.store_part", new=AsyncMock()),
...
class TestToolInputStart:
    ...
    async def test_creates_tool_part_in_db(self):
        proc = _make_processor()
        with patch("smartclaw.session.streaming.stream_processor.Message.store_part", new=AsyncMock()) as mock_store:
...
class TestToolCallExecution:
    ...
    async def test_tool_call_executes_tool(self):
        ...
        with (
            patch("smartclaw.session.streaming.stream_processor.Message.store_part", new=AsyncMock()),
            patch("smartclaw.session.streaming.stream_processor.Message.update_part", new=AsyncMock()),
            patch(
                "smartclaw.session.streaming.stream_processor.ToolRegistry.execute",
    ...
    async def test_tool_call_rejects_non_callable_tool_for_agent_without_tool_search(self):
        ...
        with (
            patch("smartclaw.session.streaming.stream_processor.Message.store_part", new=AsyncMock()) as mock_store,
            patch("smartclaw.session.streaming.stream_processor.Message.update_part", new=AsyncMock()),
            patch("smartclaw.session.streaming.stream_processor.ToolRegistry.execute", new=execute_mock),
            patch("smartclaw.agent.registry.Agent.get", new=AsyncMock(return_value=proc.agent)),
        ):
    ...
    @pytest.mark.asyncio
    async def test_non_titan_agent_can_execute_declared_tool_search(self, monkeypatch: pytest.MonkeyPatch):
        proc = _make_processor(session_id="ses_tool_search_allowed")
        proc.agent.name = "self-enhance"
        proc.agent.tools = ["tool_search"]

        monkeypatch.setattr(
            "smartclaw.session.callable_state.get_session_callable_tools",
