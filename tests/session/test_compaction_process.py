"""
Tests for SessionCompaction in smartclaw/session/lifecycle/compaction.py
...
class TestPrune:
    ...
    async def test_prune_disabled_does_nothing(self):
        ...
        with patch("smartclaw.session.message.Message.list", new=mock_list):
    ...
    async def test_prune_with_policy_no_messages(self):
        ...
        policy = CompactionPolicy.from_model(200000, 8192)
        with patch("smartclaw.session.message.Message.list", new=AsyncMock(return_value=[])):
