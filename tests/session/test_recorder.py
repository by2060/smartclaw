"""
Tests for smartclaw/session/recorder.py
...
from smartclaw.session.recorder import Recorder, _safe_truncate, _record_dir
...
class TestRecordDir:
    def test_uses_env_var_when_set(self, tmp_path):
        with patch.dict(os.environ, {"SMARTCLAW_RECORD_DIR": str(tmp_path)}):
            result = _record_dir()
        assert result == tmp_path

    def test_uses_config_path_when_no_env(self):
        with patch.dict(os.environ, {}, clear=False):
            if "SMARTCLAW_RECORD_DIR" in os.environ:
                del os.environ["SMARTCLAW_RECORD_DIR"]
            result = _record_dir()
...
class TestRecorderPaths:
    def test_paths_returns_record_paths(self, tmp_path):
        with patch("smartclaw.session.recorder._record_dir", return_value=tmp_path):
...
class TestLockLruEviction:
    ...
    async def test_locks_evicted_when_cap_exceeded(self, tmp_path):
        ...
        from smartclaw.session.recorder import _MAX_FILE_LOCKS
