...
from smartclaw.session.core.context import SessionContext, DefaultSessionContext
from smartclaw.session.session_loop import LoopContext
...
class TestDefaultSessionContext:
    ...
    async def test_get_messages_delegates_to_message(self):
        ...
        with patch("smartclaw.session.message.Message.list", new_callable=AsyncMock, return_value=mock_messages):
    ...
    async def test_store_message_delegates_to_message(self):
        ...
        mock_msg = MagicMock()
        with patch("smartclaw.session.message.Message.create", new_callable=AsyncMock, return_value=mock_msg):
    ...
    async def test_update_status_busy(self):
        ...
        with patch("smartclaw.session.core.status.SessionStatus.set") as mock_set:
    ...
    async def test_update_status_clear(self):
        ...
        with patch("smartclaw.session.core.status.SessionStatus.clear") as mock_clear:
    ...
    async def test_touch_delegates_to_session(self):
        ...
        with patch("smartclaw.session.session.Session.touch", new_callable=AsyncMock) as mock_touch:
            await ctx.touch()
