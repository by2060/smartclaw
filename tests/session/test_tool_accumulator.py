"""
Tests for smartclaw/session/streaming/tool_accumulator.py
...
from smartclaw.session.streaming.stream_events import ToolInputStartEvent, ToolCallEvent
...
class TestFeedChunkBasic:
    ...
    async def test_complete_json_triggers_tool_call_event(self):
        ...
        with patch("smartclaw.session.streaming.tool_accumulator.ToolRegistry") as mock_reg:
...
class TestFlushRemaining:
    ...
    async def test_flush_invalid_json_sends_invalid_tool(self):
        ...
        with patch("smartclaw.session.streaming.tool_accumulator.ToolRegistry") as mock_reg:
            mock_reg.get_schema.return_value = None
            mock_reg.get.return_value = None
            with patch("smartclaw.session.streaming.tool_accumulator._find_similar_tool", return_value=None):
...
class TestFindSimilarTool:
    def test_exact_case_insensitive_match(self):
        from smartclaw.session.streaming.tool_accumulator import _find_similar_tool
        with patch("smartclaw.session.streaming.tool_accumulator.ToolRegistry") as mock_reg:
            mock_tool = MagicMock()
