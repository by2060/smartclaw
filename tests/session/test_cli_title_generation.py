...
def _patch_title_deps(mock_session, user_msgs, parts, mock_provider, mock_update=None):
    ...
    return [
        patch("smartclaw.session.session.Session.get_by_id", new=AsyncMock(return_value=mock_session)),
        patch("smartclaw.session.session.Session.is_default_title", return_value=True),
        patch("smartclaw.session.session.Session.update", new=mock_update),
        patch("smartclaw.session.message.Message.list", new=AsyncMock(return_value=user_msgs)),
        patch("smartclaw.session.message.Message.parts", new=AsyncMock(return_value=parts)),
        patch("smartclaw.provider.provider.Provider._ensure_initialized"),
        patch("smartclaw.provider.provider.Provider.get", return_value=mock_provider),
    ]
...
class TestGenerateTitleAfterFirstMessage:
    ...
    async def test_skips_when_more_than_one_user_message(self):
        ...
        with (
            patch("smartclaw.session.session.Session.get_by_id", new=AsyncMock(return_value=mock_session)),
            patch("smartclaw.session.session.Session.is_default_title", return_value=True),
            patch("smartclaw.session.message.Message.list", new=AsyncMock(return_value=[msg1, msg2])),
        ):
    ...
    async def test_skips_when_title_already_set(self):
        ...
        with (
            patch("smartclaw.session.session.Session.get_by_id", new=AsyncMock(return_value=mock_session)),
            patch("smartclaw.session.session.Session.is_default_title", return_value=False),
        ):
            result = await SessionTitle.generate_title_after_first_message(
                session_id="sess-1",
                model_id="claude-3",
...
class TestPromptTitleLanguageRule:
    ...
    def test_prompt_contains_language_rule(self):
        from smartclaw.session.prompt_strings import PROMPT_TITLE
    ...
    async def test_language_rule_is_sent_to_llm(self):
        ...
        from smartclaw.session.lifecycle.title import SessionTitle, _CANONICAL_TITLE_PROMPT
...
class TestProcessMessageTitleGeneration:
    ...
    async def test_title_generated_after_successful_loop(self):
        ...
        from smartclaw.session.session_loop import LoopResult
        import smartclaw.session.session_loop as session_loop_mod
        import smartclaw.session.message as message_mod
        import smartclaw.agent.registry as agent_mod
        import smartclaw.provider.provider as provider_mod
        import smartclaw.session.lifecycle.title as title_mod
        import smartclaw.config.config as config_mod
        ...
        with (
            ...
            patch.object(agent_mod.Agent, "default_agent", new=AsyncMock(return_value="titan")),
            ...
            patch("smartclaw.cli.session_runner.Live", return_value=mock_live_ctx),
            patch("smartclaw.cli.session_runner.Spinner"),
        ):
