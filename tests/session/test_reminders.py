"""
Tests for smartclaw/session/features/reminders.py
...
class TestSessionRemindersExtractOriginalTask:
    ...
    async def test_extracts_first_user_message(self):
        ...
        with patch(
            "smartclaw.session.message.Message.get_text_content",
    ...
    async def test_skips_non_user_messages(self):
        from unittest.mock import AsyncMock, patch, MagicMock
        from smartclaw.session.message import MessageRole, Message
        ...
        with patch("smartclaw.session.message.Message.get_text_content", new=AsyncMock(side_effect=tracking_fn)):
