"""
Tests for smartclaw/session/prompt.py
...
from smartclaw.session.prompt import SessionPrompt, SystemPrompt, PromptTemplate
from smartclaw.session import prompt as prompt_module
from smartclaw.session import prompt_strings
from smartclaw.session.prompt_locale import get_prompt_locale
...
class TestSystemPromptProvider:
    def test_prompt_locale_defaults_to_english(self, monkeypatch):
        monkeypatch.delenv("SMARTCLAW_SESSION_PROMPT_LOCALE", raising=False)
        monkeypatch.delenv("SMARTCLAW_PROMPT_LOCALE", raising=False)
        monkeypatch.delenv("SMARTCLAW_SESSION_PROMPT_AUTO_LOCALE", raising=False)

        assert get_prompt_locale() == "en-US"

    def test_prompt_locale_accepts_chinese_env(self, monkeypatch):
        monkeypatch.setenv("SMARTCLAW_SESSION_PROMPT_LOCALE", "zh_CN")
    ...
    def test_load_prompt_file_prefers_chinese_variant(self):
        ...
        assert prompt.startswith("你是 SmartClaw")
