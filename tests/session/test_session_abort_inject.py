...
from smartclaw.session.session_loop import SessionLoop, LoopCallbacks, LoopContext, LoopResult
...
class TestTurnLifecycle:
    ...
    async def test_pre_compact_cleanup_emits_turn_continued_before_next_iteration(self):
        ...
        with patch(
            "smartclaw.session.session_loop.Provider.resolve_model_info",
            return_value=(20000, 1024, None),
        ), patch(
            "smartclaw.session.session_loop.SessionCompaction.truncate_oversized_tool_outputs",
            AsyncMock(return_value=1),
        ), patch(
            "smartclaw.session.session_loop.SessionPrompt.estimate_full_context_tokens",
            AsyncMock(return_value=0),
        ), patch(
            "smartclaw.session.runner.SessionRunner._process_step",
...
class TestExecuteSubtask:
    ...
    async def test_execute_subtask_passes_tool_context_first(self):
        ...
        with patch("smartclaw.agent.registry.Agent.get", AsyncMock(return_value=SimpleNamespace(name="helper"))), \
             patch("smartclaw.tool.registry.ToolRegistry.get", return_value=task_tool), \
             patch("smartclaw.session.session_loop.Message.create", AsyncMock(side_effect=[assistant_msg, synthetic_msg])), \
             patch("smartclaw.session.session_loop.Message.add_part", AsyncMock()), \
             patch("smartclaw.session.session_loop.Message.update", AsyncMock()), \
             patch("smartclaw.session.session_loop.Message.update_part", AsyncMock()):
