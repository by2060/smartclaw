...
def tmp_workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """
    Provide an isolated workspace directory via SMARTCLAW_WORKSPACE_DIR.

    Also overrides SMARTCLAW_DATA_DIR so the memory-dir view never touches ~/.smartclaw.
    """
    ...
    monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(ws))
    monkeypatch.setenv("SMARTCLAW_DATA_DIR", str(data))
...
class TestDirectoryPaths:
    ...
    def test_workspace_dir_env_source_root_falls_back_to_user_workspace(
        ...
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(project))
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home)
        WorkspaceManager._instance = None
        Config._global_config = None
        try:
            manager = WorkspaceManager.get_instance()
            assert manager.get_workspace_dir() == home / ".smartclaw" / "workspace"
            assert (
                manager.get_outputs_dir("ses_same", day="2026-05-07", create=False)
                == home / ".smartclaw" / "workspace" / "outputs" / "2026-05-07" / "ses_same"
            )
        finally:
            WorkspaceManager._instance = None
            Config._global_config = None

    def test_workspace_dir_named_smartclaw_falls_back_to_user_workspace(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        from smartclaw.config.config import Config
        from smartclaw.workspace.manager import WorkspaceManager

        deploy_root = tmp_path / "opt" / "zhhtest" / "smartclaw"
        home = tmp_path / "home"
        deploy_root.mkdir(parents=True)
        home.mkdir()
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(deploy_root))
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home)
        WorkspaceManager._instance = None
        Config._global_config = None
        try:
    ...
    def test_home_env_pointing_at_project_does_not_define_user_workspace(
        ...
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(deploy_root / ".smartclaw" / "workspace"))
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: real_home)
        WorkspaceManager._instance = None
        Config._global_config = None
        try:
            manager = WorkspaceManager.get_instance()
            assert manager.get_workspace_dir() == real_home / ".smartclaw" / "workspace"
            assert (
    ...
    def test_project_smartclaw_workspace_falls_back_to_user_workspace(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        from smartclaw.config.config import Config
        from smartclaw.workspace.manager import WorkspaceManager
        ...
        project = tmp_path / "project"
        home = tmp_path / "home"
        project_workspace = project / ".smartclaw" / "workspace"
        project_workspace.mkdir(parents=True)
        home.mkdir()
        (project / "AGENTS.md").write_text("# rules\n")
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(project_workspace))
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home)
        WorkspaceManager._instance = None
        Config._global_config = None
        try:
    ...
    def test_project_smartclaw_dir_falls_back_to_user_workspace(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        from smartclaw.config.config import Config
        from smartclaw.workspace.manager import WorkspaceManager
        ...
        project = tmp_path / "project"
        home = tmp_path / "home"
        project_smartclaw = project / ".smartclaw"
        project_smartclaw.mkdir(parents=True)
        home.mkdir()
        (project / "AGENTS.md").write_text("# rules\n")
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(project_smartclaw))
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home)
        WorkspaceManager._instance = None
        Config._global_config = None
        try:
    ...
    def test_project_uploads_dir_falls_back_to_user_workspace(
        ...
        home.mkdir()
        (project / "pyproject.toml").write_text("[project]\nname = \"demo\"\n")
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(project_uploads))
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home)
        WorkspaceManager._instance = None
        Config._global_config = None
        try:
    ...
    def test_resolve_user_workspace_path_ignores_project_workspace_env(
        ...
        try:
            ...
            assert manager.resolve_user_workspace_path("uploads/chat/ses_a") == (
                home / ".smartclaw" / "workspace" / "uploads" / "chat" / "ses_a"
            )
        finally:
            WorkspaceManager._instance = None
            Config._global_config = None
    ...
    def test_get_outputs_dir_is_session_scoped(
        ...
        assert out == home / ".smartclaw" / "workspace" / "outputs" / "2026-05-06" / "ses_abc123"
        assert out.is_dir()
    ...
    def test_get_outputs_dir_sanitizes_session_id(
        ...
        assert out == home / ".smartclaw" / "workspace" / "outputs" / "2026-05-06" / "ses_bad"
        assert out.is_dir()
    ...
    def test_rewrite_legacy_workspace_output_path(
        ...
        home.mkdir()
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home)
        legacy = home / ".smartclaw" / "workspace" / "outputs" / "2026-05-06" / "report.md"
    ...
    def test_rewrite_sandbox_smartclaw_outputs_alias(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, tmp_workspace: Path, manager
    ):
        home = tmp_path / "home"
        home.mkdir()
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home)
        legacy = Path("/workspace/.smartclaw_outputs/2026-05-07/baseline_192.168.185.174.md")
        rewritten = manager.rewrite_legacy_output_path(legacy, "ses_abc123")
        assert rewritten == (
            home
            / ".smartclaw"
    ...
    def test_rewrite_project_smartclaw_workspace_outputs_to_user_workspace(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        from smartclaw.config.config import Config
        from smartclaw.workspace.manager import WorkspaceManager
