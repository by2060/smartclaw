...
def workspace_client(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    - Config._global_config       (smartclaw config module singleton, caches data_dir)
    """
    ...
    from smartclaw.server.routes.workspace import router
...
def _user_ws(fixture): return fixture[1].parent / "home" / ".smartclaw" / "workspace"
...
class TestUpload:
    ...
    def test_chat_upload_returns_error_after_too_many_name_conflicts(self, workspace_client, monkeypatch):
        from smartclaw.server.routes import workspace as workspace_routes
    ...
    def test_upload_too_large_file_rejected(self, workspace_client, monkeypatch):
        ...
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_MAX_UPLOAD_MB", "0")
        client = _client(workspace_client)
        r = client.post(
            "/api/workspace/upload",
