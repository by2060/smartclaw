...
import smartclaw.cli.commands.update as update_cmd
import smartclaw.cli.main as cli_main
import smartclaw.cli.service_manager as service_manager
import smartclaw.updater as updater_pkg
...
def test_update_prompts_for_cn_mirror_before_upgrade_confirmation(monkeypatch) -> None:
    ...
    async def fake_check_update(*, locale: str | None = None, region: str | None = None) -> VersionInfo:
        ...
        zipball_url = "https://example.com/smartclaw.zip"
        tarball_url = "https://example.com/smartclaw.tar.gz"
        if region == "cn":
            zipball_url = "https://gitee.example.com/smartclaw.zip"
            tarball_url = "https://gitee.example.com/smartclaw.tar.gz"
        return VersionInfo(
...
def test_update_executes_smartclaw_stop_before_upgrade(monkeypatch) -> None:
    output = StringIO()
    monkeypatch.setattr(
        update_cmd,
        "console",
    ...
    assert "已执行 smartclaw stop" in output.getvalue()
