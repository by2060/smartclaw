...
import smartclaw.security as security_module
...
def test_hidden_serve_command_is_not_listed_but_still_invocable(monkeypatch, tmp_path) -> None:
    ...
    assert "Start the SmartClaw API server" in hidden_help.stdout
...
def test_tui_starts_hidden_serve_command(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: [cli_main.sys.executable, "-m", "smartclaw.cli.main"],
    )
    ...
    assert result.exit_code == 0
    assert popen_calls["args"][:4] == [cli_main.sys.executable, "-m", "smartclaw.cli.main", "serve"]
