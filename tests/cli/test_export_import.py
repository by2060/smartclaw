...
import smartclaw.cli.commands.export as export_cmd
import smartclaw.cli.commands.import_ as import_cmd
import smartclaw.mcp.server as mcp_server
from smartclaw.session.message import Message, MessageWithParts, TextPart, UserMessageInfo
...
from smartclaw.server.client import SessionClient
...
def _build_session(project_id: str = "proj_source") -> SessionInfo:
    return SessionInfo(
        ...
        directory="/tmp/smartclaw-project",
...
async def test_session_client_messages_returns_info_and_parts(monkeypatch) -> None:
    ...
    monkeypatch.setattr("smartclaw.server.client.Message.list_with_parts", fake_list_with_parts)

    client = SessionClient(base_url="http://127.0.0.1:8000")
    result = await client.messages(session.id, "/tmp/smartclaw-project")
