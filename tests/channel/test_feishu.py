...
from smartclaw.channel.base import ChatType, InboundMessage, OutboundContext
from smartclaw.channel.builtin.feishu.channel import FeishuChannel
from smartclaw.channel.builtin.feishu.config import (
    list_account_configs,
    merge_group_overrides,
    resolve_webhook_account_config,
)
from smartclaw.channel.builtin.feishu.debounce import get_debouncer
from smartclaw.channel.builtin.feishu.dedup import FeishuDedup
from smartclaw.channel.builtin.feishu.inbound_media import download_inbound_media
from smartclaw.channel.builtin.feishu.media import send_media_feishu
from smartclaw.channel.builtin.feishu.monitor import _build_ws_client, _start_single_websocket
from smartclaw.channel.builtin.feishu.send import send_message_feishu
from smartclaw.channel.inbound.dispatcher import _resolve_feishu_group_overrides
from smartclaw.config.config import ChannelConfig, FeishuGroupConfig
...
async def test_feishu_channel_start_runs_websocket_for_mixed_transport(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.monitor.start_websocket",
        start_websocket,
    )
...
async def test_handle_webhook_uses_resolved_account_and_fetches_bot_identity(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.dedup.get_dedup",
        AsyncMock(return_value=fake_dedup),
    )
    monkeypatch.setattr(channel, "_ensure_webhook_dedup_ready", AsyncMock())
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.identity.get_cached_bot_open_id",
        lambda account_id="default": None,
    )
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.identity.get_bot_identity",
        AsyncMock(return_value=("ou_bot", "bot")),
    )
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.monitor._parse_event",
        fake_parse_event,
    )
...
async def test_get_bot_identity_parses_top_level_bot_payload(monkeypatch) -> None:
    from smartclaw.channel.builtin.feishu import identity as identity_module

    identity_module._identity_cache.clear()

    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.client.api_request_for_account",
        AsyncMock(
            return_value={
                "code": 0,
                "msg": "ok",
...
async def test_channel_route_uses_plugin_status_code(monkeypatch) -> None:
    from smartclaw.server.routes.channel import channel_webhook
    ...
    monkeypatch.setattr(
        "smartclaw.server.routes.channel.default_registry.get",
...
async def test_websocket_card_action_events_are_deduplicated(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.monitor._parse_card_action_event",
...
def test_build_ws_client_falls_back_to_modern_sdk(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.monitor.importlib.import_module",
        fake_import_module,
    )
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.monitor.asyncio.run_coroutine_threadsafe",
...
async def test_parse_reaction_event_falls_back_to_user_id(monkeypatch) -> None:
    from smartclaw.channel.builtin.feishu.monitor import _parse_reaction_event

    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.monitor._is_bot_message",
...
async def test_download_inbound_media_saves_message_resource(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.inbound_media._get_http_client",
        AsyncMock(return_value=_FakeClient()),
    )
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.inbound_media.get_tenant_token",
        AsyncMock(return_value="token_1"),
    )
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.inbound_media._media_storage_dir",
...
async def test_send_media_feishu_preserves_reply_to_id(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.send.send_payload_feishu",
        fake_send_payload,
    )
...
async def test_send_message_feishu_requires_message_id(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.send.api_request_for_account",
...
async def test_sender_name_cache_isolated_by_account(monkeypatch) -> None:
    from smartclaw.channel.builtin.feishu import sender_name as sender_name_module
...
def test_group_allowlist_without_entries_blocks_group() -> None:
    from smartclaw.channel.builtin.feishu.monitor import _check_group_policy
...
async def test_channel_send_media_passes_reply_to_id(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "smartclaw.channel.builtin.feishu.media.send_media_feishu",
