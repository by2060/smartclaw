...
Layout:
  - send library    → smartclaw.channel.builtin.dingtalk.{config,client,send}
  - stream inbound  → smartclaw.channel.builtin.dingtalk.stream
  - channel plugin  → smartclaw.channel.builtin.dingtalk.channel.DingTalkChannel
"""
...
from smartclaw.channel.base import ChatType, OutboundContext
from smartclaw.channel.builtin.dingtalk.client import (
    DingTalkApiError,
    ensure_api_success,
)
from smartclaw.channel.builtin.dingtalk.config import (
    list_account_configs,
    resolve_account_credentials,
    resolve_target_kind,
    strip_target_prefix,
)
from smartclaw.channel.builtin.dingtalk.send import (
    build_app_payload,
    send_message_app,
)
...
class TestConfigHelpers:
    ...
    def test_robot_code_defaults_to_app_key(self):
        ...
        # so users don't have to repeat themselves in smartclaw.json.
...
class TestSendApp:
    async def test_user_target_uses_oto_endpoint(self):
        ...
        cfg = {"appKey": "k", "appSecret": "s", "robotCode": "r"}
        with patch(
            "smartclaw.channel.builtin.dingtalk.send.api_request_for_account",
...
class TestBuiltinChannelPlugin:
    ...
    def test_builtin_channel_module_exists(self):
        spec = importlib.util.find_spec(
            "smartclaw.channel.builtin.dingtalk.channel"
        )
        assert spec is not None

    def test_validate_config_accepts_client_id_alias(self):
        from smartclaw.channel.builtin.dingtalk import DingTalkChannel
    ...
    async def test_send_text_delegates_to_send_message_app(self):
        ...
        with patch(
            "smartclaw.channel.builtin.dingtalk.send_message_app",
            new=fake_send,
        ):
    ...
    async def test_wait_until_done_suppresses_permanent_auth_error(self):
        ...
        """
        from smartclaw.channel.builtin.dingtalk import DingTalkChannel
        from smartclaw.channel.builtin.dingtalk.stream import (
...
class TestStreamHelpers:
    def test_message_gate_dm_always_processes(self):
        from smartclaw.channel.builtin.dingtalk.stream import _MessageGate
    ...
    async def test_preflight_raises_on_invalid_credentials(self):
        ...
        from smartclaw.channel.builtin.dingtalk import stream as stream_mod
    ...
    def test_chatbot_message_to_inbound_dm_uses_staff_id_as_chat_id(self):
        ...
        """
        from smartclaw.channel.builtin.dingtalk.config import resolve_target_kind
        from smartclaw.channel.builtin.dingtalk.stream import (
            chatbot_message_to_inbound,
        )
        message = SimpleNamespace(
    ...
    def test_resolve_chat_id_dm_prefers_staff_id(self):
        from smartclaw.channel.builtin.dingtalk.stream import _resolve_chat_id
        msg = SimpleNamespace(
...
class TestStreamRunnerResilience:
    ...
    def _make_runner(self, *, on_message=None, account_overrides=None):
        from smartclaw.channel.builtin.dingtalk.stream import DingTalkStreamRunner
...
class TestSessionBindingServiceBindSession:
    ...
    async def test_bind_session_inserts_row_for_existing_session(self, monkeypatch):
        from smartclaw.channel.inbound import session_binding as sb_mod

        svc = sb_mod.SessionBindingService()

        monkeypatch.setattr(
            "smartclaw.session.session.Session.get_by_id",
            AsyncMock(return_value=SimpleNamespace(id="ses_42", agent="titan")),
        )
...
class TestBindEndpoint:
    ...
    def client(self):
        ...
        from smartclaw.server.routes.channel import router
    ...
    def test_bind_endpoint_calls_service_and_returns_payload(self, client, monkeypatch):
        from smartclaw.channel.base import ChatType as _ChatType
        from smartclaw.channel.inbound import session_binding as sb_mod
