...
from smartclaw.channel.base import (
    ChannelCapabilities,
    ChannelMeta,
    ChannelPlugin,
    ChannelStatus,
...
from smartclaw.config.config import ChannelAccountConfig, ChannelConfig, ConfigInfo
from smartclaw.utils.rate_limiter import AsyncTokenBucket
...
class TestMessageDedup:
    def test_first_message_not_duplicate(self):
        from smartclaw.channel.inbound.dispatcher import MessageDedup
...
class TestFeishuNativeCommands:
    ...
    async def test_status_command_reports_session_state(self, monkeypatch):
        from smartclaw.channel.inbound.dispatcher import InboundDispatcher
        from smartclaw.channel.inbound.session_binding import SessionBinding
        ...
        monkeypatch.setattr(
            "smartclaw.channel.outbound.deliver.OutboundDelivery.deliver",
            fake_deliver,
        )
        monkeypatch.setattr(
            "smartclaw.session.session.Session.get_by_id",
            AsyncMock(
        ...
        monkeypatch.setattr(
            "smartclaw.session.session_loop.SessionLoop._resolve_model",
            AsyncMock(return_value=("anthropic", "claude-sonnet-4-20250514")),
        )
        monkeypatch.setattr(
            "smartclaw.session.core.status.SessionStatus.get",
    ...
    async def test_model_command_updates_session_model(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "smartclaw.provider.provider.Provider.get",
            lambda provider_id: object() if provider_id == "anthropic" else None,
        )
        monkeypatch.setattr(
            "smartclaw.provider.provider.Provider.list_models",
            lambda provider_id=None: [SimpleNamespace(id="claude-sonnet-4-20250514")],
        )
        monkeypatch.setattr(
            "smartclaw.session.session.Session.update",
            fake_update,
        )
    ...
    async def test_new_command_rebinds_conversation(self, monkeypatch):
        ...
        monkeypatch.setattr("smartclaw.session.session.Session.create", create_mock)
    ...
    async def test_append_user_message_stores_feishu_media_part(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "smartclaw.session.message.Message.create",
            AsyncMock(return_value=created_message),
        )
        monkeypatch.setattr(
            "smartclaw.session.message.Message.store_part",
            store_part,
        )
        monkeypatch.setattr(
            "smartclaw.channel.builtin.feishu.inbound_media.download_inbound_media",
            AsyncMock(
                return_value=SimpleNamespace(
...
class TestMultimodalInput:
    ...
    async def test_runner_builds_multimodal_user_message_for_image_parts(self, tmp_path, monkeypatch):
        from smartclaw.session.message import FilePart, MessageRole, TextPart
        ...
        monkeypatch.setattr(
            "smartclaw.session.runner.Message.parts",
            AsyncMock(
                return_value=[
                    TextPart(
                        sessionID="session_1",
    ...
    async def test_runner_extracts_plain_text_file_content(self, tmp_path, monkeypatch):
        from smartclaw.session.message import FilePart, MessageRole
        from smartclaw.session.runner import SessionRunner
    ...
    async def test_runner_extracts_pdf_content(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "smartclaw.session.utils.file_extractor.extract_pdf_text_from_bytes",
...
class TestCheckAllowlist:
    ...
    def test_dm_open_allows_all(self):
        from smartclaw.channel.inbound.dispatcher import _check_allowlist
...
class TestChannelDeliveryCallbacks:
    def test_build_ctx(self):
        from smartclaw.channel.inbound.dispatcher import ChannelDeliveryCallbacks
    ...
    async def test_deliver_text_empty_noop(self):
        ...
        with patch("smartclaw.channel.outbound.deliver.OutboundDelivery.deliver") as mock:
...
class TestOutboundDelivery:
    async def test_deliver_unregistered_channel(self):
        ...
        with patch("smartclaw.channel.outbound.deliver.default_registry") as reg:
    ...
    async def test_deliver_single_chunk(self):
        ...
        with patch("smartclaw.channel.outbound.deliver.default_registry") as reg, \
             patch("smartclaw.channel.outbound.deliver.HookPipeline") as hp:
...
class TestChannelRegistry:
    def test_register_and_get(self):
        from smartclaw.channel.registry import ChannelRegistry
        reg = ChannelRegistry()
...
class TestGatewayManagerHelpers:
    async def test_sleep_or_abort_returns_false_on_timeout(self):
        from smartclaw.channel.gateway.manager import GatewayManager
        event = asyncio.Event()
    ...
    async def test_mark_connected_sets_flag(self):
        ...
        with patch("smartclaw.bus.bus.Bus.publish", new_callable=AsyncMock):
    ...
    async def test_stop_all_drains_cancelled_tasks(self, monkeypatch):
        ...
        monkeypatch.setattr("smartclaw.channel.gateway.manager.asyncio.wait", _fake_wait)
