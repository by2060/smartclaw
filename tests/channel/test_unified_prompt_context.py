...
These cover the three knobs introduced to make Titan see the same system
...
2. ``default_agent`` — when the channel config does not specify an agent,
   ...
   the global ``defaultAgent`` config and finally returns ``titan``) instead
...
from smartclaw.channel.base import ChatType, InboundMessage
from smartclaw.channel.inbound import session_binding as binding_mod
from smartclaw.channel.inbound.dispatcher import (
    InboundDispatcher,
    _resolve_session_model,
)
from smartclaw.channel.inbound.session_binding import (
...
class TestSessionBindingDirectoryPropagation:
    ...
    async def test_create_session_uses_explicit_directory(self):
        ...
        with patch("smartclaw.session.session.Session.create", new=_fake_create):
    ...
    async def test_create_session_falls_back_to_instance(self):
        ...
        with patch("smartclaw.session.session.Session.create", new=_fake_create), \
            ...
            await SessionBindingService._create_session(
                _msg(), default_agent="titan", directory=None,
            )
...
class TestDispatcherDefaultAgentResolution:
    ...
    async def test_channel_config_default_agent_takes_precedence(self):
        ...
        with patch(
            ...
            new=AsyncMock(return_value="titan"),
        ) as mock_default:
    ...
    @pytest.mark.asyncio
    async def test_falls_back_to_titan_when_resolver_raises(self):
        cfg = ChannelConfig()
...
class TestResolveSessionModel:
    ...
    async def test_returns_provider_model_dict(self):
        fake_session = SimpleNamespace(
            id="ses_x", provider=None, model=None, agent="titan",
        ...
        with patch("smartclaw.session.session.Session.get_by_id", new=_fake_get), \
             patch(
    ...
    async def test_returns_none_when_session_missing(self):
        ...
        with patch("smartclaw.session.session.Session.get_by_id", new=_missing):
...
class TestAppendUserMessagePersistsModel:
    ...
    async def test_model_is_passed_to_message_create(self):
        ...
        with patch("smartclaw.session.message.Message.create", new=_fake_create):
    ...
    async def test_agent_is_passed_to_message_create(self):
        # Without this, last_user.agent silently defaults to "titan" inside
    ...
    async def test_omits_agent_when_none_or_empty(self):
        ...
        # When omitted, Message.create's own default ("titan") kicks in —
...
class TestLegacyBindingAgentIdMigration:
    ...
    ``Agent.get(...) or Agent.get("titan")`` fallback masks it. We rewrite
    ...
    async def test_rewrites_default_and_empty_to_null(self, db):
        ...
        await self._insert(db, agent_id="titan", chat_id="c3", session_id="s3")
    ...
    async def test_is_noop_when_no_legacy_rows(self, db):
        await self._insert(db, agent_id="titan", chat_id="c1", session_id="s1")
