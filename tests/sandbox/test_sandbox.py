...
class TestSandboxConfig:
    ...
    def test_default_config(self):
        """默认配置: mode=off."""
        from smartclaw.sandbox.config import resolve_sandbox_config_for_agent
    ...
    def test_agent_override(self):
        ...
        cfg = resolve_sandbox_config_for_agent(config_data, agent_id="titan")
        assert cfg.mode == "on"
    ...
    def test_scope_resolution(self):
        """scope 解析."""
        from smartclaw.sandbox.config import resolve_sandbox_scope
...
class TestRuntimeStatus:
    ...
    def test_mode_off(self):
        """mode=off 不沙箱化."""
        from smartclaw.sandbox.runtime_status import resolve_sandbox_runtime_status
...
class TestPaths:
    ...
    def test_resolve_within_root(self):
        """正常路径解析."""
        from smartclaw.sandbox.paths import resolve_sandbox_path
    ...
    def test_data_url_rejection(self):
        """data: URL 拒绝."""
        from smartclaw.sandbox.paths import assert_media_not_data_url
...
class TestToolPolicy:
    ...
    def test_default_policy_allows_bash(self):
        """默认策略允许 bash."""
        from smartclaw.sandbox.tool_policy import is_tool_allowed, resolve_tool_policy
    ...
    def test_deny_overrides_allow(self):
        """deny 优先于 allow."""
        from smartclaw.sandbox.tool_policy import is_tool_allowed
        from smartclaw.sandbox.types import SandboxToolPolicy
    ...
    def test_agent_overrides_global(self):
        """Agent 覆盖 global 策略."""
        from smartclaw.sandbox.tool_policy import resolve_tool_policy
...
class TestDockerArgs:
    ...
    def test_build_create_args_basic(self):
        """基本容器创建参数."""
        from smartclaw.sandbox.docker import build_sandbox_create_args
        from smartclaw.sandbox.types import SandboxDockerConfig
    ...
    async def test_create_container_appends_extra_binds_after_workspace(self, monkeypatch):
        ...
        from smartclaw.sandbox import docker as docker_module
        from smartclaw.sandbox.types import SandboxDockerConfig
    ...
    def test_build_exec_args(self):
        """docker exec 参数构建."""
        from smartclaw.sandbox.docker import build_docker_exec_args
    ...
    def test_build_exec_args_with_custom_path(self):
        ...
        # 应该包含 SMARTCLAW_PREPEND_PATH
        args_str = " ".join(args)
        assert "SMARTCLAW_PREPEND_PATH" in args_str

    def test_build_sandbox_env(self):
        """沙箱环境变量构建."""
        from smartclaw.sandbox.docker import build_sandbox_env
...
class TestShared:
    ...
    def test_slugify_session_key(self):
        """Session key slug 化."""
        from smartclaw.sandbox.shared import slugify_session_key
    ...
    def test_scope_key_session(self):
        """session scope key."""
        from smartclaw.sandbox.shared import resolve_sandbox_scope_key
    ...
    def test_scope_key_agent(self):
        ...
        key = resolve_sandbox_scope_key("agent", "titan:session-123")
        assert key == "agent:titan"

    def test_workspace_dir(self):
        """工作区目录解析."""
        from smartclaw.sandbox.shared import resolve_sandbox_workspace_dir
...
class TestConfigHash:
    ...
    def test_same_config_same_hash(self):
        """相同配置产生相同哈希."""
        from smartclaw.sandbox.config_hash import compute_sandbox_config_hash
        from smartclaw.sandbox.types import SandboxDockerConfig
...
class TestEnvSecurity:
    ...
    def test_rejects_ld_preload(self):
        """拒绝 LD_PRELOAD."""
        from smartclaw.sandbox.env_security import validate_host_env
...
class TestTypes:
    ...
    def test_sandbox_context(self):
        """SandboxContext 创建."""
        from smartclaw.sandbox.types import (
            SandboxContext,
            SandboxDockerConfig,
            SandboxToolPolicy,
        )
        ...
        ctx = SandboxContext(
            ...
            workspace_access="rw",
            container_name="smartclaw-sbx-test",
            container_workdir="/workspace",
            docker=SandboxDockerConfig(),
            tools=SandboxToolPolicy(),
        )
        assert ctx.enabled is True
        assert ctx.container_name == "smartclaw-sbx-test"
...
class TestRegistry:
    ...
    def test_registry_entry_roundtrip(self):
        """注册表条目序列化/反序列化."""
        from smartclaw.sandbox.registry import RegistryEntry
    ...
    def test_registry_roundtrip(self):
        """注册表整体序列化/反序列化."""
        from smartclaw.sandbox.registry import Registry, RegistryEntry
...
class TestElevatedConfig:
    ...
    def test_elevated_default_disabled(self):
        """默认 elevated 关闭."""
        from smartclaw.sandbox.types import SandboxElevatedConfig
...
class TestFileToolSandboxPaths:
    ...
    async def test_read_workspace_plugin_path_uses_project_plugins_dir(self, tmp_path):
        from smartclaw.tool.file.read import _resolve_sandbox_file_path
        from smartclaw.tool.registry import ToolContext

        project_plugins = tmp_path / "project" / ".smartclaw" / "plugins"
        ...
        resolved, error = await _resolve_sandbox_file_path(
            ctx, "/workspace/.smartclaw/plugins/skills/demo/scripts/run.py"
        )
    ...
    async def test_write_sandbox_path_resolution(self):
        """write 工具的沙箱路径解析."""
        from smartclaw.tool.file.write import _resolve_sandbox_file_path
        from smartclaw.tool.registry import ToolContext
    ...
    async def test_edit_sandbox_blocks_escape(self):
        """edit 工具的沙箱路径逃逸检测."""
        from smartclaw.tool.file.edit import _resolve_sandbox_file_path
        from smartclaw.tool.registry import ToolContext
...
class TestBashSandboxHelpers:
    ...
    def test_get_sandbox_config_from_ctx_dict(self):
        ...
        from smartclaw.tool.code.bash import _get_sandbox_config_from_ctx
        from smartclaw.tool.registry import ToolContext
        ...
        ctx = ToolContext(
            ...
            extra={
                "sandbox": {
                    ...
                    "project_plugins_dir": "/tmp/project/.smartclaw/plugins",
                }
            },
        )
        cfg = _get_sandbox_config_from_ctx(ctx)
        assert cfg is not None
        assert cfg.container_name == "smartclaw-sbx-test"
        assert cfg.workspace_dir == "/tmp/ws"
        assert cfg.agent_workspace_dir == "/tmp/project/subdir"
        assert cfg.project_plugins_dir == "/tmp/project/.smartclaw/plugins"

    def test_rewrite_project_plugin_host_paths_for_sandbox(self, tmp_path):
        from smartclaw.sandbox.types import BashSandboxConfig
        from smartclaw.tool.code.bash import _rewrite_project_plugin_host_paths_for_sandbox
        ...
        assert rewritten == 'python "/workspace/.smartclaw/plugins/skills/demo/scripts/run.py"'

    @pytest.mark.asyncio
    async def test_resolve_sandbox_workdir_allows_project_plugins(self, tmp_path):
        from smartclaw.sandbox.types import BashSandboxConfig
        from smartclaw.tool.code.bash import _resolve_sandbox_workdir
        ...
        assert container_workdir == "/workspace/.smartclaw/plugins/skills/demo/scripts"
    ...
    def test_get_sandbox_config_from_ctx_model(self):
        ...
        model = BashSandboxConfig(
            container_name="smartclaw-sbx-m",
            workspace_dir="/ws",
            container_workdir="/workspace",
        )
        ctx = ToolContext(
        ...
        cfg = _get_sandbox_config_from_ctx(ctx)
        assert cfg is not None
        assert cfg.container_name == "smartclaw-sbx-m"

    def test_is_elevated_allowed_enabled(self):
        """elevated 已启用且工具在列表中."""
        from smartclaw.tool.code.bash import _is_elevated_allowed
        from smartclaw.tool.registry import ToolContext
...
class TestStreamProcessorSandboxMeta:
    ...
    async def test_sandbox_runtime_cache(self):
        ...
        with patch(
            "smartclaw.sandbox.runtime_status.resolve_sandbox_runtime_status",
            wraps=__import__(
                "smartclaw.sandbox.runtime_status",
...
class TestWorkspace:
    ...
    def test_resolve_project_plugins_dir_prefers_worktree(self, tmp_path, monkeypatch):
        ...
        plugins = project / ".smartclaw" / "plugins"
    ...
    async def test_ensure_sandbox_workspace_creates_dir(self):
        """确保工作区目录被创建."""
        from smartclaw.sandbox.workspace import ensure_sandbox_workspace
...
class TestRegistryIO:
    ...
    async def test_read_empty_registry(self, monkeypatch):
        """读取不存在的注册表返回空."""
        import smartclaw.sandbox.registry as reg_mod
        from smartclaw.sandbox.registry import read_registry
    ...
    async def test_write_and_read_registry(self, monkeypatch):
        """写入并读回注册表."""
        import smartclaw.sandbox.registry as reg_mod
        from smartclaw.sandbox.registry import (
            Registry,
            RegistryEntry,
            read_registry,
            write_registry,
