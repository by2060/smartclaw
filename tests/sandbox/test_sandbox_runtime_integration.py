...
from smartclaw.session.streaming.stream_events import ToolCallEvent, ToolInputStartEvent
from smartclaw.tool.registry import ToolContext, ToolResult
from smartclaw.tool.registry import ToolRegistry
from smartclaw.session.message import AssistantMessageInfo, MessagePath, TokenUsage
...
def _build_processor(config_data: dict) -> StreamProcessor:
    ...
    agent = SimpleNamespace(name="titan")
...
async def test_sandbox_meta_injects_bash_context(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from smartclaw.sandbox.types import SandboxContext, SandboxDockerConfig, SandboxToolPolicy

    async def fake_resolve_sandbox_context(**_kwargs):
        return SandboxContext(
            enabled=True,
            session_key="session-sandbox-test",
            workspace_dir="/tmp/.smartclaw/sandboxes/session-sandbox-test",
    ...
    monkeypatch.setattr(
        "smartclaw.sandbox.context.resolve_sandbox_context",
    ...
    assert meta["extra"]["sandbox"]["container_name"] == "smartclaw-sbx-test"
...
async def test_agent_tool_result_diff_when_sandbox_on_off(
    ...
    monkeypatch.setattr("smartclaw.tool.code.bash._execute_host", fake_host)
    monkeypatch.setattr("smartclaw.tool.code.bash._execute_sandboxed", fake_sandbox)

    from smartclaw.tool.code.bash import bash_tool
...
async def test_stream_processor_tool_call_diff_when_sandbox_on_off(
    ...
    monkeypatch.setattr("smartclaw.session.message.Message.store_part", fake_store_part)
    monkeypatch.setattr("smartclaw.session.message.Message.parts", fake_parts)
    ...
    assert captured_extras[1][1]["sandbox"]["container_name"] == "smartclaw-sbx-test"
