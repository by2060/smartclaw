...
5. tool/code/bash.py         - _is_smartclaw_plugin_write_path 移除 /workspace/ 硬编码检查
...
from smartclaw.sandbox.config import resolve_sandbox_config_for_agent, resolve_sandbox_docker_config
from smartclaw.sandbox.uploads import (
    UPLOADS_CHAT_PREFIX,
    UPLOADS_TASK_PREFIX,
...
class TestSandboxPathsContainerWorkdirPriority:
    ...
    def test_upload_path_with_real_container_workdir_matched(self, tmp_path):
        ...
        with patch("smartclaw.workspace.manager.WorkspaceManager.get_instance") as mock_cls:
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. bash.py - _is_smartclaw_plugin_write_path（/workspace/ 检查移除后的行为）
# ─────────────────────────────────────────────────────────────────────────────

class TestIsSmartClawPluginWritePathAfterWorkspaceCheckRemoval:
    """
    /workspace/.smartclaw/plugins/ 硬编码检查已移除。
    验证：相对路径和实际目录验证的兜底检查仍然有效。
    """

    def test_relative_prefix_still_works(self, tmp_path):
        """.smartclaw/plugins/ 相对路径前缀检查不受影响"""
    ...
    def test_workspace_container_path_no_longer_auto_allowed(self, tmp_path):
        """
        /workspace/.smartclaw/plugins/ 路径不再通过硬编码检查自动允许。
        当 base_dir 不含实际 .smartclaw/plugins 目录时，应返回 False。
        """
        assert _is_smartclaw_plugin_write_path(
            "/workspace/.smartclaw/plugins/agent.yaml", str(tmp_path)
        ) is False

    def test_absolute_path_in_base_dir_plugins_still_allowed(self, tmp_path):
        """base_dir/.smartclaw/plugins/ 下的绝对路径通过目录验证兜底"""
        plugins = tmp_path / ".smartclaw" / "plugins"
        plugins.mkdir(parents=True)
        tool = plugins / "my_tool.yaml"
        assert _is_smartclaw_plugin_write_path(str(tool), str(tmp_path)) is True

    def test_cwd_plugins_still_allowed(self):
        """当前工作目录下的 .smartclaw/plugins 路径仍被允许"""
        cwd_plugin = str(Path.cwd() / ".smartclaw" / "plugins" / "tool.yaml")
        # 只验证不抛异常，具体 True/False 取决于是否存在
        result = _is_smartclaw_plugin_write_path(cwd_plugin, str(Path.cwd()))
    ...
    def test_smartclaw_workspace_outputs_not_confused(self, tmp_path):
        """含 .smartclaw 字样但指向 workspace/outputs 的路径不应误判"""
        path = (
...
class TestDockerBindPathUnification:
    ...
    def test_plugin_bind_host_equals_container(self, tmp_path):
        ...
        plugins_dir = tmp_path / ".smartclaw" / "plugins"
