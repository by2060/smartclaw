...
async def test_write_tool_allows_session_outputs_in_ro_sandbox(monkeypatch, tmp_path) -> None:
    ...
    home_dir.mkdir()
    sandbox_dir.mkdir()
    monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: home_dir)
    try:
...
async def test_doc_parser_default_output_is_readable_in_sandbox(monkeypatch) -> None:
    ...
    with (
        ...
        monkeypatch.setattr("smartclaw.workspace.manager._user_home_dir", lambda: Path(home_dir))
        monkeypatch.setenv("SMARTCLAW_WORKSPACE_DIR", str(Path(home_dir) / ".smartclaw" / "workspace"))
        docx_path = Path(upload_dir) / "brief.docx"
        _write_minimal_docx(docx_path, "Uploaded brief text")
        ...
        try:
            ...
            assert output_path.is_relative_to(Path(home_dir) / ".smartclaw" / "workspace" / "outputs")
...
async def test_sandbox_context_mounts_main_session_uploads(monkeypatch, tmp_path) -> None:
    ...
    async def fake_ensure_sandbox_container(**_kwargs):
        ...
        return "smartclaw-sbx-test"

    async def fake_maybe_prune_sandboxes(_cfg):
        return None

    monkeypatch.setattr(
        "smartclaw.sandbox.context.ensure_sandbox_container",
        fake_ensure_sandbox_container,
    )
    monkeypatch.setattr(
        "smartclaw.sandbox.context.maybe_prune_sandboxes",
    ...
    try:
        ...
        assert env["SMARTCLAW_OUTPUTS_DIR"] == container_output_dir
        assert env["SMARTCLAW_ARTIFACTS_DIR"] == f"{container_output_dir}/artifacts"
    finally:
        WorkspaceManager._instance = previous_instance
