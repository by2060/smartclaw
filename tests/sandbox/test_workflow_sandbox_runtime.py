...
from smartclaw.workflow.repl_runtime import SandboxPythonExecRuntime
from smartclaw.workflow.runner import run_workflow
...
def test_sandbox_runtime_success_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("smartclaw.workflow.repl_runtime.uuid.uuid4", lambda: SimpleNamespace(hex="tok"))
    monkeypatch.setattr("smartclaw.workflow.repl_runtime.subprocess.Popen", FakePopen)
...
def test_run_workflow_uses_sandbox_runtime_when_context_has_sandbox(
    ...
    monkeypatch.setattr("smartclaw.workflow.runner.SandboxPythonExecRuntime", FakeSandboxRuntime)
    monkeypatch.setattr("smartclaw.workflow.runner.PythonExecRuntime", FakeHostRuntime)
    monkeypatch.setattr("smartclaw.workflow.runner.WorkflowEngine", FakeEngine)
    monkeypatch.setattr("smartclaw.workflow.runner.get_tool_registry", lambda tool_context=None: object())
    monkeypatch.setattr(
        "smartclaw.workflow.runner._load_config_data",
...
def test_run_workflow_uses_sandbox_runtime_when_sandbox_mode_on(
    ...
    async def fake_resolve_sandbox_context(**kwargs):
        _ = kwargs
        return SimpleNamespace(
            container_name="smartclaw-sbx-default",
            workspace_dir="/tmp/workspace",
            container_workdir="/workspace",
            workspace_access="rw",
            agent_workspace_dir="/tmp/workspace/.smartclaw/agents/build",
            env={"HELLO": "1"},
            upload_mounts=[],
        )
    ...
    monkeypatch.setattr("smartclaw.workflow.runner.WorkflowEngine", FakeEngine)
    monkeypatch.setattr("smartclaw.workflow.runner.get_tool_registry", lambda tool_context=None: object())
    monkeypatch.setattr("smartclaw.workflow.runner.resolve_sandbox_context", fake_resolve_sandbox_context)
    monkeypatch.setattr(
        "smartclaw.workflow.runner._load_config_data",
        lambda: {"sandbox": {"mode": "on"}},
    )

    ctx = ToolContext(session_id="s1", message_id="m1", agent="titan")
    workflow = {
        "id": "wf-1",
        "name": "wf",
        "start": "n1",
...
def test_run_workflow_installs_requirements_in_sandbox_runtime(
    ...
    monkeypatch.setattr("smartclaw.workflow.runner.PythonExecRuntime", FakeHostRuntime)
    monkeypatch.setattr("smartclaw.workflow.runner.WorkflowEngine", FakeEngine)
    monkeypatch.setattr("smartclaw.workflow.runner.RequirementsInstaller", FakeHostInstaller)
    monkeypatch.setattr("smartclaw.workflow.runner.SandboxRequirementsInstaller", FakeSandboxInstaller)
    monkeypatch.setattr("smartclaw.workflow.runner.get_tool_registry", lambda tool_context=None: object())
    monkeypatch.setattr(
        "smartclaw.workflow.runner._load_config_data",
        lambda: {"sandbox": {"mode": "on"}},
...
def test_stdin_bridge_rpc_tool_and_llm(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("smartclaw.workflow.repl_runtime.get_lazy_llm", lambda: FakeLLM())
    runtime = SandboxPythonExecRuntime(
...
def test_stdin_wrapper_uses_sys_dunder_stdout_for_rpc() -> None:
    ...
    assert "/workspace/.smartclaw/workflow/site-packages" in cmd
