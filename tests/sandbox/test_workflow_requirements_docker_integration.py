...
from smartclaw.sandbox.docker import exec_docker, remove_container
from smartclaw.tool.registry import ToolContext
from smartclaw.workflow.requirements import requirements_cache_key
from smartclaw.workflow.runner import run_workflow


def _docker_test_enabled() -> bool:
    return os.getenv("SMARTCLAW_RUN_DOCKER_TEST", "").strip() == "1"
...
async def test_real_docker_workflow_requirements_install_and_marker_cache() -> None:
    ...
    if not _docker_test_enabled():
        pytest.skip("Set SMARTCLAW_RUN_DOCKER_TEST=1 to run real Docker integration test")
    ...
    marker_path = f"/workspace/.smartclaw/workflow/requirements/{marker_key}.installed"
