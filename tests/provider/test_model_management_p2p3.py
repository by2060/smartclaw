...
- model_manager.py: Model listing, settings (smartclaw.json backed), default models
...
def reset_provider_runtime():
    """Reset provider registry and apply config from the user smartclaw.json."""
...
def temp_project(tmp_path, monkeypatch):
    """Create a temporary user config directory with provider models in smartclaw.json."""
    config_dir = tmp_path / "home" / ".smartclaw" / "config"
    config_dir.mkdir(parents=True)
    monkeypatch.setenv("SMARTCLAW_CONFIG_DIR", str(config_dir))
    from smartclaw.config.config import Config
    Config._global_config = None
    Config._cached_config = None
    config_file = config_dir / "smartclaw.json"
...
class TestModelCatalog:
    ...
    def test_anthropic_meta(self):
        from smartclaw.provider.model_catalog import get_provider_meta
...
# ==================== model_manager.py (now sync, smartclaw.json backed) ====================


class TestModelManager:
    """Test model management service — settings stored in smartclaw.json."""

    def test_list_all_models(self, temp_project):
        from smartclaw.provider.model_manager import ModelManager
    ...
    def test_settings_persisted_in_smartclaw_json(self, temp_project):
        """Verify that model settings are persisted in smartclaw.json."""
        from smartclaw.provider.model_manager import ModelManager
    ...
    def test_default_model_persisted_in_smartclaw_json(self, temp_project):
        """Verify that default models are persisted in smartclaw.json."""
        from smartclaw.provider.model_manager import ModelManager
        from smartclaw.config.config_writer import ConfigWriter
...
class TestCostCalculator:
    ...
    def test_basic_calculation(self):
        from smartclaw.provider.cost_calculator import CostCalculator
...
class TestUsageTracking:
    ...
    def test_record_usage_basic(self, setup_storage):
        from smartclaw.server.routes.usage import RecordUsageRequest, record_usage
    ...
    def test_backfill_skips_existing_live_record_with_same_message_id(self, setup_storage, monkeypatch):
        from smartclaw.provider import usage_service
        from smartclaw.provider.usage_service import (
            RecordUsageRequest,
        ...
        from smartclaw.session.session import SessionInfo, SessionTime
    ...
    def test_record_multiple_and_query(self, setup_storage):
        ...
        # Query stats
        from smartclaw.server.routes.usage import get_usage_stats
    ...
    def test_query_with_provider_filter(self, setup_storage):
        from smartclaw.server.routes.usage import RecordUsageRequest, record_usage, get_usage_stats
    ...
    def test_backfill_usage_records_is_idempotent(self, setup_storage, monkeypatch):
        from smartclaw.provider import usage_service
        from smartclaw.provider.usage_service import backfill_usage_records, get_usage_records
        from smartclaw.session.message import (
            AssistantMessageInfo,
            MessagePath,
            MessageWithParts,
