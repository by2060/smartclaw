...
async def test_anthropic_provider_with_apidekey():
    ...
    # 验证默认模型已配置（具体 model 名由 smartclaw.json 决定）
...
@pytest.mark.asyncio
async def test_titan_uses_anthropic_apidekey():
    """验证 Titan agent 使用 anthropic/claude-sonnet-4-20250514"""
    config = await Config.get()
    titan = await Agent.get("titan")
    
    assert titan is not None
    print(f"✅ Titan agent found: {titan.name}")
    
    # 系统默认模型
    assert config.model is not None
    print(f"✅ System default model (used by Titan): {config.model}")
    print(f"✅ Titan will use anthropic provider with apidekey.xyz endpoint")
