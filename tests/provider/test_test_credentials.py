...
from smartclaw.tool.registry import ToolResult, ToolInfo, ToolCategory, ToolParameter, ParameterType
...
_PATCH_SECRET_MGR = "smartclaw.security.get_secret_manager"
_PATCH_PROVIDER = "smartclaw.server.routes.provider.Provider"
_PATCH_TOOL_REGISTRY = "smartclaw.tool.registry.ToolRegistry"
_PATCH_CONFIG_GET = "smartclaw.config.config.Config.get"
_PATCH_CONFIG_RAW = "smartclaw.config.config_writer.ConfigWriter.get_provider_raw"
_PATCH_TOOL_SOURCE = "smartclaw.server.routes.tool._get_tool_source"
...
class TestTestCredentialsToolExecution:
    ...
    async def test_tool_failure_returns_failure(self):
        ...
        with (
            ...
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.threatbook": ["threatbook_ip_query"],
            }
    ...
    async def test_service_prefers_lightweight_query_tool_over_file_upload(self):
        ...
        with (
            ...
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.threatbook_cn": [
    ...
    async def test_onesec_service_prefers_threat_probe_and_uses_enum_action(self):
        ...
        with (
            ...
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.onesec": ["onesec_dns", "onesec_threat"],
            }
            mock_tr.execute = AsyncMock(return_value=ToolResult(
                success=True,
                output={"items": []},
    ...
    async def test_service_prefers_login_probe_over_action_dispatch_tool(self):
        ...
        with (
            ...
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.qingteng": ["qingteng_assets", "qingteng_login"],
            }
            mock_tr.execute = AsyncMock(return_value=ToolResult(
                success=True,
    ...
    async def test_declared_manifest_probe_is_used_before_heuristic_tool_selection(self):
        ...
        from smartclaw.server.routes.provider import test_provider_credentials
        from smartclaw.tool.probe_loader import ConnectivitySpec
        ...
        with (
            ...
            patch(
                "smartclaw.tool.probe_loader.get_connectivity_spec",
            ...
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.tdp_api": ["tdp_assets_domain_list"],
            }
            mock_tr.execute = AsyncMock(return_value=ToolResult(
                success=True,
                output={"status": "ok"},
    ...
    async def test_login_probe_does_not_overmatch_business_tools(self):
        ...
        with (
            ...
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.tdp_api": [
                    "tdp_login_api_list",
                    "tdp_ip_query",
                ],
            }
    ...
    async def test_single_attempt_only_to_avoid_account_lockout(self):
        ...
        with (
            ...
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.qingteng": ["qingteng_assets"],
            }
    ...
    async def test_action_dispatch_login_tool_uses_test_action(self):
        ...
        Why: handlers like ``smartclaw/.smartclaw/plugins/tools/api/onesig/onesig.handler.py``
        ...
        with (
            ...
            mock_tr.list_tools.return_value = [assets_tool, login_tool]
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.onesig": ["onesig_assets", "onesig_login"],
            }
            mock_tr.execute = AsyncMock(return_value=ToolResult(
                success=True,
    ...
    async def test_service_metadata_secret_is_used_for_hyphenated_service(self):
        ...
        with (
            ...
            patch(_PATCH_PROVIDER) as mock_provider_cls,
            patch(_PATCH_TOOL_REGISTRY) as mock_tr,
            patch("smartclaw.config.config_writer.ConfigWriter.get_api_service_raw", return_value={}),
            patch(
            ...
            mock_tr.list_tools.return_value = [tool_info]
            mock_tr._dynamic_tools_by_module = {
                "smartclaw.tool.generated.threatbook_cn": ["threatbook_ip_query"],
            }
            mock_tr.execute = AsyncMock(return_value=ToolResult(
                success=True,
                output={"ip": "8.8.8.8", "severity": "info"},
...
class TestTestCredentialsInlineConfigFallback:
    """Inline smartclaw.json apiKey values should work when no secret exists."""
    ...
    async def test_requested_azure_deployment_model_is_used_for_provider_test(self):
        from smartclaw.server.routes.provider import TestCredentialRequest, test_provider_credentials
