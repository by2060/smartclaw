...
2. ``chat()`` and ``chat_stream()`` must honour caller-provided
   ...
   :func:`smartclaw.provider.options.build_provider_options`) and ``max_tokens``
...
3. ``_convert_messages()`` must accept an explicit ``session_id`` so the
   ...
   ``messages[0].sessionID`` because :class:`smartclaw.provider.provider.ChatMessage`
...
from smartclaw.provider.provider import ChatMessage, StreamChunk
...
class TestSessionIdForwarding:
    ...
    def test_convert_messages_accepts_explicit_session_id(self):
        ...
        with patch("smartclaw.session.message.MessageSync") as mock_ms:
