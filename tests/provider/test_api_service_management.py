...
from smartclaw.tool.registry import Tool, ToolCategory, ToolInfo, ToolRegistry


class TestAPIServiceManagement:
    @pytest.mark.asyncio
    async def test_list_api_services_returns_enabled_state_and_bilingual_descriptions(self):
        from smartclaw.server.routes.provider import list_api_services
        ...
        with (
            patch("smartclaw.tool.registry.ToolRegistry") as mock_tool_registry,
            patch("smartclaw.config.config_writer.ConfigWriter.list_api_services_raw", return_value={
                "threatbook_api": {"enabled": False},
            }),
            patch("smartclaw.config.config_writer.ConfigWriter.get_api_service_raw", side_effect=lambda service_id: {
                "threatbook_api": {"enabled": False},
            }.get(service_id, {})),
            patch(
                "smartclaw.server.routes.provider._load_api_service_metadata_data",
                side_effect=lambda service_id: metadata_by_id[service_id],
            ),
            patch(
                "smartclaw.server.routes.provider._read_api_service_status_cache",
            ...
            patch(
                "smartclaw.server.routes.provider._get_api_service_tool_infos",
    ...
    async def test_update_api_service_persists_enabled_flag_and_updates_status_cache(self):
        from smartclaw.server.routes.provider import (
            APIServiceSummary,
        ...
        with (
            patch("smartclaw.config.config_writer.ConfigWriter.get_api_service_raw", return_value=existing_config.copy()),
            patch("smartclaw.config.config_writer.ConfigWriter.set_api_service") as mock_set_service,
            patch("smartclaw.server.routes.provider._set_api_service_tools_enabled", return_value=2) as mock_toggle_tools,
            patch(
                "smartclaw.server.routes.provider._read_api_service_status_cache",
                new=AsyncMock(return_value={}),
            ),
            patch(
                "smartclaw.server.routes.provider._write_api_service_status_cache",
                new=AsyncMock(),
            ) as mock_write_status,
            patch(
                "smartclaw.server.routes.provider._build_api_service_summary",
    ...
    def test_set_api_service_tools_enabled_invalidates_agent_cache_when_tools_matched(self):
        from smartclaw.server.routes.provider import _set_api_service_tools_enabled

        tools = [MagicMock(enabled=False), MagicMock(enabled=False)]

        with (
            patch("smartclaw.server.routes.provider._get_api_service_tool_infos", return_value=tools),
            patch("smartclaw.agent.registry.Agent.invalidate_cache") as mock_invalidate,
        ):
...
class TestToolRouteAPIServiceSync:
    def test_effective_api_tool_state_requires_service_credentials(self):
        from smartclaw.server.routes.tool import _get_effective_tool_enabled
        ...
        with patch("smartclaw.server.routes.provider._get_api_service_enabled", return_value=False):
            assert _get_effective_tool_enabled(tool_info) is False

    def test_effective_api_tool_state_keeps_tool_level_disable(self):
        from smartclaw.server.routes.tool import _get_effective_tool_enabled, _build_tool_response
        ...
        with patch(
            "smartclaw.config.config_writer.ConfigWriter.get_api_service_raw",
    ...
    async def test_update_tool_allows_api_tool_toggle(self):
        from smartclaw.server.routes.tool import ToolUpdateRequest, update_tool
        ...
        with (
            patch("smartclaw.server.routes.tool.ToolRegistry.init"),
            patch("smartclaw.server.routes.tool.ToolRegistry.get", return_value=tool),
            patch("smartclaw.tool.tool_loader.find_yaml_tool", return_value=None),
            patch("smartclaw.server.routes.tool._build_tool_response") as mock_build,
        ):
    ...
    async def test_create_tool_auto_enables_provider_backed_api_service(self):
        from smartclaw.server.routes.tool import (
            CreateToolRequest,
            ToolInfoResponse,
            ToolRegistry,
            create_tool,
        ...
        with (
            patch("smartclaw.server.routes.tool.ToolRegistry.init"),
            patch("smartclaw.tool.tool_loader.create_yaml_tool", return_value="dummy.yaml"),
            patch("smartclaw.tool.tool_loader.yaml_to_tool", return_value=tool),
            patch("smartclaw.server.routes.tool.ToolRegistry.register"),
            patch.object(ToolRegistry, "_plugin_tool_names", []),
            patch("smartclaw.server.routes.tool._build_tool_response", return_value=response),
            patch(
                "smartclaw.server.routes.provider.update_api_service",
                new=AsyncMock(),
    ...
    async def test_create_tool_does_not_enable_service_when_explicitly_disabled(self):
        from smartclaw.server.routes.tool import CreateToolRequest, ToolInfoResponse, ToolRegistry, create_tool
...
class TestProviderBackedApiServiceStatus:
    ...
    async def test_provider_test_updates_api_service_status_cache_when_configured(self):
        from smartclaw.server.routes.provider import test_provider_credentials
        ...
        with (
            patch("smartclaw.security.get_secret_manager", return_value=mock_secrets),
            patch("smartclaw.config.config.Config.get", new_callable=AsyncMock, return_value=mock_config),
            patch("smartclaw.config.config_writer.ConfigWriter.get_api_service_raw", return_value={"apiKey": "{secret:threatbook_api_key}"}),
            patch("smartclaw.server.routes.provider.Provider") as mock_provider_cls,
            patch("smartclaw.server.routes.provider._save_api_service_status", new=AsyncMock()) as mock_save_status,
        ):
    ...
    async def test_skip_status_cache_update_for_disabled_api_service(self):
        from smartclaw.server.routes.provider import _save_api_service_status_if_configured

        with (
            patch("smartclaw.config.config_writer.ConfigWriter.get_api_service_raw", return_value={"enabled": False}),
            patch("smartclaw.server.routes.provider._save_api_service_status", new=AsyncMock()) as mock_save_status,
        ):
...
class TestAPIServiceStatusCache:
    ...
    async def test_read_api_service_status_cache_ignores_expired_entries(self):
        from smartclaw.server.routes.provider import _read_api_service_status_cache

        with (
            patch("smartclaw.server.routes.provider.Storage.init", new=AsyncMock()),
            patch(
                "smartclaw.server.routes.provider.Storage.read",
                new=AsyncMock(return_value={
                    "checked_at": 1,
...
class TestProviderYamlMetadata:
    def test_load_api_service_metadata_uses_config_writer_raw_data(self):
        from smartclaw.server.routes.provider import _load_api_service_metadata_data
        ...
        with (
            ...
            patch(
                "smartclaw.server.routes.provider._load_provider_yaml_metadata",
                return_value={
    ...
    def test_load_provider_yaml_metadata_from_project_plugins(self, tmp_path, monkeypatch):
        from smartclaw.server.routes.provider import _load_provider_yaml_metadata

        provider_dir = tmp_path / ".smartclaw" / "plugins" / "tools" / "api" / "threatbook"
    ...
    async def test_get_api_service_metadata_returns_credential_schema(self):
        from smartclaw.server.routes.provider import get_api_service_metadata
...
class TestBootstrapUserApiServices:
    ...
    def _run_bootstrap(self, tools: dict, get_raw_side_effect):
        ...
        with (
            ...
            patch(
                "smartclaw.config.config_writer.ConfigWriter.set_api_service",
            ) as mock_set,
        ):
    ...
    def test_bootstrap_then_sync_keeps_disabled_tool_off(self):
        ...
        with (
            ...
            patch("smartclaw.config.config_writer.ConfigWriter.set_api_service") as mock_set,
            patch(
                "smartclaw.config.config_writer.ConfigWriter.list_api_services_raw",
