...
from smartclaw.cli import service_manager
from smartclaw.updater import updater
...
def test_resolve_npm_executable_prefers_bundled_node_home_on_windows(
    ...
    monkeypatch.setattr(updater.sys, "platform", "win32")
    monkeypatch.setenv("SMARTCLAW_NODE_HOME", str(node_home))
    monkeypatch.delenv("SMARTCLAW_INSTALL_ROOT", raising=False)
...
def test_resolve_npm_executable_falls_back_to_find_executable(
    ...
    monkeypatch.setattr(updater.sys, "platform", "win32")
    monkeypatch.delenv("SMARTCLAW_NODE_HOME", raising=False)
    monkeypatch.delenv("SMARTCLAW_INSTALL_ROOT", raising=False)
...
def test_upgrade_page_html_contains_marker_and_version() -> None:
    ...
    assert "smartclaw-upgrade-in-progress" in html
...
def test_gitee_archive_url_uses_web_archive_zip_endpoint() -> None:
    assert updater._gitee_archive_url("smartclaw/smartclaw", "2026.4.1", "tar.gz") == (
        "https://gitee.com/smartclaw/smartclaw/archive/refs/tags/v2026.4.1.zip"
    )
...
async def test_fetch_gitee_release_returns_web_archive_urls(monkeypatch: pytest.MonkeyPatch) -> None:
    class _FakeResponse:
        ...
        def json(self) -> dict[str, str]:
            return {
                "tag_name": "v2026.4.1",
                "body": "notes",
                "html_url": "https://gitee.com/smartclaw/smartclaw/releases/v2026.4.1",
            }
    ...
    class _FakeClient:
        ...
        async def get(self, url, params=None, follow_redirects=True):
            assert url == "https://gitee.com/api/v5/repos/smartclaw/smartclaw/releases/latest"
    ...
    tag, notes, html_url, zip_url, tar_url = await updater._fetch_gitee_release("smartclaw/smartclaw", "token")

    assert tag == "2026.4.1"
    assert notes == "notes"
    assert html_url == "https://gitee.com/smartclaw/smartclaw/releases/v2026.4.1"
    assert zip_url == "https://gitee.com/smartclaw/smartclaw/archive/refs/tags/v2026.4.1.zip"
...
async def test_download_archive_uses_curl_user_agent_for_gitee_web_archive(
    ...
    archive_path = await updater._download_archive(
        ...
        token="secret",
        dest_dir=tmp_path,
        filename="smartclaw-2026.4.1.tar.gz",
    )

    assert archive_path.name == "smartclaw-2026.4.1.zip"
    assert archive_path.read_bytes() == b"zip-bytes"
    assert captured["method"] == "GET"
    assert captured["url"] == "https://gitee.com/smartclaw/smartclaw/archive/refs/tags/v2026.4.1.zip"
...
async def test_download_archive_keeps_auth_header_for_non_gitee_sources(
    ...
    archive_path = await updater._download_archive(
        "https://github.com/AgentSmartClaw/SmartClaw/archive/refs/tags/v2026.4.1.tar.gz",
        token="secret",
        dest_dir=tmp_path,
        filename="smartclaw-2026.4.1.tar.gz",
    )

    assert archive_path.name == "smartclaw-2026.4.1.tar.gz"
...
def test_build_restart_argv_uses_windows_venv_python(
    ...
    monkeypatch.setattr(
        updater.sys,
        "argv",
        [r"C:\Users\worker\.local\bin\smartclaw", "start", "--reload", "--port", "8000"],
    )
...
def test_build_restart_argv_uses_venv_python_on_non_windows(
    ...
    monkeypatch.setattr(updater.sys, "argv", ["/usr/local/bin/smartclaw", "start", "--reload"])
...
def test_refresh_global_cli_entry_creates_symlink_on_unix(
    ...
    venv_smartclaw = install_root / ".venv" / "bin" / "smartclaw"
    venv_smartclaw.parent.mkdir(parents=True)
    venv_smartclaw.write_text("#!/usr/bin/env python\n", encoding="utf-8")

    updater._refresh_global_cli_entry(install_root)

    link = tmp_path / "home" / ".local" / "bin" / "smartclaw"
    assert link.is_symlink()
    assert link.resolve() == venv_smartclaw.resolve()
...
def test_refresh_global_cli_entry_creates_cmd_wrapper_on_windows(
    ...
    wrapper = tmp_path / "home" / ".local" / "bin" / "smartclaw.cmd"
    assert wrapper.exists()
    content = wrapper.read_text(encoding="ascii")
    assert str(venv_python) in content
    assert "-m smartclaw.cli.main %*" in content
...
def test_refresh_global_cli_entry_noop_when_venv_missing(
    ...
    assert not (link_dir / "smartclaw").exists()
...
def test_refresh_global_cli_entry_defers_legacy_uv_tool_uninstall_for_running_tool_env(
    ...
    monkeypatch.setattr(updater.sys, "platform", "darwin")
    monkeypatch.setattr(updater.sys, "executable", "/Users/test/.local/share/uv/tools/smartclaw/bin/python")
    monkeypatch.setattr(updater.Path, "home", lambda: tmp_path / "home")
    ...
    def fake_run(cmd: list[str], **kwargs) -> subprocess.CompletedProcess[str]:
        calls.append(cmd)
        return subprocess.CompletedProcess(args=cmd, returncode=0, stdout="smartclaw 0.0.0\n", stderr="")
...
def test_refresh_global_cli_entry_uninstalls_legacy_uv_tool_after_switching_runtime(
    ...
    assert calls == [
        ...
        ["/usr/local/bin/uv", "tool", "uninstall", "smartclaw"],
    ]
...
def test_safe_remove_renames_locked_file_on_windows(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    leftovers = list(tmp_path.glob("locked.exe.smartclaw_old_*"))
...
def test_safe_remove_renames_locked_directory_on_windows(
    ...
    leftovers = list(tmp_path.glob("webui.smartclaw_old_*"))
    assert not target.exists()
    assert len(leftovers) == 1
...
def test_prepare_upgrade_handover_writes_state_and_stops_frontend(
    ...
    monkeypatch.setenv("SMARTCLAW_ROOT", str(tmp_path / ".smartclaw"))
    paths = service_manager.RuntimePaths(
        root=tmp_path / ".smartclaw",
        run_dir=tmp_path / ".smartclaw" / "run",
        log_dir=tmp_path / ".smartclaw" / "logs",
        backend_pid=tmp_path / ".smartclaw" / "run" / "backend.pid",
        frontend_pid=tmp_path / ".smartclaw" / "run" / "webui.pid",
        backend_log=tmp_path / ".smartclaw" / "logs" / "backend.log",
        frontend_log=tmp_path / ".smartclaw" / "logs" / "webui.log",
    )
...
def test_start_upgrade_page_server_binds_configured_frontend_host(
    ...
    smartclaw_root = tmp_path / ".smartclaw"
    (smartclaw_root / "run").mkdir(parents=True)
    monkeypatch.setenv("SMARTCLAW_ROOT", str(smartclaw_root))
...
def test_backup_current_version_excludes_all_dist_directories(
    ...
    (install_root / "smartclaw.json").write_text('{"keep": true}', encoding="utf-8")
    ...
    assert "smartclaw/webui/dist/index.html" not in names
    assert "smartclaw/smartclaw.json" in names
    assert "smartclaw/.git/HEAD" not in names
    assert "smartclaw/dist/ignored.txt" not in names
...
def test_cleanup_old_backups_keeps_latest_only(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    backup_dir = tmp_path / "backups"
    backup_dir.mkdir()
    newest = backup_dir / "smartclaw-new.tar.gz"
    middle = backup_dir / "smartclaw-mid.tar.gz"
    oldest = backup_dir / "smartclaw-old.tar.gz"
...
def test_replace_install_dir_copies_dot_smartclaw_plugins_from_source(
    tmp_path: Path,
) -> None:
    """New release plugins under .smartclaw/plugins must be applied; removed plugins dropped."""
    source_dir = tmp_path / "source"
    install_root = tmp_path / "install"

    src_plugins = source_dir / ".smartclaw" / "plugins" / "tools" / "api"
    ...
    inst_plugins = install_root / ".smartclaw" / "plugins" / "tools" / "api"
    ...
    (source_dir / "smartclaw.json").write_text('{"version": "new"}', encoding="utf-8")
    (install_root / "smartclaw.json").write_text('{"keep": true}', encoding="utf-8")
    ...
    assert (install_root / "smartclaw.json").read_text(encoding="utf-8") == '{"version": "new"}'
...
async def test_perform_update_builds_current_frontend_after_handover(
    ...
    archive_path = tmp_path / "smartclaw.zip"
    ...
    monkeypatch.setattr(updater, "_build_restart_argv", lambda install_root=None: ["/usr/bin/python3", "-m", "smartclaw.cli.main", "start"])
...
async def test_perform_update_uses_cn_mirror_profile_for_sources_and_dependency_commands(
    ...
    archive_path = tmp_path / "smartclaw.tar.gz"
    archive_path.write_text("archive", encoding="utf-8")
    staged_root = tmp_path / "staged"
    staged_webui = staged_root / "webui"
    staged_webui.mkdir(parents=True)
...
async def test_perform_update_prefers_bundled_npm_for_windows_frontend_rebuild(
    ...
    monkeypatch.setattr(updater, "_extract_archive", lambda *_args, **_kwargs: staged_root)
    monkeypatch.setattr(updater, "_run_async", fake_run_async)
    monkeypatch.setattr(updater, "_find_executable", lambda name: r"C:\Users\smartclaw\AppData\Local\Programs\SmartClaw\tools\uv\uv.exe" if name == "uv" else None)
    monkeypatch.setattr(updater, "_build_uv_sync_env", lambda: None)
    ...
    monkeypatch.setattr(updater, "_build_restart_argv", lambda install_root=None: [r"C:\tool\python.exe", "-m", "smartclaw.cli.main", "start"])
...
async def test_perform_update_retries_windows_frontend_with_system_npm_after_bundled_build_failure(
    ...
    def fake_find(name: str) -> str | None:
        if name == "npm.cmd":
            return system_npm
        if name == "uv":
            return r"C:\Users\smartclaw\AppData\Local\Programs\SmartClaw\tools\uv\uv.exe"
        return None
...
async def test_perform_update_repairs_broken_uv_managed_python_cache_before_retrying_sync(
    ...
    monkeypatch.setattr(updater, "_extract_archive", lambda *_a, **_kw: staged_root)
    monkeypatch.setattr(updater, "_run_async", fake_run_async)
    monkeypatch.setattr(updater, "_find_executable", lambda _name: r"C:\Users\worker\AppData\Local\Programs\SmartClaw\tools\uv\uv.exe")
    monkeypatch.setattr(updater, "_build_uv_sync_env", lambda: None)
    monkeypatch.setattr(updater, "_replace_install_dir", lambda *_a, **_kw: None)
    monkeypatch.setattr(updater, "_write_version_marker", lambda _v: None)
    monkeypatch.setattr(updater, "_refresh_global_cli_entry", lambda _root: None)
...
async def test_perform_update_rolls_back_when_replace_fails_on_windows_locked_file(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            PermissionError(
                "[WinError 5] Access is denied: 'C:\\Users\\worker\\Desktop\\smartclaw-main\\webui\\node_modules\\@esbuild\\win32-x64\\esbuild.exe'"
            )
        ),
    )
    monkeypatch.setattr(updater, "_restore_backup_if_possible", lambda *_args: events.append("restore"))
...
async def test_perform_update_spawns_restart_process_on_windows(
    ...
    assert popen_calls == [
        ([r"C:\tool\python.exe", "-m", "smartclaw.cli.main", "start"], tmp_path / "install-root", True),
    ]
...
async def test_perform_update_yields_error_when_windows_spawn_fails(
    ...
    monkeypatch.setattr(updater, "_write_version_marker", lambda _v: None)
    monkeypatch.setattr(updater, "_refresh_global_cli_entry", lambda _root: None)
    monkeypatch.setattr(updater, "_build_restart_argv", lambda install_root=None: [r"C:\tool\python.exe", "-m", "smartclaw.cli.main"])
