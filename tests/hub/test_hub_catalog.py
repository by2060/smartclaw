...
from smartclaw.hub.catalog import list_catalog, load_manifest, load_taxonomy
from smartclaw.hub.files import file_tree, read_file_content
from smartclaw.hub.installer import install_plugin, uninstall_plugin
...
def isolated_hub_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("SMARTCLAW_DATA_DIR", str(data_dir))
    (config_dir / "smartclaw.json").write_text(json.dumps({}), encoding="utf-8")
...
async def test_hub_installs_and_uninstalls_skill(isolated_hub_env):
    ...
    skill_dir = isolated_hub_env["home"] / ".smartclaw" / "plugins" / "skills" / "ndr-alert-analysis"
...
async def test_hub_installs_nested_anthropic_skill(isolated_hub_env):
    ...
    skill_dir = isolated_hub_env["home"] / ".smartclaw" / "plugins" / "skills" / "triaging-security-incident"
    assert (skill_dir / "SKILL.md").is_file()
...
async def test_hub_installs_pentest_subagent(isolated_hub_env):
    ...
    agent_dir = isolated_hub_env["home"] / ".smartclaw" / "plugins" / "agents" / "web-hunter"
...
def test_hub_routes_cover_catalog_files_install_and_uninstall(isolated_hub_env):
    from smartclaw.server.routes.hub import router
