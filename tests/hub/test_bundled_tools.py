"""Tests for SmartClawHub-bundled tool plugin discovery and install.

Covers the multi-version-tool fix wired through:

* :func:`smartclaw.hub.catalog._bundled_tool_roots` and
  :func:`smartclaw.hub.catalog._entry_from_bundled_tool` — bundled tools
  surface in ``list_catalog`` as ``available`` until installed.
* :func:`smartclaw.hub.installer._resolve_install_destination` — tools
  shipped under ``smartclawhub/plugins/tools/api/<id>/`` install to
  ``<plugins>/tools/api/<id>/`` so :mod:`smartclaw.config.api_versioning`
  can still find ``_provider.yaml``.
* :func:`smartclaw.hub.security.validate_package` — generated bytecode
...
from smartclaw.hub import catalog, local
from smartclaw.hub.installer import _resolve_install_destination, install_plugin, uninstall_plugin
...
def isolated_hub(tmp_path, monkeypatch):
    """Isolated HOME + ``SMARTCLAW_HUB_ROOT`` pointing at a temp smartclawhub.

    Each test gets:
    * a private ``$HOME`` so user-level installs land under
      ``$HOME/.smartclaw/plugins/tools/...``
    * an empty project ``cwd`` so project-level installs don't pollute
      the real repo
    * a fresh ``SMARTCLAW_HUB_ROOT`` we can populate with bundled fixtures
    """
    home = tmp_path / "home"
    project = tmp_path / "project"
    bundled = tmp_path / "smartclawhub"
    ...
    monkeypatch.setenv("SMARTCLAW_HUB_ROOT", str(bundled))
    monkeypatch.setenv("SMARTCLAW_CONFIG_DIR", str(config))
    monkeypatch.setenv("SMARTCLAW_DATA_DIR", str(data))
    (config / "smartclaw.json").write_text("{}", encoding="utf-8")

    from smartclaw.config import api_versioning as versioning
    from smartclaw.config.config import Config
    from smartclaw.hub import catalog as hub_catalog
    from smartclaw.skill.skill import Skill

    def _clear_hub_caches() -> None:
        # ``load_index``/``load_taxonomy``/``_manifest_path_lookup`` are
        # ``@lru_cache``-d on the *real* ``SMARTCLAW_HUB_ROOT``. If we leave
        # them populated against the test's tmp smartclawhub, sibling tests
...
def _write_bundled_tool(
    ...
    *,
    ...
    """Drop a minimal valid tool plugin under ``smartclawhub/plugins/tools/<group>/<id>/``."""
...
class TestBundledToolRoots:
    ...
    def test_returns_empty_when_no_bundled_dir(self, isolated_hub, monkeypatch):
        # Point SMARTCLAW_HUB_ROOT at an empty directory.
        empty_hub = isolated_hub["bundled"].parent / "empty_hub"
        empty_hub.mkdir()
        monkeypatch.setenv("SMARTCLAW_HUB_ROOT", str(empty_hub))
...
class TestResolveInstallDestination:
    def test_tool_from_bundled_api_preserves_subdir(self, isolated_hub):
        ...
        assert dst.parent.name == "api", (
            "tool installs from smartclawhub api/ must keep the api/ subdir "
    ...
    def test_tool_from_project_level_api_dir_preserves_subdir(self, isolated_hub):
        ...
        Regression for the case where the SmartClawHub catalog surfaces a
        project-level plugin (``<cwd>/.smartclaw/plugins/tools/api/<id>/``)
        as installable: dropping the group prefix here landed the copy at
        ``~/.smartclaw/plugins/tools/<id>/`` and silently broke
...
class TestBundledInstallFlow:
    ...
    async def test_uninstall_cleans_orphan_api_service_entry(self, isolated_hub):
        """SmartClawhub uninstall must drop the bootstrapped api_services
...
class TestDeleteApiServiceCascade:
    ...
    async def test_finds_user_installed_plugin_by_storage_key(self, isolated_hub):
        from smartclaw.server.routes.provider import _find_user_installed_tool_plugin_for
    ...
    async def test_returns_none_for_project_level_plugin(self, isolated_hub, monkeypatch):
        """Project-level plugins (under ``<cwd>/.smartclaw/plugins/``) are
        ...
        project_plugin_dir = (
            isolated_hub["project"] / ".smartclaw" / "plugins" / "tools" / "api" / "project_tool"
        )
    ...
    async def test_delete_api_service_cascades_to_uninstall(self, isolated_hub):
        ...
        uninstall the backing user-installed plugin so the SmartClawHub
        catalog no longer reports it as ``installed``.
        """
        from smartclaw.config.config_writer import ConfigWriter
        from smartclaw.server.routes.provider import delete_api_service
...
class TestDisambiguateSummaryNames:
    ...
    def _summary(
        ...
        *,
        ...
        from smartclaw.server.routes.provider import APIServiceSummary
    ...
    def test_same_name_siblings_get_version_suffix(self):
        from smartclaw.server.routes.provider import _disambiguate_summary_names
