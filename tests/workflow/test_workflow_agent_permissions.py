...
from smartclaw.agent.controls import agent_allows_workflow_execution, filter_workflow_entries_for_agent
from smartclaw.tool import ToolContext
from smartclaw.tool.task.run_workflow import run_workflow_tool
from smartclaw.tool.task.run_workflow_node import run_workflow_node_tool
...
async def test_agent_workflow_execution_allows_declared_workflow(monkeypatch):
    ...
    assert await agent_allows_workflow_execution(
        "worker",
        workflow_path=".smartclaw/plugins/workflows/alert_triage/workflow.json",
    ) is True
    assert await agent_allows_workflow_execution(
        "worker",
        workflow_path=".smartclaw/plugins/workflows/alert_triage",
    ) is False
    assert await agent_allows_workflow_execution(
        "worker",
    ...
    assert await agent_allows_workflow_execution(
        "worker",
        workflow_id=".smartclaw/plugins/workflows/alert_triage/workflow.json",
    ) is True
...
@pytest.mark.asyncio
async def test_titan_workflow_session_bypasses_workflow_execution_grants(monkeypatch):
...
@pytest.mark.asyncio
async def test_titan_workflow_session_bypasses_workflow_catalog_filter(monkeypatch):
    titan = AgentInfo(name="titan", mode="primary", workflows=[])
    session = type("SessionObj", (), {"category": "workflow"})()

    async def fake_get(name: str):
        return titan if name == "titan" else None
    ...
    visible = await filter_workflow_entries_for_agent(entries, "titan", session_id="workflow-session")
...
async def test_workflow_route_catalog_default_management_view_keeps_all(monkeypatch):
    from smartclaw.server.routes.workflow import _filter_workflow_items_for_request
    from smartclaw.agent.registry import Agent
...
@pytest.mark.asyncio
async def test_workflow_route_agent_scope_empty_agent_defaults_to_titan(monkeypatch):
    from smartclaw.agent.registry import Agent
    from smartclaw.server.routes.workflow import _filter_workflow_items_for_request
...
async def test_run_workflow_tool_blocks_agent_without_workflow_grant():
    ...
    with patch(
        "smartclaw.tool.task.run_workflow._get_workflow_runtime",
