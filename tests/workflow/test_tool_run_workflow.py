...
- Error handling when smartclaw_workflow is not available
...
from smartclaw.tool import (
    ToolRegistry,
    ToolContext,
    ToolResult,
)

import smartclaw.tool.task.run_workflow as run_workflow_module
...
# =============================================================================
# Test Error Handling (smartclaw_workflow not available)
# =============================================================================

class TestRunWorkflowToolWithoutDependency:
    """Test run_workflow tool when smartclaw_workflow is not available"""
    
    @pytest.mark.anyio
    async def test_run_workflow_without_smartclaw_workflow(self, tool_context, simple_workflow):
        """Test that tool returns error when smartclaw_workflow is not available"""
        with patch.object(run_workflow_module, "_get_workflow_runtime", return_value=(None, None, None)):
            ...
            assert "smartclaw-workflow package is not available" in result.error
...
# =============================================================================
# Test Workflow Execution (with mocked smartclaw_workflow)
# =============================================================================
...
class TestRunWorkflowToolExecution:
    ...
    async def test_run_workflow_with_inputs(self, tool_context_with_permission, workflow_with_inputs):
        ...
        fake = FakeRunWorkflowResult(**{
            ...
            "steps": 1,
            "last_node_id": "node-1",
            "outputs": {"greeting": "Hello, SmartClaw!"},
            "history": [
                {"node_id": "node-1", "status": "SUCCEEDED", "outputs": {"greeting": "Hello, SmartClaw!"}}
            ],
            "error": None
        })
        mock_run = Mock(name="run_workflow", return_value=fake)
...
# =============================================================================
# Integration Test (if smartclaw_workflow is available)
# =============================================================================
