"""
Tests for the YAML tool plugin system (smartclaw.tool.tool_loader).
"""
...
class TestSubstituteParams:
    ...
    def test_preserves_secret_placeholders(self):
        with patch("smartclaw.tool.tool_loader._resolve_secrets", side_effect=lambda s: s):
    ...
    def test_sm4_value_is_decrypted_before_user_context(self, monkeypatch):
        monkeypatch.setattr("smartclaw.tool.tool_loader._decrypt_sm4_value", lambda value: "Bearer {user:currentToken}")
...
class TestYamlToTool:
    ...
    def test_api_tool_type_sets_source(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setattr("smartclaw.tool.tool_loader._TOOLS_SUBDIR", tmp_path)
...
class TestCrudHelpers:
    def test_create_uses_project_plugin_root(self, tmp_path: Path, monkeypatch):
        ...
        assert path == project_dir / ".smartclaw" / "plugins" / "tools" / "api" / "project_tool.yaml"
        assert path.exists()
    ...
    def test_find_project_level_yaml_tool(self, tmp_path: Path, monkeypatch):
        monkeypatch.setattr("smartclaw.tool.tool_loader._TOOLS_SUBDIR", tmp_path / "user_tools")
        monkeypatch.chdir(tmp_path)

        project_yaml = _write_yaml(
            tmp_path / ".smartclaw" / "plugins" / "tools" / "api" / "acme" / "project_tool.yaml",
    ...
    def test_delete_project_level_yaml_tool(self, tmp_path: Path, monkeypatch):
        ...
        project_yaml = _write_yaml(
            tmp_path / ".smartclaw" / "plugins" / "tools" / "api" / "acme" / "project_del_tool.yaml",
...
class TestScanDirectoryRecursive:
    def test_recursive_finds_subdirs(self, tmp_path: Path):
        from smartclaw.plugin.loader import scan_directory
...
class TestGetApiServiceIds:
    ...
    def test_excludes_non_api_tools(self, tmp_path: Path, monkeypatch):
        from smartclaw.tool.registry import ToolRegistry, ToolInfo, ToolCategory, Tool
    ...
    def test_ignores_dynamic_modules_for_api_service_discovery(self):
        ...
        try:
            ToolRegistry._dynamic_tools_by_module = {
                "smartclaw.tool.security.made_up_service": ["made_up_lookup"],
                "smartclaw.tool.security.ssh_host_cmd": [],
                "smartclaw.tool.security.ssh_utils": [],
                "smartclaw.tool.security.ssh_run_script": [],
            }
            ids = ToolRegistry.get_api_service_ids()
...
class TestGetToolSource:
    ...
    def test_api_source(self):
        ...
        from smartclaw.tool.registry import ToolInfo, ToolCategory
...
class TestPluginPyRegistration:
    def test_load_plugin_tools_marks_decorator_registered_tools_as_plugin_py(self):
        ...
        try:
            ...
            with patch("smartclaw.plugin.PluginLoader.load_all", side_effect=_fake_plugin_load):
...
class TestHttpHandler:
    ...
    async def test_basic_auth_templates_are_resolved(self, monkeypatch):
        ...
        handler = _build_http_handler(cfg)
        monkeypatch.setattr("smartclaw.tool.tool_loader._decrypt_sm4_value", lambda value: "demo-password")
        monkeypatch.setattr("smartclaw.tool.tool_loader._resolve_secrets", lambda value: value.replace("{secret:basic_user}", "demo-user"))
