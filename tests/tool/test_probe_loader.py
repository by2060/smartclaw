"""Unit tests for ``smartclaw.tool.probe_loader``."""
...
from smartclaw.tool.probe_loader import (
    ConnectivitySpec,
    Fixture,
    TestManifest,
    clear_cache,
...
def _patch_plugin_dir(plugin_dir: Path):
    ...
    return patch(
        "smartclaw.tool.probe_loader._plugin_dir_for",
...
class TestLoadTestManifest:
    ...
    def test_no_plugin_dir_returns_none(self):
        with patch("smartclaw.tool.probe_loader._plugin_dir_for", return_value=None):
...
class TestFixtureParsing:
    ...
    def test_get_tool_fixtures_by_tool_name_fallback_scan(self, tmp_path):
        ...
        """
        plugin_dir = _make_plugin_dir(tmp_path)
        from smartclaw.tool import probe_loader as probe_loader_module
        from smartclaw.config import api_versioning
        ...
        with (
            patch(
                "smartclaw.tool.probe_loader._service_id_for_tool",
                return_value=None,
            ),
            patch(
                "smartclaw.config.api_versioning.discover_api_service_descriptors",
...
_PATCH_PROBE_SPEC = "smartclaw.tool.probe_loader.get_connectivity_spec"
...
class TestTestCredentialsManifestBranch:
    ...
    async def test_manifest_exception_falls_back_to_heuristic(self):
        ...
        """
        from smartclaw.server.routes.provider import test_provider_credentials
        from smartclaw.tool.registry import ToolInfo, ToolCategory, ToolParameter, ParameterType
