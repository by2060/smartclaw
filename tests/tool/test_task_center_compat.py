...
import smartclaw.tool.task.task_center  # noqa: F401
...
from smartclaw.task.models import SchedulerMode, TaskTrigger
...
class TestTaskCenterCompatibility:
    ...
    async def test_task_create_infers_scheduled_type_from_cron(self):
        result = await ToolRegistry.execute(
            ...
            ctx=_make_ctx(),
            title="终端输出测试",
            description='每4分钟在终端输出"我是 smartclaw-04"',
            cron="*/4 * * * *",
            user_prompt="在终端中输出：我是 smartclaw-04",
        )
