...
from smartclaw.tool.task.task import _resolve_child_model, task_tool
...
class TestTaskModelPinning:
    ...
    async def test_resolve_child_model_ignores_unpinned_parent_session_model(self):
        ...
        with patch("smartclaw.storage.storage.Storage.read", AsyncMock(return_value={})), \
             patch("smartclaw.agent.registry.Agent.get", AsyncMock(return_value=None)), \
    ...
    async def test_task_tool_explicit_model_override_pins_child_session(self):
        ...
        with patch("smartclaw.tool.task.task.is_delegatable", return_value=True), \
             patch("smartclaw.tool.task.task.Session.get_by_id", AsyncMock(return_value=parent_session)), \
             patch("smartclaw.tool.task.task.get_background_manager", return_value=manager):
    ...
    async def test_task_tool_sync_continue_returns_session_loop_error(self):
        ...
        with patch("smartclaw.tool.task.task.is_delegatable", return_value=True), \
             patch("smartclaw.tool.task.task.Session.get_by_id", AsyncMock(side_effect=[parent_session, child_session])), \
             patch("smartclaw.tool.task.task.Message.create", AsyncMock()), \
             patch("smartclaw.tool.task.task.SessionLoop.run", AsyncMock(return_value=SimpleNamespace(
                 action="error",
    ...
    async def test_task_tool_sync_continue_fails_when_last_message_has_no_text(self):
        ...
        with patch("smartclaw.tool.task.task.is_delegatable", return_value=True), \
             ...
             patch("smartclaw.tool.subagent_result.Message.get_text_content", AsyncMock(return_value="")):
            result = await task_tool(
                _make_ctx(),
                description="continue explore",
                prompt="Continue the task",
