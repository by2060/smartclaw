...
单元测试：run_slash_command 工具（smartclaw.tool.system.slash_command）
...
from smartclaw.tool.system.slash_command import build_tools_catalog_summary, run_slash_command_tool
...
class TestToolsCommand:
    ...
    async def test_returns_success(self, mock_tools):
        with patch("smartclaw.tool.system.slash_command.ToolRegistry.list_tools", return_value=mock_tools), \
             patch("smartclaw.tool.system.slash_command.ToolRegistry.init"):
    ...
    async def test_catalog_truncates_descriptions(self):
        ...
        with patch("smartclaw.tool.system.slash_command.ToolRegistry.list_tools", return_value=[long_tool]), \
             patch("smartclaw.tool.system.slash_command.ToolRegistry.init"):
    ...
    async def test_empty_registry_no_crash(self):
        with patch("smartclaw.tool.system.slash_command.ToolRegistry.list_tools", return_value=[]), \
             patch("smartclaw.tool.system.slash_command.ToolRegistry.init"):
            result = await run_slash_command_tool(_make_ctx(), "tools")
        assert result.success
...
class TestSkillsCommand:
    # Skill.all is a classmethod on smartclaw.skill.skill.Skill (lazy-imported inside the tool)
    _SKILL_ALL = "smartclaw.skill.skill.Skill.all"
...
class TestWorkflowsCommand:
    ...
    _SCAN_WF = "smartclaw.workflow.center.scan_skill_workflows"
    ...
    async def test_workflow_paths_in_output(self):
        entries = [_make_workflow_entry("wf", path="/tmp/.smartclaw/workflow/wf/workflow.json")]
        with patch(self._SCAN_WF, new_callable=AsyncMock, return_value=entries):
            result = await run_slash_command_tool(_make_ctx(), "workflows")
        assert "/tmp/.smartclaw/workflow/wf/workflow.json" in result.output
