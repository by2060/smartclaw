"""Tests for smartclaw.tool.system.skill.
...
from smartclaw.tool.system.skill import (
    MAX_SKILL_DESCRIPTION_PREVIEW_CHARS,
    _truncate_skill_description,
    build_description,
    skill_tool_impl,
)
from smartclaw.tool.truncation import MAX_BYTES as REGISTRY_MAX_BYTES
from smartclaw.tool.truncation import MAX_LINES as REGISTRY_MAX_LINES
...
class TestSkillLoadNoTruncation:
    ...
    async def test_registry_execute_does_not_truncate_skill_output(self, fake_skill_dir):
        ...
        with patch.object(Skill, "all", AsyncMock(return_value=[skill_info])), \
             ...
             patch(
                 ...
                 AsyncMock(return_value=AgentInfo(name="titan", mode="primary", tools=["skill"])),
             ):
    ...
    @pytest.mark.asyncio
    async def test_workflow_context_can_load_skill_outside_titan_yaml_allowlist(self, fake_skill_dir):
        skill_info = SkillInfo(
