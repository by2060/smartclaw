...
class TestLooksLikeShellFileWrite:
    ...
    def test_session_scoped_output_path_detected(self):
        # 含日期 + session ID 的深层绝对路径，路径中包含隐藏目录 (.flocks) 和混合大小写 session ID
        cmd = (
            "hostname -I > /opt/home/flocks/.flocks/workspace/outputs/"
...
class TestNormalizeContainerPluginPaths:
    ...
    def test_backslash_in_suffix_normalized(self):
        cmd = r"cat /workspace/.flocks/plugins\tools\api.yaml"
        result = _normalize_container_plugin_paths(cmd, "/workspace/.flocks/plugins")
        assert "\\" not in result
        assert "/workspace/.flocks/plugins/tools/api.yaml" in result

    def test_forward_slash_unchanged(self):
        cmd = "cat /workspace/.flocks/plugins/tools/api.yaml"
        result = _normalize_container_plugin_paths(cmd, "/workspace/.flocks/plugins")
        assert result == cmd

    def test_container_root_trailing_slash_handled(self):
        # container_root 末尾带 /，应被 rstrip 处理
        cmd = "cat /workspace/.flocks/plugins/test.yaml"
        result = _normalize_container_plugin_paths(cmd, "/workspace/.flocks/plugins/")
        assert "/workspace/.flocks/plugins/test.yaml" in result
...
class TestSandboxProjectPluginRoots:
    ...
    def test_host_root_from_project_plugins_dir(self, tmp_path):
        plugins = tmp_path / "project" / ".flocks" / "plugins"
    ...
    def test_host_root_from_agent_workspace_dir(self, tmp_path):
        ...
        result = _sandbox_project_plugins_host_root(sb)
        assert result == workspace / ".flocks" / "plugins"
    ...
    def test_container_root_default_workdir(self):
        ...
        assert _sandbox_project_plugins_container_root(sb) == "/workspace/.flocks/plugins"

    def test_container_root_custom_workdir(self):
        sb = _sandbox(container_workdir="/app")
        assert _sandbox_project_plugins_container_root(sb) == "/app/.flocks/plugins"
...
class TestIsFlocksPluginWritePathExtra:
    ...
    def test_relative_path_resolved_against_base(self, tmp_path):
        ...
        assert _is_flocks_plugin_write_path(str(rel), str(tmp_path)) is True
    ...
    def test_flocks_workspace_outputs_not_matched(self, tmp_path):
        # 路径含 .flocks 隐藏目录，但指向 workspace/outputs 而非 plugins，不应误判为插件路径
        path = (
            "/opt/home/flocks/.flocks/workspace/outputs/"
            "2026-07-08/ses_0bde9c089ffeGHv7F2g9xCEw0G/hostname.txt"
        )
        assert _is_flocks_plugin_write_path(path, "/opt/home/flocks") is False
        assert _is_flocks_plugin_write_path(path, str(tmp_path)) is False

    def test_user_home_plugin_not_matched_as_project(self, tmp_path):
        user_plugin = str(Path.home() / ".flocks" / "plugins" / "tool.yaml")
        assert _is_flocks_plugin_write_path(user_plugin, str(tmp_path)) is False
...
class TestIsUserFlocksPluginWritePathExtra:

    def test_home_plugins_path_matched(self):
        target = str(Path.home() / ".flocks" / "plugins" / "tools" / "api" / "demo.yaml")
        assert _is_user_flocks_plugin_write_path(target) is True

    def test_relative_path_returns_false(self):
        assert _is_user_flocks_plugin_write_path(".flocks/plugins/tool.yaml") is False

    def test_workspace_path_not_matched(self):
        assert _is_user_flocks_plugin_write_path("/workspace/.flocks/plugins/tool.yaml") is False
