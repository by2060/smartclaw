...
from smartclaw.tool.tool_loader import _read_yaml_raw, yaml_to_tool

_WORKSPACE_ROOT = Path(__file__).resolve().parents[2]
_TDP_HANDLER = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/tdp_v3_3_10/tdp.handler.py"
_SKYEYE_HANDLER = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/skyeye_v4_0_14_0_SP2/skyeye.handler.py"
...
def test_tdp_incident_yaml_loads_with_provider():
    yaml_path = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/tdp_v3_3_10/tdp_incident_list.yaml"
...
def test_tdp_query_yaml_promotes_semantic_top_level_fields():
    ...
    for filename, fields in expected_fields.items():
        yaml_path = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/tdp_v3_3_10" / filename
        raw = _read_yaml_raw(yaml_path)
...
def test_tdp_platform_yaml_uses_keyword_and_requires_confirmation():
    yaml_path = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/tdp_v3_3_10/tdp_platform_config.yaml"
    raw = _read_yaml_raw(yaml_path)
    tool = yaml_to_tool(raw, yaml_path)
...
def test_tdp_policy_yaml_requires_confirmation_and_uses_object_ioc_list():
    yaml_path = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/tdp_v3_3_10/tdp_policy_settings.yaml"
    raw = _read_yaml_raw(yaml_path)
    tool = yaml_to_tool(raw, yaml_path)
...
def test_tdp_log_yaml_uses_object_columns_and_supports_cascade_asset_group():
    yaml_path = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/tdp_v3_3_10/tdp_log_search.yaml"
    raw = _read_yaml_raw(yaml_path)
    tool = yaml_to_tool(raw, yaml_path)
...
def test_skyeye_alarm_list_yaml_loads_with_provider():
    yaml_path = _WORKSPACE_ROOT / ".smartclaw/plugins/tools/api/skyeye_v4_0_14_0_SP2/skyeye_alarm_list.yaml"
    raw = _read_yaml_raw(yaml_path)
    tool = yaml_to_tool(raw, yaml_path)
