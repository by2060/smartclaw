...
def _allow_tools(monkeypatch: pytest.MonkeyPatch, tool_names: set[str]) -> None:
    ...
    monkeypatch.setattr("flocks.tool.system.tool_search.agent_allowed_tools", allowed_tools)
...
async def test_tool_search_returns_user_plugin_tools(
    ...
    ctx = SimpleNamespace(session_id="session-plugin", agent="rex", event_publish_callback=AsyncMock())
...
async def test_tool_search_adds_matching_tools_to_callable_set(
    ...
    ctx = SimpleNamespace(session_id="session-nondeferred", agent="rex", event_publish_callback=AsyncMock())
    ...
    add_callable.assert_awaited_once_with("session-nondeferred", ["read"], agent_name="rex")
...
async def test_tool_search_does_not_return_disabled_tools(
    ...
    ctx = SimpleNamespace(session_id="session-disabled", agent="rex", event_publish_callback=AsyncMock())
    ...
    add_callable.assert_awaited_once_with("session-disabled", [], agent_name="rex")
...
async def test_tool_search_hides_tools_outside_agent_allowlist(
    ...
    ctx = SimpleNamespace(session_id="session-hidden", agent="rex", event_publish_callback=AsyncMock())
    ...
    assert result.output["matches"] == []
    assert result.output["callableToolNames"] == []
    add_callable.assert_awaited_once_with("session-hidden", [], agent_name="rex")
...
async def test_tool_search_select_cannot_bypass_agent_allowlist(
    ...
    ctx = SimpleNamespace(session_id="session-select-hidden", agent="rex", event_publish_callback=AsyncMock())
...
async def test_tool_search_includes_always_load_tools(
    ...
    ctx = SimpleNamespace(session_id="session-always-load", agent="rex", event_publish_callback=AsyncMock())
