"""Tests for ``smartclaw.config.api_versioning``.
...
from smartclaw.config import api_versioning as versioning
from smartclaw.config.api_versioning import (
    derive_storage_key,
...
def isolated_env(tmp_path, monkeypatch):
    ...
    ``Path.home() / .smartclaw/plugins/tools/api``) doesn't pick up the
    ...
    user_config = home / ".smartclaw" / "config"
    user_config.mkdir(parents=True)
    project_root.mkdir(parents=True)

    monkeypatch.chdir(project_root)
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("SMARTCLAW_CONFIG_DIR", str(user_config))
...
def api_root(isolated_env):
    ...
    root = project_root / ".smartclaw" / "plugins" / "tools" / "api"
    root.mkdir(parents=True)
    return root


def _write_smartclaw_json(user_config: Path, payload: dict) -> Path:
    path = user_config / "smartclaw.json"
...
class TestDiscovery:
    ...
    def test_multiple_versions_of_same_service_id_coexist(self, api_root):
        ...
        Regression test for the bug where smartclawhub couldn't tell
...
class TestLegacyResolution:
    ...
    def test_not_shadowed_when_versioned_key_absent(self, api_root):
        _write_provider_yaml(api_root / "tdp_v3_3_10", service_id="tdp_api", version="3.3.10")
        versioning._reset_descriptor_cache()
        # Versioned key not present in smartclaw.json -> legacy block stays visible.
...
class TestMigration:
    def test_copies_legacy_to_storage_key(self, isolated_env, api_root):
        ...
        _write_provider_yaml(api_root / "tdp_v3_3_10", service_id="tdp_api", version="3.3.10")
        config_path = _write_smartclaw_json(user_config, {
            "api_services": {
                "tdp_api": {
                    "enabled": True,
    ...
    def test_idempotent_when_storage_key_exists(self, isolated_env, api_root):
        _, user_config = isolated_env
        _write_provider_yaml(api_root / "tdp_v3_3_10", service_id="tdp_api", version="3.3.10")
        _write_smartclaw_json(user_config, {
            "api_services": {
        ...
        services = json.loads((user_config / "smartclaw.json").read_text())["api_services"]
    ...
    def test_no_legacy_no_op(self, isolated_env, api_root):
        _, user_config = isolated_env
        _write_provider_yaml(api_root / "tdp_v3_3_10", service_id="tdp_api", version="3.3.10")
        _write_smartclaw_json(user_config, {"api_services": {}})
    ...
    def test_creates_backup_only_when_writing(self, isolated_env, api_root):
        ...
        migrate_api_services()
        backups = list(user_config.glob("smartclaw.json.bak.*"))
        assert len(backups) == 1, f"expected one backup, got {backups}"

        # Second run is a no-op; no extra backup is created.
        migrate_api_services()
        backups_after = list(user_config.glob("smartclaw.json.bak.*"))
    ...
    def test_no_backup_when_disabled(self, isolated_env, api_root):
        ...
        assert list(user_config.glob("smartclaw.json.bak.*")) == []
    ...
    def test_preserves_other_sections(self, isolated_env, api_root):
        ...
        migrate_api_services()
        data = json.loads((user_config / "smartclaw.json").read_text())
    ...
    def test_missing_config_file_is_safe(self, isolated_env, api_root):
        _write_provider_yaml(api_root / "tdp_v3_3_10", service_id="tdp_api", version="3.3.10")
        # No smartclaw.json on disk → silently no-op.
        assert migrate_api_services() == {}
...
class TestConfigWriterFallback:
    ...
    def test_set_api_service_warns_when_writing_shadowed_legacy(
        ...
        # SmartClaw logs structured records to stderr; assert against that.
    ...
    def test_set_api_service_promotes_ssl_verify_alias_to_verify_ssl(self, isolated_env, api_root):
        ...
        services = json.loads((user_config / "smartclaw.json").read_text(encoding="utf-8"))["api_services"]
    ...
    def test_get_api_service_raw_handles_null_api_services(self, isolated_env, api_root):
        """``"api_services": null`` in smartclaw.json must not crash the reader."""
        _, user_config = isolated_env
        from smartclaw.config.config_writer import ConfigWriter

        _write_provider_yaml(api_root / "tdp_v3_3_10", service_id="tdp_api", version="3.3.10")
        _write_smartclaw_json(user_config, {"api_services": None})
        versioning._reset_descriptor_cache()
