...
def verify_config_init():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 步骤 1: 创建 .smartclaw 目录和示例文件
        print("📁 步骤 1: 创建 .smartclaw 目录和示例文件")
        smartclaw_dir = test_dir / ".smartclaw"
        smartclaw_dir.mkdir()
        
        # 创建示例文件
        config_example = smartclaw_dir / "smartclaw.json.example"
        config_example.write_text('{\n  "provider": {},\n  "mcp": {}\n}')
        print(f"   ✓ 创建 {config_example.name}")

        mcp_example = smartclaw_dir / "mcp_list.json.example"
        mcp_example.write_text('{\n  "version": "1.0.0",\n  "categories": {},\n  "servers": []\n}')
        print(f"   ✓ 创建 {mcp_example.name}")
        
        secret_example = smartclaw_dir / ".secret.json.example"
        secret_example.write_text('{}')
        print(f"   ✓ 创建 {secret_example.name}\n")
        
        # 步骤 2: 检查文件不存在
        print("📋 步骤 2: 检查配置文件不存在")
        config_file = smartclaw_dir / "smartclaw.json"
        mcp_file = smartclaw_dir / "mcp_list.json"
        secret_file = smartclaw_dir / ".secret.json"
        
        if not config_file.exists():
            print("   ✓ smartclaw.json 不存在")
        else:
            print("   ✗ smartclaw.json 已存在（测试失败）")
            return False
        ...
        if config_file.exists():
            ...
            print(f"   ✓ smartclaw.json 已创建")
            print(f"   内容: {content[:50]}...")
        else:
            print("   ✗ smartclaw.json 未创建（测试失败）")
            return False
