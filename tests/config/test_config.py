...
from smartclaw.config.config import Config, GlobalConfig, ConfigInfo


@pytest.fixture(autouse=True)
def isolated_user_config(tmp_path, monkeypatch):
    """Isolate config state under a temporary ~/.smartclaw/config directory."""
    home_dir = tmp_path / "home"
    home_dir.mkdir()
    monkeypatch.setenv("HOME", str(home_dir))
    monkeypatch.delenv("SMARTCLAW_ROOT", raising=False)
    monkeypatch.delenv("SMARTCLAW_CONFIG_DIR", raising=False)
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    Config._global_config = None
    Config._cached_config = None
    yield home_dir / ".smartclaw" / "config"
    Config._global_config = None
    Config._cached_config = None
...
async def test_xdg_config_dir(tmp_path, monkeypatch):
    """The config dir should stay under ~/.smartclaw/config."""
    ...
    # Get global config - XDG should not override the unified ~/.smartclaw/config path.
    global_cfg = Config.get_global()
    expected_dir = Path(tmp_path / "home" / ".smartclaw" / "config")
...
async def test_backward_compatibility_legacy_dir(tmp_path, monkeypatch):
    """Default config location should be ~/.smartclaw/config."""
...
async def test_project_config_is_discovered_from_nested_cwd(tmp_path, monkeypatch):
    """Config.get should ignore project .smartclaw and only use the user config."""
    project_root = tmp_path / "project"
    config_dir = project_root / ".smartclaw"
    ...
    user_config_dir = tmp_path / "home" / ".smartclaw" / "config"
    user_config_dir.mkdir(parents=True)
    (user_config_dir / "smartclaw.json").write_text(
...
async def test_load_text_handles_secret_with_backslash(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "smartclaw.security.resolve_secret_value",
    ...
    config_file = tmp_path / "smartclaw.json"
    config_file.write_text(
...
async def test_load_text_handles_env_with_backslash(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SMARTCLAW_TEST_BACKSLASH", r"C:\Users\me\token\nVAL")

    config_file = tmp_path / "smartclaw.json"
    config_file.write_text(
        json.dumps({"provider": {"x": {"options": {"apiKey": "{env:SMARTCLAW_TEST_BACKSLASH}"}}}}, indent=2),
        encoding="utf-8",
    )
