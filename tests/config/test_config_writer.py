"""
Tests for ConfigWriter — smartclaw.json provider CRUD.
...
def temp_project(tmp_path, monkeypatch):
    """Create an isolated user config directory with a preloaded smartclaw.json."""
...
class TestConfigWriter:
    ...
    def test_empty_config_file(self, tmp_path, monkeypatch):
        ...
        Config._global_config = None
        Config._cached_config = None
        (config_dir / "smartclaw.json").write_text("{}")
    ...
    def test_no_config_file(self, tmp_path, monkeypatch):
        """ConfigWriter creates smartclaw.json if it doesn't exist."""
        config_dir = tmp_path / "home" / ".smartclaw" / "config"
        config_dir.mkdir(parents=True)
        monkeypatch.setenv("SMARTCLAW_CONFIG_DIR", str(config_dir))
        Config._global_config = None
