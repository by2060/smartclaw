...
def test_ensure_config_files_creates_from_examples(tmp_path, monkeypatch):
    ...
    config_example = example_dir / "smartclaw.json.example"
    ...
    from smartclaw.config import config_writer
...
def test_ensure_config_files_creates_smartclaw_dir(tmp_path, monkeypatch):
    """Test that ensure_config_files creates ~/.smartclaw/config if needed."""
    config_dir = tmp_path / "home" / ".smartclaw" / "config"
    example_dir = tmp_path / "examples"
    example_dir.mkdir(parents=True)
    monkeypatch.setenv("SMARTCLAW_CONFIG_DIR", str(config_dir))
