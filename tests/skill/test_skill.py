...
from smartclaw.skill.skill import Skill, SkillInfo, SkillRequires, SkillInstallSpec
...
def test_eligibility_env_present():
    ...
    os.environ["_SMARTCLAW_TEST_VAR"] = "1"
    try:
        skill = _make_skill(SkillRequires(env=["_SMARTCLAW_TEST_VAR"]))
        result = Skill.check_eligibility(skill)
        assert result.eligible is True
    finally:
        del os.environ["_SMARTCLAW_TEST_VAR"]
...
def test_parse_skill_md_with_metadata(tmp_path):
    """SKILL.md with nested metadata.smartclaw → install_specs and requires populated."""
...
async def test_discover_user_source_for_global_plugins(tmp_path):
    """Skills under ~/.smartclaw/plugins/skills/ must get source='user'."""
    home = tmp_path
    global_plugins = home / ".smartclaw" / "plugins" / "skills" / "my-user-skill"
    _write_skill_md(global_plugins / "SKILL.md", "my-user-skill")

    with (
        patch("os.path.expanduser", return_value=str(home)),
        patch("smartclaw.skill.skill.Instance.get_directory", return_value=str(tmp_path)),
        patch("smartclaw.skill.skill.Instance.get_worktree", return_value=str(tmp_path)),
    ):
        Skill.clear_cache()
        skills = await Skill.all()
...
async def test_discover_project_source_for_project_plugins(tmp_path):
    """Skills under <project>/.smartclaw/plugins/skills/ must get source='project'."""
    project_dir = tmp_path / "myproject"
    project_dir.mkdir()
    project_plugins = project_dir / ".smartclaw" / "plugins" / "skills" / "my-project-skill"
    _write_skill_md(project_plugins / "SKILL.md", "my-project-skill")

    # Use a separate home to avoid ~/.smartclaw interference
    fake_home = tmp_path / "home"
    fake_home.mkdir()

    with (
        patch("os.path.expanduser", return_value=str(fake_home)),
        patch("smartclaw.skill.skill.Instance.get_directory", return_value=str(project_dir)),
        patch("smartclaw.skill.skill.Instance.get_worktree", return_value=str(project_dir)),
    ):
        Skill.clear_cache()
        skills = await Skill.all()
...
@pytest.mark.asyncio
async def test_discover_smartclaw_source_for_builtin_skills(tmp_path):
    """Skills under .smartclaw/skills/ (not plugins/) must get source='smartclaw'."""
    project_dir = tmp_path / "myproject"
    project_dir.mkdir()
    builtin_skill = project_dir / ".smartclaw" / "skills" / "my-builtin-skill"
    ...
    assert skill.source == "smartclaw", f"expected 'smartclaw', got {skill.source!r}"
...
async def test_discover_project_overrides_global(tmp_path):
    ...
    global_skill = fake_home / ".smartclaw" / "plugins" / "skills" / "shared-skill"
    _write_skill_md(global_skill / "SKILL.md", "shared-skill", "Global version")

    project_skill = project_dir / ".smartclaw" / "plugins" / "skills" / "shared-skill"
...
def test_watcher_start_stop_no_crash(tmp_path):
    ...
    from smartclaw.skill.skill import SkillFileWatcher

    # Create a fake skill directory so the watcher has something to watch
    skill_dir = tmp_path / ".smartclaw"
    skill_dir.mkdir()
...
def test_parse_skill_md_invalid_metadata_still_parses(tmp_path):
    """Malformed metadata.smartclaw block logs a warning but still returns SkillInfo."""
...
async def test_create_skill_writes_to_plugins_path(tmp_path, monkeypatch):
    """POST /skills should write to project .smartclaw/plugins/skills/, not ~/.smartclaw/skills/."""
    ...
    monkeypatch.setattr(
        "smartclaw.project.instance.Instance.get_directory",
    ...
    assert data["location"] == str(
        project_dir / ".smartclaw" / "plugins" / "skills" / "write-path-test" / "SKILL.md"
    )
    assert data["source"] == "project"
    assert not (tmp_path / ".smartclaw" / "plugins" / "skills" / "write-path-test" / "SKILL.md").exists()
...
async def test_delete_project_skill_allows_api_managed_marker(tmp_path, monkeypatch):
    ...
    project_dir = tmp_path / "project"
    project_dir.mkdir()
    skills_root = project_dir / ".smartclaw" / "plugins" / "skills"
    ...
    monkeypatch.setattr(
        "smartclaw.project.instance.Instance.get_worktree",
        classmethod(lambda cls: str(project_dir)),
    )
    Skill.clear_cache()
