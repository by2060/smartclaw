"""
Tests for smartclaw.skill.installer and eligibility checking.
"""
...
from smartclaw.skill.skill import Skill, SkillInfo, SkillInstallSpec, SkillRequires, SkillMetadata
from smartclaw.skill.installer import (
    SkillInstaller,
    SkillInstallResult,
    DepInstallResult,
    _resolve_source,
...
def tmp_skills_dir(tmp_path: Path):
    ...
    d = tmp_path / ".smartclaw" / "plugins" / "skills"
...
class TestCheckEligibility:
    ...
    def test_env_present(self):
        os.environ["TEST_SMARTCLAW_VAR"] = "hello"
        try:
            skill = make_skill_info("env-skill", requires_env=["TEST_SMARTCLAW_VAR"])
            result = Skill.check_eligibility(skill)
            assert result.eligible is True
        finally:
            del os.environ["TEST_SMARTCLAW_VAR"]
...
class TestParseFrontmatter:
    ...
    def test_nested_metadata_smartclaw(self):
        content = (
            "---\n"
            "name: test-skill\n"
            "description: Hello\n"
        ...
        data = Skill._parse_frontmatter(content)
        assert data["name"] == "test-skill"
        meta = data.get("metadata", {}).get("smartclaw", {})
...
class TestSaveSkillContent:
    def test_save_valid_content(self, tmp_skills_dir: Path):
        ...
        with patch("smartclaw.skill.installer._user_skills_root", return_value=tmp_skills_dir):
...
class TestInstallFromSource:
    ...
    async def test_workspace_zip_installs_nested_skill_and_cleans_zip(self, tmp_path: Path, monkeypatch):
        ...
        project_skills = tmp_path / "project" / ".smartclaw" / "plugins" / "skills"
        ...
        monkeypatch.setattr(
            "smartclaw.workspace.manager.WorkspaceManager.get_instance",
            classmethod(lambda cls: FakeWorkspace()),
        )
        monkeypatch.setattr("smartclaw.skill.installer._project_skills_root", lambda: project_skills)
    ...
    async def test_url_success(self, tmp_skills_dir: Path):
        ...
        with (
            patch("smartclaw.skill.installer._user_skills_root", return_value=tmp_skills_dir),
