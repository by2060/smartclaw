...
SKILL_FILE = PROJECT_ROOT / ".smartclaw" / "plugins" / "skills" / "onboarding" / "SKILL.md"
...
def test_parse_onboarding_project_skill_file() -> None:
    ...
    assert "complete SmartClaw setup process" in parsed.description
...
def test_onboarding_skill_uses_shared_preflight_helper_and_mcp_status_rules() -> None:
    ...
    assert "from smartclaw.skill.onboarding_status import print_onboarding_preflight_status" in content
