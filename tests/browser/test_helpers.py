...
from smartclaw.browser import helpers
...
def _run(fake_png, width: int, height: int, **kwargs):
    ...
    with patch("smartclaw.browser.helpers.cdp", side_effect=fake), tempfile.TemporaryDirectory() as temp_dir:
...
def test_page_info_raises_clear_error_on_js_exception() -> None:
    ...
    with (
        patch("smartclaw.browser.helpers._send", side_effect=fake_send),
        patch("smartclaw.browser.helpers.cdp", side_effect=fake_cdp),
    ):
...
def test_attach_tab_does_not_activate_target() -> None:
    ...
    with (
        patch("smartclaw.browser.helpers.cdp", side_effect=fake_cdp),
        patch("smartclaw.browser.helpers._send", side_effect=lambda req: sent.append(req) or {"session_id": "session-1"}),
    ):
...
def test_switch_tab_activates_then_attaches_target() -> None:
    ...
    with (
        patch("smartclaw.browser.helpers.cdp", side_effect=fake_cdp),
        patch("smartclaw.browser.helpers._send", return_value={"session_id": "session-1"}),
    ):
...
def test_close_tab_can_skip_activating_next_tab() -> None:
    ...
    with patch("smartclaw.browser.helpers.cdp", side_effect=fake_cdp):
...
def test_save_state_writes_portable_schema(tmp_path) -> None:
    ...
    with (
        patch(
            "smartclaw.browser.helpers.page_info", return_value={"url": "https://www.zhihu.com/app", "title": "Example"}
        ),
        patch("smartclaw.browser.helpers.cdp", side_effect=fake_cdp),
        patch("smartclaw.browser.helpers.js", side_effect=fake_js),
    ):
...
def test_load_state_restores_cookies_and_storage(tmp_path) -> None:
    ...
    with (
        patch("smartclaw.browser.helpers.cdp", side_effect=fake_cdp),
        patch("smartclaw.browser.helpers.goto_url", side_effect=lambda url: goto_calls.append(url)),
        patch("smartclaw.browser.helpers.wait_for_load"),
        patch("smartclaw.browser.helpers.current_tab", return_value={"url": ""}),
        patch(
            "smartclaw.browser.helpers._set_storage_entries",
