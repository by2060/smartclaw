...
from smartclaw.browser import run


def test_c_flag_executes_code() -> None:
    stdout = StringIO()
    with (
        patch.object(sys, "argv", ["smartclaw", "browser", "-c", "print('hello from -c')"]),
        patch("smartclaw.browser.run.ensure_daemon"),
        patch("smartclaw.browser.run.print_update_banner"),
...
def test_c_flag_does_not_read_stdin() -> None:
    ...
    with (
        patch.object(sys, "argv", ["smartclaw", "browser", "-c", "x = 1"]),
        patch("smartclaw.browser.run.ensure_daemon"),
        patch("smartclaw.browser.run.print_update_banner"),
...
def test_state_show_prints_summary_without_daemon() -> None:
    stdout = StringIO()
    with (
        patch("smartclaw.browser.run.summarize_state", return_value={"cookies": 1}),
        patch("smartclaw.browser.run.ensure_daemon") as mock_daemon,
        patch("sys.stdout", stdout),
    ):
...
def test_state_save_ensures_daemon_and_prints_result() -> None:
    stdout = StringIO()
    with (
        patch("smartclaw.browser.run.save_state", return_value={"path": "/tmp/auth-state.json", "cookies": 2}),
        patch("smartclaw.browser.run.ensure_daemon") as mock_daemon,
        patch("smartclaw.browser.run.print_update_banner") as mock_banner,
        patch("sys.stdout", stdout),
    ):
...
def test_state_load_passes_optional_flags() -> None:
    stdout = StringIO()
    with (
        patch("smartclaw.browser.run.load_state", return_value={"finalUrl": "https://example.com"}) as mock_load,
        patch("smartclaw.browser.run.ensure_daemon"),
        patch("smartclaw.browser.run.print_update_banner"),
        patch("sys.stdout", stdout),
    ):
