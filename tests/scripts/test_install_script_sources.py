...
def test_install_zh_bash_bootstrap_uses_gitee_archive_and_delegates_to_zh_workspace_installer() -> None:
    ...
    assert "printf '[smartclaw-bootstrap-zh] %s\\n' \"$1\" >&2" in script
    assert 'has_cmd unzip || fail "缺少 unzip，无法解压 Gitee zip 源码包。"' in script
    assert 'archive_path="$TMP_DIR/smartclaw.zip"' in script
    assert 'unzip -q "$archive_path" -d "$TMP_DIR"' in script
    assert 'scripts/install_zh.sh' in script
    assert 'https://gitee.com/smartclaw/smartclaw/raw/main/install_zh.sh' in script
    assert 'https://gitee.com/smartclaw/smartclaw/raw/main/install_zh.ps1' in script
    assert 'SMARTCLAW_INSTALL_LANGUAGE' in script
    assert 'zh-CN' in script
    assert 'SMARTCLAW_UV_INSTALL_SH_URL' in script
    assert 'https://astral.org.cn/uv/install.sh' in script
    assert 'SMARTCLAW_UV_INSTALL_SH_FALLBACK_URL' in script
    assert 'https://uv.agentsmirror.com/install-cn.sh' in script
    assert 'SMARTCLAW_UV_INSTALL_SH_SECONDARY_FALLBACK_URL' in script
    assert 'https://astral.sh/uv/install.sh' in script
    assert 'SMARTCLAW_UV_INSTALL_PS1_URL' in script
    assert 'https://astral.org.cn/uv/install.ps1' in script
    assert 'SMARTCLAW_UV_INSTALL_PS1_SECONDARY_FALLBACK_URL' in script
    assert 'https://astral.sh/uv/install.ps1' in script
    assert 'SMARTCLAW_NPM_REGISTRY' in script
    assert 'https://registry.npmmirror.com/' in script
    assert 'SMARTCLAW_NVM_INSTALL_SCRIPT_URL' in script
...
def test_install_zh_powershell_bootstrap_uses_gitee_archive_and_delegates_to_zh_workspace_installer() -> None:
    ...
    assert 'SMARTCLAW_UV_INSTALL_PS1_URL' in script
    assert 'https://astral.org.cn/uv/install.ps1' in script
    assert 'SMARTCLAW_UV_INSTALL_PS1_FALLBACK_URL' in script
...
def test_install_zh_bash_wrapper_sets_cn_sources_and_reuses_main_installer() -> None:
    ...
    assert 'SMARTCLAW_INSTALL_LANGUAGE' in script
    assert 'zh-CN' in script
    assert 'SMARTCLAW_INSTALL_REPO_URL' in script
    assert 'https://gitee.com/smartclaw/smartclaw.git' in script
    assert 'SMARTCLAW_RAW_INSTALL_SH_URL' in script
    assert 'https://gitee.com/smartclaw/smartclaw/raw/main/install_zh.sh' in script
    assert 'SMARTCLAW_RAW_INSTALL_PS1_URL' in script
    assert 'https://gitee.com/smartclaw/smartclaw/raw/main/install_zh.ps1' in script
    assert 'PUPPETEER_CHROME_DOWNLOAD_BASE_URL' in script
    assert 'https://cdn.npmmirror.com/binaries/chrome-for-testing' in script
    assert 'SMARTCLAW_UV_DEFAULT_INDEX' in script
    ...
    assert 'SMARTCLAW_NVM_INSTALL_SCRIPT_URL' in script
    assert 'https://gitee.com/mirrors/nvm/raw/v0.40.3/install.sh' in script
    assert 'SMARTCLAW_NODEJS_MANUAL_DOWNLOAD_URL' in script
...
def test_main_bash_installer_uses_configured_default_sources_without_probing() -> None:
    ...
    assert "SMARTCLAW_NVM_GITEE_REPO_URL" in script
    assert "https://gitee.com/mirrors/nvm.git" in script
    assert "SMARTCLAW_NVM_GITEE_RAW_URL_PREFIX" in script
...
def test_windows_bootstrap_installers_detect_system32_and_fall_back_to_home() -> None:
    for path in (
        ...
        assert 'function Test-IsSystem32Path' in script
        assert 'function Get-DefaultInstallDir' in script
        assert '[System.IO.Path]::Combine($baseDirectory, "smartclaw")' in script
        assert 'Test-IsSystem32Path -Path $CurrentDirectory' in script
        assert '$InstallDir = if ([string]::IsNullOrWhiteSpace($env:SMARTCLAW_INSTALL_DIR)) { $DefaultInstallDir } else { $env:SMARTCLAW_INSTALL_DIR }' in script
...
def test_main_bash_installer_uses_cn_uv_official_fallback_after_agentsmirror() -> None:
    ...
    test_script = script_without_main + textwrap.dedent(
        ...
        export HOME="$(mktemp -d)"
        export SMARTCLAW_INSTALL_LANGUAGE="zh-CN"
        export SMARTCLAW_UV_INSTALL_SH_URL="https://primary.example/install.sh"
        export SMARTCLAW_UV_INSTALL_SH_FALLBACK_URL="https://uv.agentsmirror.com/install-cn.sh"
        export SMARTCLAW_UV_INSTALL_SH_SECONDARY_FALLBACK_URL="https://astral.sh/uv/install.sh"
        export TEST_LOG="$HOME/install-uv.log"
        INSTALL_LANGUAGE="$SMARTCLAW_INSTALL_LANGUAGE"
        UV_INSTALL_SH_URL="$SMARTCLAW_UV_INSTALL_SH_URL"
        UV_INSTALL_SH_FALLBACK_URL="$SMARTCLAW_UV_INSTALL_SH_FALLBACK_URL"
        UV_INSTALL_SH_SECONDARY_FALLBACK_URL="$SMARTCLAW_UV_INSTALL_SH_SECONDARY_FALLBACK_URL"
