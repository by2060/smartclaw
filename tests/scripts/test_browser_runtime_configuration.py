...
def test_bash_installer_prefers_explicit_browser_configuration() -> None:
    ...
    assert 'If browser installation fails, SmartClaw can still start and you can reinstall it later.' in script
    ...
    assert '"$HOME/.smartclaw/browser"' in script
...
def test_powershell_installer_prefers_explicit_browser_configuration() -> None:
    ...
    assert "SMARTCLAW_BROWSER_EXECUTABLE_OVERRIDE" in script
    ...
    assert 'Join-Path $HOME ".smartclaw\\browser"' in script
    assert 'Downloading Chrome for Testing.' in script
    assert 'If browser installation fails, SmartClaw can still start and you can reinstall it later.' in script
...
def test_powershell_installer_is_bundled_unaware() -> None:
    """install.ps1 must not branch on SMARTCLAW_INSTALL_ROOT — bundled glue lives in packaging/windows/bootstrap-windows.ps1."""
    script = (SCRIPT_DIR / "install.ps1").read_text(encoding="utf-8-sig")
    ...
    assert "smartclaw-bundled-chrome.exe.relative.txt" not in script
    assert "SMARTCLAW_INSTALL_ROOT" not in script
...
def test_powershell_bootstrap_wires_bundled_toolchain() -> None:
    ...
    assert "SMARTCLAW_SKIP_ADMIN_CHECK" in script
    assert "SMARTCLAW_BROWSER_EXECUTABLE_OVERRIDE" in script
    assert "SMARTCLAW_BUNDLED_PYTHON" in script
    ...
    assert ".smartclaw\\browser" in script
...
def test_build_staging_bundles_python_runtime() -> None:
    ...
    assert "SMARTCLAW_PYTHON_STANDALONE_MIRROR_BASE_URL" in script


def test_inno_setup_points_to_packaging_bootstrap() -> None:
    """smartclaw-setup.iss must invoke the bootstrap from its new packaging location."""
    iss = (PACKAGING_WINDOWS_DIR / "smartclaw-setup.iss").read_text(encoding="utf-8")
...
def test_inno_shortcuts_point_to_elevated_launcher() -> None:
    ...
    expected_script = 'start-smartclaw-elevated.ps1'
    start_menu_lines = [
        line
        for line in icons_block.splitlines()
        if "Start SmartClaw" in line or "{userdesktop}" in line
    ]
    assert start_menu_lines, "expected Start SmartClaw + desktop shortcut entries"
    ...
    assert "{%USERPROFILE}\\.local\\bin\\smartclaw.cmd" not in icons_block
...
def test_windows_elevated_launcher_runs_shared_wrapper_as_admin() -> None:
    ...
    script = (PACKAGING_WINDOWS_DIR / "start-smartclaw-elevated.ps1").read_text(
        encoding="utf-8-sig"
    )

    assert 'Join-Path $HOME ".local\\bin\\smartclaw.cmd"' in script
...
def test_inno_finish_page_reminds_user_to_reopen_terminal() -> None:
    ...
    run `smartclaw start` with stale env vars (no SMARTCLAW_NODE_HOME / updated PATH)."""
    iss = (PACKAGING_WINDOWS_DIR / "smartclaw-setup.iss").read_text(encoding="utf-8")
    ...
    assert "smartclaw start" in messages_block
