...
def test_bash_installer_stops_processes_before_retrying_locked_operations() -> None:
    ...
    assert "stop_smartclaw_processes()" in script
    assert "get_runtime_pid_file_paths()" in script
    assert "get_pid_from_runtime_file()" in script
    assert 'smartclaw stop >/dev/null 2>&1 || true' in script
    assert "list_smartclaw_process_ids()" in script
    ...
    assert 'ln -sf "$target" "$link_dir/smartclaw"' in script
...
def test_powershell_installer_stops_processes_before_retrying_locked_operations() -> None:
    ...
    assert "function Stop-SmartClawProcesses" in script
    ...
    assert "& $smartclawCommand.Source stop" in script
...
$env:SMARTCLAW_ROOT = Join-Path ([System.IO.Path]::GetTempPath()) 'SmartClawRoot'
