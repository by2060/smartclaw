...
async def test_all_configured_providers():
    ...
    # Check default provider from smartclaw.json
    from smartclaw.config.config import Config
    config = await Config.get()
    
    if hasattr(config, 'model') and config.model:
        print(f"\nDefault model from smartclaw.json: {config.model}")
...
async def test_cli_will_show_warning_for_unconfigured_provider():
    ...
    with patch('smartclaw.session.message.Message.create'):
        with patch('smartclaw.session.session_loop.SessionLoop.run') as mock_run:
