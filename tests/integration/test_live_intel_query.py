...
def _live_test_enabled() -> bool:
    return os.environ.get("SMARTCLAW_LIVE_TEST") == "1"
...
skip_unless_live = pytest.mark.skipif(
    ...
    reason=(
        "真实服务测试需要：(1) 设置 SMARTCLAW_LIVE_TEST=1；"
...
async def _create_session(client, title: str = "live-test", agent: str = "titan") -> str:
...
class TestWebUIAPI:
    ...
    async def test_agent_list_matches_expected(self, async_http_client):
        ...
        # 必须有的 visible agents
        for expected in ["titan", "explore", "hephaestus", "oracle", "librarian"]:
...
skip_unless_llm = pytest.mark.skipif(
    ...
    reason="CLI 实时测试需要设置 SMARTCLAW_LIVE_TEST=1 且配置好 LLM provider",
)
...
class TestQueryTypes:
    ...
    @skip_unless_live
    @pytest.mark.asyncio
    async def test_agent_selection_titan(self, async_http_client):
        """明确指定 titan agent 执行查询。"""
        sid = await _create_session(async_http_client, "titan-agent-test", agent="titan")
        resp = await _send_message(async_http_client, sid, "查询IP 8.8.8.8威胁情报")

        info = resp.get("info", {})
        assert info.get("finish") == "stop"
        # agent 字段应为 titan
        assert info.get("agent") == "titan", f"Expected agent=titan, got {info.get('agent')}"
