...
2. Workflow 注入全链路（使用临时文件模拟真实环境）
   - scan_skill_workflows() 正确发现 .smartclaw/workflow 目录中的 workflow.json
   - build_workflows_section() 能把它渲染成 prompt 片段
   - inject_dynamic_prompts() 将 AvailableWorkflow 传递给 prompt builder

3. Titan prompt 包含 workflow 段落
   - Titan agent 加载后，其 prompt 包含 "Available Workflows" 字样（前提：存在工作流）
...
from smartclaw.agent.agent import AvailableWorkflow
...
class TestToolVisibility:
    ...
    def test_tools_grouped_by_real_categories(self):
        ...
        from smartclaw.agent.prompt_utils import _CATEGORY_LABELS
        ToolRegistry.init()
        all_tools = ToolRegistry.list_tools()
        if not all_tools:
            pytest.skip("No tools registered")
    ...
    def test_mock_tool_set_all_categories_visible(self):
        ...
        with patch("smartclaw.tool.registry.ToolRegistry.get", side_effect=lambda n: tool_map.get(n)), \
             patch("smartclaw.tool.registry.ToolRegistry.init"):
...
class TestWorkflowInjectionPipeline:
    ...
    def temp_workflow_dir(self, tmp_path: Path):
        """Create a fake .smartclaw/workflow/ndr_triage directory with workflow.json."""
        wf_dir = tmp_path / ".smartclaw" / "workflow" / "ndr_triage"
    ...
    async def test_scan_discovers_workflow_json(self, temp_workflow_dir):
        ...
        base_dir 是项目根（包含 .smartclaw/），而非 .smartclaw/ 本身。
        """
        from smartclaw.workflow.center import scan_skill_workflows

        # Pass base_dir = tmp_path so that scan finds <base_dir>/.smartclaw/workflow/ndr_triage/
    ...
    def test_build_workflows_section_with_real_objects(self):
        ...
        workflows = [
            AvailableWorkflow(
                name="ndr_triage",
                description="NDR 告警自动研判",
                path="/tmp/.smartclaw/workflow/ndr_triage/workflow.json",
                source="project",
            ),
            AvailableWorkflow(
                name="host_compromise",
                description="主机入侵研判",
                path="/tmp/.smartclaw/workflow/host_compromise/workflow.json",
                source="global",
            ),
        ]
    ...
    def test_workflow_section_injected_into_agent_prompt(self, tmp_path):
        ...
        from smartclaw.agent.agent_factory import inject_dynamic_prompts
        from smartclaw.agent.agent import AgentInfo
        import textwrap

        builder_code = textwrap.dedent("""
        from smartclaw.agent.prompt_utils import build_workflows_section
...
# ===========================================================================
# 3. Titan prompt 包含 workflow 段落（存在工作流时）
# ===========================================================================

@pytest.mark.integration
class TestTitanPromptWorkflowAwareness:

    @pytest.mark.asyncio
    async def test_titan_prompt_contains_tools_section(self):
        """Titan prompt 必须包含工具信息（基础能力感知）。"""
        from smartclaw.agent.registry import Agent
        titan = await Agent.get("titan")
        assert titan is not None
        assert titan.prompt is not None
        assert len(titan.prompt) > 100

    @pytest.mark.asyncio
    async def test_titan_prompt_contains_subagents_section(self):
        """Titan prompt 必须包含 SubAgent 信息。"""
        from smartclaw.agent.registry import Agent
        titan = await Agent.get("titan")
        assert titan is not None
        # Titan should know about at least one of its common subagents
        prompt = titan.prompt or ""
        assert any(name in prompt for name in ["explore", "oracle", "metis", "momus"]), (
            "Titan prompt does not reference any known subagents"
        )

    @pytest.mark.asyncio
    async def test_titan_prompt_contains_skills_section(self):
        """Titan prompt 必须包含 skills 相关信息。"""
        from smartclaw.agent.registry import Agent
        titan = await Agent.get("titan")
        prompt = (titan.prompt or "").lower()
        assert "skill" in prompt

    @pytest.mark.asyncio
    async def test_titan_prompt_has_workflow_placeholder_or_section(self):
        """Titan prompt 应包含 workflow 相关内容（已注入或有占位结构）。"""
        from smartclaw.agent.registry import Agent
        titan = await Agent.get("titan")
        prompt = (titan.prompt or "").lower()
    ...
    @pytest.mark.asyncio
    async def test_titan_prompt_prefers_direct_ioc_lookup_before_delegation(self):
        """单 IOC 情报查询应在提示词中优先走 Titan 直查路径。"""
        from smartclaw.agent.registry import Agent
        titan = await Agent.get("titan")
        prompt = titan.prompt or ""
        assert "Single IOC basic lookup only" in prompt
        assert '"查询 8.8.8.8 的情报" -> Titan should directly query TI tools' in prompt
...
class TestSkillsCommandE2E:
    ...
    async def test_skills_command_does_not_error(self):
        ...
        from smartclaw.tool.system.slash_command import run_slash_command_tool
...
class TestWorkflowsCommandE2E:
    ...
    async def test_workflows_command_with_mock_scan(self):
        ...
        with patch(
            "smartclaw.workflow.center.scan_skill_workflows",
