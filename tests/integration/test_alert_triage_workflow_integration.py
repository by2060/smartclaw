...
.smartclaw/workflow/alert_triage/workflow.json

测试要求：
- ThreatBook API Key 已配置（.smartclaw/.secret.json）
- LLM Provider 已配置（.smartclaw/smartclaw.json）
...
WORKFLOW_PATH = PROJECT_ROOT / ".smartclaw" / "workflow" / "alert_triage" / "workflow.json"
...
def test_host_mode() -> RunWorkflowResult:
    ...
    import smartclaw.workflow.runner as runner_module
...
def test_sandbox_mode() -> RunWorkflowResult:
    ...
    try:
        cp = subprocess.run(
            ["docker", "ps", "--filter", "name=smartclaw-sbx", "--format", "{{.Names}}"],
            capture_output=True,
            text=True,
            timeout=10,
        )
        ...
        if containers:
            ...
            _logger.warning("未检测到 smartclaw sandbox 容器，将尝试自动创建")
    except Exception:
        pass
