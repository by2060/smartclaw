...
import pytest
import asyncio
from smartclaw.mcp import MCP, McpStatus, get_manager
...
class TestThreatBookIntegration:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_tools_registered_in_smartclaw(self, threatbook_config):
        """测试工具注册到 SmartClaw"""
        await MCP.connect("threatbook", threatbook_config)
    ...
    async def test_tool_source_tracking(self, threatbook_config):
        ...
        assert source.smartclaw_tool == tool_name
