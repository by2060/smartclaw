...
async def test_titan_query_ip_intelligence():
    """
    测试 Titan 执行 IP 情报查询：查一下 8.8.8.8 的情报
    
    完整流程：
    1. 用户输入："查一下 8.8.8.8 的情报"
    2. Titan 分析后调用 threatbook_ip_query 工具
    3. 工具返回结果
    4. Titan 整理结果给出最终回答
    """
    # 初始化工具注册表
    ...
    # Mock LLM 响应序列
    with patch('smartclaw.provider.provider.Provider.chat') as mock_chat:
        # 第一次调用：Titan 决定调用工具
...
这是 Google 提供的公共 DNS 服务器，安全可靠，无威胁风险。"""
        ...
        # Mock 工具执行
        with patch('smartclaw.tool.registry.ToolRegistry.execute', side_effect=mock_tool_execute):
            # 运行 SessionLoop
...
@pytest.mark.asyncio
async def test_titan_tool_call_with_callbacks():
    """
    测试 Titan 工具调用的回调机制
    ...
    with patch('smartclaw.provider.provider.Provider.chat') as mock_chat:
        ...
        # Mock 工具执行
        with patch('smartclaw.tool.registry.ToolRegistry.execute') as mock_tool:
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="Mock path incorrect: Provider.chat classmethod is not intercepted by patch; needs refactoring")
async def test_titan_multi_tool_calls():
    """
    测试 Titan 执行多个工具调用
