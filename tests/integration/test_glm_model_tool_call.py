...
async def test_parse_glm_model_string():
    ...
    # Test parsing the GLM model string from smartclaw.json
...
async def test_custom_provider_configuration():
    ...
    if provider:
        ...
        print("Provider should be registered in smartclaw/provider/provider.py")
...
async def test_glm_model_supports_tools():
    ...
    print(f"\nDefault model from smartclaw.json: {config.model if hasattr(config, 'model') else 'None'}")
    ...
    if hasattr(config, 'model') and config.model:
        ...
        if "/" in config.model:
            ...
            # Note: This info is in smartclaw.json under provider.models
            print("\nAccording to smartclaw.json config:")
...
async def test_check_secret_file():
    ...
    secret_file = Path(".smartclaw/.secret.json")
    ...
    if secret_file.exists():
        ...
        if has_key:
            ...
            print("\nTo fix, add to .smartclaw/.secret.json:")
...
async def test_recommend_debugging_steps():
    ...
    print("\n6️⃣  临时解决方案:")
    print("   使用已知支持工具的模型:")
    print("   smartclaw run --model claude-sonnet-4 --provider anthropic")
