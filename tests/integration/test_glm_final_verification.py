...
验证整个流程：smartclaw.json -> custom provider -> GLM 模型 -> 工具调用
"""
...
@pytest.mark.asyncio
async def test_smartclaw_json_default_model():
    """
    验证：smartclaw.json 中的默认模型配置
    """
...
async def test_glm_model_tool_support():
    ...
    验证：GLM 模型配置显示支持工具调用
    """
    # Check smartclaw.json model capabilities
    print("\n✅ According to smartclaw.json:")
...
async def test_summary():
    ...
    print("\n" + "="*60)
    print("✅ 配置完成！系统应该能够：")
    print("  1. 从 smartclaw.json 读取默认 GLM 模型")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_smartclaw_json_default_model())
