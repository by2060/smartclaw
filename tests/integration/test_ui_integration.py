...
class TestCLIIntegration:
    ...
    def test_cli_help_command(self):
        ...
        assert "smartclaw" in result.stdout.lower()
...
class TestServerAPI:
    ...
    async def test_server_imports(self):
        """测试 Server 模块可以正常导入"""
        from smartclaw.server import app
        assert app is not None
    
    @pytest.mark.asyncio
    async def test_session_routes_available(self):
        """测试 Session 路由可用"""
        from smartclaw.server.routes import session
    ...
    async def test_create_session_via_routes(self):
        """测试通过路由创建 session"""
        from smartclaw.server.routes.session import create_session
...
class TestPermissionFlow:
    ...
    async def test_tool_declaration_check_in_dialogue(self):
        ...
        result = await Agent.has_tool("titan", "read")
...
class TestSessionLifecycle:
    ...
    async def test_complete_session_lifecycle(self):
        ...
        from smartclaw.session.session import Session
        from smartclaw.session.message import Message, MessageRole
        from smartclaw.session.core.status import SessionStatus, SessionStatusBusy
