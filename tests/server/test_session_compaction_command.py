...
from smartclaw.server.routes.session import (
    CommandRequest,
    _resolve_compaction_context,
    _run_session_compaction,
    send_session_command,
)
from smartclaw.session.lifecycle.compaction.compaction import SessionCompaction
from smartclaw.session.message import CompactionPart, MessageRole
...
async def test_run_session_compaction_uses_latest_user_and_publishes_status():
    ...
    with patch(
        "smartclaw.server.routes.session.Session.get_by_id",
        new=AsyncMock(return_value=session),
    ), patch(
        "smartclaw.server.routes.session._resolve_compaction_context",
        new=AsyncMock(return_value=("titan", "anthropic", "claude-test")),
    ), patch(
        "smartclaw.session.lifecycle.revert.SessionRevert.cleanup",
        new=AsyncMock(),
    ), patch(
        "smartclaw.session.message.Message.list",
        new=AsyncMock(return_value=messages),
    ), patch(
        "smartclaw.provider.provider.Provider.resolve_model_info",
        return_value=(200_000, 8_192, None),
    ), patch(
        "smartclaw.session.lifecycle.compaction.compaction.SessionCompaction.process",
    ...
    assert result == ("titan", "anthropic", "claude-test")
...
async def test_session_compaction_create_adds_compaction_part():
    ...
    with patch(
        ...
        "smartclaw.session.message.Message.add_part",
        new=AsyncMock(),
    ) as add_part_mock:
...
async def test_resolve_compaction_context_prefers_requested_agent_override():
    ...
    with patch(
        ...
        "smartclaw.storage.storage.Storage.read",
        ...
        "smartclaw.config.config.Config.resolve_default_llm",
...
async def test_compact_command_uses_resolved_compaction_context_for_message_metadata():
    ...
    with patch(
        ...
        "smartclaw.project.instance.Instance.provide",
        new=AsyncMock(side_effect=provide_side_effect),
    ), patch(
        "smartclaw.server.routes.session._run_session_compaction",
        new=AsyncMock(),
