...
async def _cleanup_test_agent(_server_isolated_env):
    from smartclaw.agent.agent_factory import delete_yaml_agent
...
class TestAgentList:
    ...
    async def test_list_agents_have_required_fields(self, client: AsyncClient):
        """Each agent in the list has the required SmartClaw-compatible fields."""
        resp = await client.get("/api/agent")
...
class TestAgentGet:
    ...
    async def test_get_builtin_agent(self, client: AsyncClient):
        """GET /api/agent/titan returns the built-in titan agent."""
        resp = await client.get("/api/agent/titan")
        assert resp.status_code == status.HTTP_200_OK
...
class TestAgentCreate:
    ...
    async def test_create_agent_writes_yaml_and_storage_overlay(self, client: AsyncClient):
        ...
        from smartclaw.agent.agent_factory import delete_yaml_agent, find_yaml_agent
        from smartclaw.storage.storage import Storage
...
class TestAgentDelete:
    ...
    async def test_delete_builtin_agent_returns_error(self, client: AsyncClient):
        ...
        (no Storage key 'agent/custom/titan' and no YAML override file)."""
        resp = await client.delete("/api/agent/titan")
        # If titan has no Storage / YAML entry the route returns 404.
...
class TestAgentRun:
    ...
    async def test_run_agent_creates_session(self, client: AsyncClient):
        ...
        then call /test on it.  Built-in agents (titan etc.) depend on the
