...
touch the user's real ``~/.smartclaw/config`` directory.
"""
...
from smartclaw.server.routes import provider as provider_routes
...
def patched_runtime(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("smartclaw.security.get_secret_manager", lambda: fake_secrets)
    ...
    # Pretend the provider already exists in smartclaw.json so set_provider_credentials
...
class TestGetCredentialsHidesPlaceholder:
    ...
    async def test_placeholder_is_hidden_but_has_credential_true(
        ...
        monkeypatch.setattr(
            "smartclaw.security.get_secret_manager", lambda: fake_secrets
        )
        monkeypatch.setattr(
            provider_routes.ConfigWriter,
            "get_provider_raw",
    ...
    @pytest.mark.asyncio
    async def test_inline_placeholder_in_smartclaw_json_also_hidden(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        """Defense-in-depth: even if a hand-edited smartclaw.json contains an
