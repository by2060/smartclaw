...
def isolated_workflow_filesystem(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    from smartclaw.server.routes import workflow as workflow_routes
    from smartclaw.workflow import fs_store

    workspace_root = tmp_path / "workspace"
    project_root = workspace_root / ".smartclaw" / "plugins" / "workflows"
    global_root = tmp_path / "home" / ".smartclaw" / "plugins" / "workflows"
    legacy_project_plugin = workspace_root / ".smartclaw" / "plugins" / "workflow"
    legacy_project_main = workspace_root / ".smartclaw" / "workflow"
    legacy_global_plugin = tmp_path / "home" / ".smartclaw" / "plugins" / "workflow"
    legacy_global_main = tmp_path / "home" / ".smartclaw" / "workflow"

    for root in [
        workspace_root / ".smartclaw",
        project_root,
        global_root,
...
class TestWorkflowRoutes:
    ...
    async def test_run_node_returns_before_saving_test_results(
        ...
        from smartclaw.server.routes import workflow as workflow_routes
        import smartclaw.workflow.engine as workflow_engine_module
...
class TestPermissionRoutes:
    ...
    async def test_permission_routes_preserve_request_created_time(self, client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
        ...
        from smartclaw.permission.next import PermissionRequestInfo
        ...
        monkeypatch.setattr(
            "smartclaw.server.routes.permission.PermissionNext.list_pending_infos",
            AsyncMock(return_value=[info]),
        )
        monkeypatch.setattr(
            "smartclaw.server.routes.permission.PermissionNext.get_pending_info",
            AsyncMock(return_value=info),
        )
