...
from smartclaw.auth.context import AuthUser
from smartclaw.session.message import Message, MessageRole
from smartclaw.session.session import Session
from smartclaw.tool.registry import Tool, ToolCategory, ToolInfo, ToolRegistry, ToolResult
...
def _patch_session_user(monkeypatch: pytest.MonkeyPatch, role: str) -> None:
    from smartclaw.server import auth as auth_module
...
class TestToolRouteSecurity:
    ...
    async def test_viewer_cannot_create_plugin_tool(self, client: AsyncClient, monkeypatch: pytest.MonkeyPatch):
        ...
        response = await client.post(
            "/api/tools",
            headers={"cookie": "smartclaw_session=viewer-session"},
            json={
    ...
    async def test_http_tool_context_with_child_session_scopes_outputs_to_root(self):
        from smartclaw.server.routes import tool as tool_routes
...
class TestAPIToolDraftRoutes:
    ...
    async def test_generate_overrides_provider_references_with_provider_id(
        ...
        from smartclaw.tool import api_tool_draft_llm
        from smartclaw.tool.api_tool_draft import APIToolDraft, NonAPIToolDraftResult, ProviderDraft, ToolDraft
    ...
    async def test_generate_returns_non_api_related_result_without_draft_validation(
        ...
        from smartclaw.tool import api_tool_draft_llm
        from smartclaw.tool.api_tool_draft import NonAPIToolDraftResult
    ...
    async def test_confirm_writes_provider_yaml_and_registers_tool(
        ...
        provider_path = project_dir / ".smartclaw" / "plugins" / "tools" / "api" / provider_id / "_provider.yaml"
        tool_path = project_dir / ".smartclaw" / "plugins" / "tools" / "api" / provider_id / f"{tool_name}.yaml"
    ...
    async def test_confirm_rejects_invalid_error_mapping_without_writing_files(
        ...
        provider_dir = project_dir / ".smartclaw" / "plugins" / "tools" / "api" / provider_id
    ...
    async def test_confirm_upsert_rejects_cross_provider_tool_collision(
        ...
        assert not (
            project_dir / ".smartclaw" / "plugins" / "tools" / "api" / other_provider_id / "_provider.yaml"
        ).exists()
    ...
    async def test_delete_draft_provider_removes_directory_and_config_keeps_secret_by_default(
        ...
        from smartclaw.security import secrets as secrets_module
        ...
        assert not (project_dir / ".smartclaw" / "plugins" / "tools" / "api" / provider_id).exists()
