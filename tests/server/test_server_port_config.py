...
from smartclaw.cli import main as cli_main
from smartclaw.server.app import ServerInfo
...
class TestPortConfigurationFromConfig:
    ...
    @patch.dict(os.environ, {'SMARTCLAW_SERVER_PORT': '9000'})
    def test_port_from_environment_variable(self):
        """Test port configuration from SMARTCLAW_SERVER_PORT environment variable."""
        # Clear cached config
    ...
    @patch.dict(os.environ, {'SMARTCLAW_SERVER_HOST': '0.0.0.0'})
    def test_host_from_environment_variable(self):
        """Test host configuration from SMARTCLAW_SERVER_HOST environment variable."""
        # Clear cached config
        Config._global_config = None
...
class TestCommandLinePortConfiguration:
    ...
    def test_restart_environment_overrides_runtime_record(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setenv("SMARTCLAW_SERVER_HOST", "127.0.0.1")
        monkeypatch.setenv("SMARTCLAW_SERVER_PORT", "9101")
        monkeypatch.setenv("SMARTCLAW_WEBUI_HOST", "127.0.0.1")
        monkeypatch.setenv("SMARTCLAW_WEBUI_PORT", "5275")
        Config._global_config = None
    ...
    def test_service_config_prefers_cli_values(self, monkeypatch):
        ...
        monkeypatch.setenv("SMARTCLAW_SERVER_HOST", "10.0.0.1")
        monkeypatch.setenv("SMARTCLAW_SERVER_PORT", "8100")
        monkeypatch.setenv("SMARTCLAW_WEBUI_HOST", "10.0.0.2")
        monkeypatch.setenv("SMARTCLAW_WEBUI_PORT", "5274")
        Config._global_config = None
    ...
    def test_service_config_uses_server_and_webui_environment(self, monkeypatch):
        ...
        monkeypatch.setenv("SMARTCLAW_SERVER_HOST", "0.0.0.0")
        monkeypatch.setenv("SMARTCLAW_SERVER_PORT", "9001")
        monkeypatch.setenv("SMARTCLAW_WEBUI_HOST", "0.0.0.0")
        monkeypatch.setenv("SMARTCLAW_WEBUI_PORT", "5175")
        Config._global_config = None
    ...
    def test_service_config_keeps_legacy_env_fallbacks(self, monkeypatch):
        ...
        monkeypatch.delenv("SMARTCLAW_SERVER_HOST", raising=False)
        monkeypatch.delenv("SMARTCLAW_SERVER_PORT", raising=False)
        monkeypatch.delenv("SMARTCLAW_WEBUI_HOST", raising=False)
        monkeypatch.delenv("SMARTCLAW_WEBUI_PORT", raising=False)
        monkeypatch.setenv("SMARTCLAW_BACKEND_HOST", "0.0.0.0")
        monkeypatch.setenv("SMARTCLAW_BACKEND_PORT", "9200")
        monkeypatch.setenv("SMARTCLAW_FRONTEND_HOST", "0.0.0.0")
        monkeypatch.setenv("SMARTCLAW_FRONTEND_PORT", "5176")
        Config._global_config = None
    ...
    def test_cli_tui_command_default_port(self):
        ...
        from smartclaw.cli.main import app
...
class TestClientPortConfiguration:
    ...
    @pytest.mark.skip(reason="SmartClawClient has import issues with get_manager")
    def test_smartclaw_client_default_url(self):
        """Test SmartClawClient uses correct default base URL."""
        from smartclaw.server.client import SmartClawClient

        # Default should be http://127.0.0.1:8000
        client = SmartClawClient()

        assert "8000" in client.base_url
        assert "127.0.0.1" in client.base_url or "localhost" in client.base_url

    @pytest.mark.skip(reason="SmartClawClient has import issues with get_manager")
    def test_smartclaw_client_custom_url(self):
        """Test SmartClawClient with custom base URL."""
        from smartclaw.server.client import SmartClawClient

        custom_url = "http://192.168.1.100:9000"
        client = SmartClawClient(base_url=custom_url)
...
class TestEnvironmentVariablePortConfig:
    ...
    @patch.dict(os.environ, {'SMARTCLAW_PORT': '7000'})
    def test_script_port_env_var(self):
        """Test SMARTCLAW_PORT environment variable (used in scripts)."""
        port = os.getenv('SMARTCLAW_PORT', '8000')

        assert port == '7000'

    def test_script_port_env_var_default(self):
        """Test SMARTCLAW_PORT defaults to 8000 when not set."""
        # Temporarily remove the env var if it exists
        old_value = os.environ.pop('SMARTCLAW_PORT', None)

        try:
            port = int(os.getenv('SMARTCLAW_PORT', '8000'))
            assert port == 8000
        finally:
            # Restore old value if it existed
            if old_value is not None:
                os.environ['SMARTCLAW_PORT'] = old_value

    @patch.dict(os.environ, {'SMARTCLAW_HOST': '0.0.0.0'})
    def test_script_host_env_var(self):
        """Test SMARTCLAW_HOST environment variable."""
        host = os.getenv('SMARTCLAW_HOST', '127.0.0.1')
