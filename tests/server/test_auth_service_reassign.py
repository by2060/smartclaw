...
sibling exposed via ``smartclaw admin reassign-orphan-sessions``; the tests
...
async def test_reassign_orphan_sessions_skips_owned_and_rewrites_orphans(monkeypatch):
    ...
    from smartclaw.auth import service as auth_service_module
    from smartclaw.session import session as session_module
