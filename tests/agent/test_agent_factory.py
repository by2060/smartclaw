...
覆盖 smartclaw.agent.agent_factory 的核心逻辑：
...
import smartclaw.agent.agent_factory as _factory_module
...
from smartclaw.agent.agent import AgentInfo, AgentModel, AgentPromptMetadata
...
class TestLoadAgent:
    ...
    def test_non_titan_agent_can_declare_tool_search(self, tmp_path):
        agent_dir = _write_agent_dir(tmp_path, """
...
class TestScanAndLoad:
    ...
    def test_scans_builtin_agents(self):
        ...
        from smartclaw.agent.agent_factory import _BUILTIN_AGENTS_DIR
...
class TestInjectDynamicPrompts:
    ...
    @pytest.mark.asyncio
    async def test_titan_gets_injected_prompt(self):
        """Titan agent must receive a dynamic prompt after full load."""
        ...
        assert len(titan.prompt) > 100, "Titan prompt should be non-trivially long"
    ...
    @pytest.mark.asyncio
    async def test_titan_junior_gets_injected_prompt(self):
        from smartclaw.agent.registry import Agent
        jr = await Agent.get("titan-junior")
    ...
    async def test_plugin_agent_has_prompt_if_installed(self):
        """Plugin agents installed in ~/.smartclaw/plugins/agents/ should load their prompt."""
        from smartclaw.agent.registry import Agent
...
class TestAgentConfigOverrides:
    ...
    @pytest.mark.asyncio
    async def test_agent_alias_sisyphus_resolves_to_titan(self):
        from smartclaw.agent.registry import Agent
        titan = await Agent.get("titan")
        alias = await Agent.get("sisyphus")
        assert alias is not None
        assert alias.name == titan.name
    ...
    async def test_enabled_agents_whitelist(self):
        ...
        from smartclaw.agent.agent_factory import scan_and_load
        result = scan_and_load()
...
class TestBuildAvailableAgents:
    ...
    def test_primary_agents_excluded(self):
        from smartclaw.agent.registry import _build_available_agents
        agents = {
            "titan": AgentInfo(name="titan", mode="primary", delegatable=False, native=True),
    ...
    def test_non_delegatable_agents_excluded(self):
        from smartclaw.agent.registry import _build_available_agents
        agents = {
            "titan-junior": AgentInfo(name="titan-junior", mode="subagent", hidden=False, delegatable=False, native=True),
            "hephaestus": AgentInfo(name="hephaestus", mode="subagent", hidden=False, delegatable=True, native=True),
        }
        available = _build_available_agents(agents)
        names = [a.name for a in available]
        assert "titan-junior" not in names
        assert "hephaestus" in names

    def test_default_metadata_generated_for_agent_without_prompt_metadata(self):
        from smartclaw.agent.registry import _build_available_agents, _make_default_prompt_metadata
        agent = AgentInfo(
            name="plugin_x",
            description="Does X",
            mode="subagent",
...
class TestProjectLevelAgentScan:
    """scan_and_load() should discover agents from <cwd>/.smartclaw/plugins/agents/."""
    ...
    def test_project_plugin_agents_are_discovered(
        ...
        """Agents placed in <cwd>/.smartclaw/plugins/agents/ are loaded by scan_and_load()."""
        project_agents_dir = tmp_path / ".smartclaw" / "plugins" / "agents"
    ...
    def test_project_agent_does_not_override_builtin(
        ...
        project_agents_dir = tmp_path / ".smartclaw" / "plugins" / "agents"
        self._write_agent(project_agents_dir, "titan")

        monkeypatch.chdir(tmp_path)
        result = scan_and_load()

        # Built-in titan should be retained, project-level duplicate skipped
        assert result["titan"].native is True

    def test_project_plugin_agent_is_native(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        """Agents in <cwd>/.smartclaw/plugins/agents/ are marked native=True."""
        project_agents_dir = tmp_path / ".smartclaw" / "plugins" / "agents"
    ...
    def test_project_agent_pack_nested_agents_are_discovered(
        ...
        """Agent packs can expose one-level nested SmartClaw agent directories."""
        pack_dir = tmp_path / ".smartclaw" / "plugins" / "agents" / "pentest-ai-agents"
    ...
    def test_create_yaml_agent_uses_project_plugin_dir(
        ...
        expected = tmp_path / ".smartclaw" / "plugins" / "agents" / "created-agent" / "agent.yaml"
    ...
    def test_user_plugin_agent_is_not_native(
        ...
        """Agents in ~/.smartclaw/plugins/agents/ are marked native=False."""
        import smartclaw.agent.agent_factory as _fac
