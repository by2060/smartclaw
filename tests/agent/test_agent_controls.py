...
@pytest.mark.asyncio
async def test_rex_session_allowed_subagents_scope_delegation(monkeypatch):
    rex = AgentInfo(name="rex", mode="primary", delegatable=False)
    ...
    async def fake_get_agent(name: str):
        return {"rex": rex, "explore": explore, "oracle": oracle}.get(name)
    ...
    assert await rex_session_allows_subagent("session-1", "rex", "explore") is True
    assert await rex_session_allows_subagent("session-1", "rex", "oracle") is False


@pytest.mark.asyncio
async def test_workflow_rex_session_allows_only_l1_subagents(monkeypatch):
    rex = AgentInfo(name="rex", mode="primary", delegatable=False)
    ...
    async def fake_get_agent(name: str):
        return {"rex": rex, "l1-agent": l1_agent, "non-l1": non_l1}.get(name)
    ...
    assert await rex_session_allows_subagent("session-1", "rex", "l1-agent") is True
    assert await rex_session_allows_subagent("session-1", "rex", "non-l1") is False
...
async def test_workflow_tool_context_allows_only_l1_subagents(monkeypatch):
    ...
    assert await rex_session_allows_subagent("session-1", "rex", "l1-agent", extra) is True
    assert await rex_session_allows_subagent("session-1", "rex", "non-l1", extra) is False


@pytest.mark.asyncio
async def test_workflow_rex_session_uses_full_skill_catalog(monkeypatch):
    ...
    assert await rex_session_uses_full_skill_catalog("session-1", "rex") is True
    assert await rex_session_uses_full_skill_catalog("session-1", "worker") is False


@pytest.mark.asyncio
async def test_non_rex_ignores_session_allowed_subagents(monkeypatch):
    ...
    assert await rex_session_allows_subagent("session-1", "worker", "oracle") is True
