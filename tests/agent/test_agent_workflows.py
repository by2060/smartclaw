...
from smartclaw.agent.agent import AgentInfo, AvailableSkill, AvailableWorkflow
...
def _make_agent_with_builder(tmp_path: Path, inject_body: str) -> AgentInfo:
    ...
    # Derive module path from file path (simulate smartclaw package root = tmp_path.parent)
...
def _simple_workflows() -> List[AvailableWorkflow]:
    return [
        ...
        AvailableWorkflow(
            name="global_scan",
            description="全局扫描工作流",
            path="/home/user/.smartclaw/workflow/global_scan/workflow.json",
            source="global",
        ),
    ]
