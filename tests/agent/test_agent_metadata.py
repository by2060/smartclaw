...
import asyncio
import pytest
from smartclaw.agent.registry import (
    is_delegatable,
    get_agent_mode,
    is_hidden,
...
class TestAgentMetadata:
    ...
    def test_is_delegatable_primary_agents(self):
        ...
        assert is_delegatable("titan") is False
    ...
    def test_is_delegatable_special_agents(self):
        """特殊 agents（plan, titan-junior）不应该被委派"""
        assert is_delegatable("plan") is False
        assert is_delegatable("titan-junior") is False
    ...
    def test_get_agent_mode(self):
        """测试获取 agent 模式"""
        assert get_agent_mode("titan") == "primary"
    ...
    def test_is_hidden(self):
        ...
        assert is_hidden("titan") is False
...
class TestNoCircularDependency:
    ...
    def test_no_circular_dependency(self):
        from smartclaw.agent.registry import is_delegatable as _is_delegatable
        from smartclaw.agent.registry import Agent
        from smartclaw.tool.registry import ToolRegistry
        from smartclaw.tool.agent.delegate_task import delegate_task_tool as delegate_task_tool
...
class TestDelegateTaskIntegration:
    ...
    def test_delegate_task_uses_registry(self):
        import inspect
        import smartclaw.tool.agent.delegate_task as delegate_task_module
        ...
        assert any(
            ...
            and node.module == "smartclaw.agent.registry"
            and any(alias.name == "is_delegatable" for alias in node.names)
            for node in tree.body
        )
        assert "from smartclaw.agent.metadata import is_delegatable" not in source
