...
单元测试：smartclaw.agent.prompt_utils 中被修改 / 新增的函数
...
from smartclaw.agent.agent import AvailableAgent, AvailableCategory, AvailableSkill, AvailableTool, AvailableWorkflow
...
class TestCategorizeTools:
    ...
    _REGISTRY_GET = "smartclaw.tool.registry.ToolRegistry.get"
    _REGISTRY_INIT = "smartclaw.tool.registry.ToolRegistry.init"
...
class TestBuildWorkflowsSection:
    ...
    def test_path_column_in_table(self):
        wf = self._make_workflow(path="/home/user/.smartclaw/workflow/ndr/workflow.json")
        output = build_workflows_section([wf])
        assert "/home/user/.smartclaw/workflow/ndr/workflow.json" in output
