...
class TestResolveModel:
    ...
    async def test_priority_1_request_model(self):
        ...
        from smartclaw.server.routes.session import _resolve_model
    ...
    async def test_priority_5_env_fallback(self):
        ...
        with patch("smartclaw.server.routes.session._get_last_model", return_value=None), \
             patch("smartclaw.config.config.Config") as mock_config_cls:
    ...
    async def test_pinned_session_model_beats_agent_and_config(self):
        ...
        with patch("smartclaw.server.routes.session.Session.get_by_id", AsyncMock(return_value=pinned_session)), \
             patch("smartclaw.storage.storage.Storage.read", AsyncMock(return_value={})):
            provider_id, model_id, source = await _resolve_model(request, agent, "test-session")
    ...
    async def test_unpinned_session_model_falls_through_to_config(self):
        ...
        with patch("smartclaw.server.routes.session.Session.get_by_id", AsyncMock(return_value=legacy_session)), \
             patch("smartclaw.storage.storage.Storage.read", AsyncMock(return_value={})), \
             patch("smartclaw.config.config.Config.resolve_default_llm", AsyncMock(return_value={
                 "provider_id": "openai",
                 "model_id": "gpt-4o",
             })), \
             patch("smartclaw.server.routes.session._get_last_model", AsyncMock(return_value=None)):
            provider_id, model_id, source = await _resolve_model(request, agent, "test-session")
    ...
    async def test_process_session_message_pins_explicit_request_model(self, monkeypatch):
        ...
        from types import SimpleNamespace
        from smartclaw.server.routes import session as session_routes
        ...
        monkeypatch.setattr(
            "smartclaw.agent.registry.Agent.default_agent",
            AsyncMock(return_value="titan"),
        )
        monkeypatch.setattr(
            "smartclaw.agent.registry.Agent.get",
            AsyncMock(return_value=SimpleNamespace(name="titan", model=None)),
        )
        ...
        monkeypatch.setattr("smartclaw.session.session.Session.update", update_mock)
        monkeypatch.setattr("smartclaw.provider.provider.Provider._ensure_initialized", lambda: None)
        monkeypatch.setattr("smartclaw.provider.provider.Provider.apply_config", AsyncMock())
        monkeypatch.setattr("smartclaw.provider.provider.Provider.get", lambda _provider_id: object())
        monkeypatch.setattr("smartclaw.config.config.Config.get", AsyncMock(return_value=SimpleNamespace()))
        monkeypatch.setattr("smartclaw.tool.registry.ToolRegistry.init", lambda: None)
        monkeypatch.setattr(
            "smartclaw.session.lifecycle.revert.SessionRevert.cleanup",
            AsyncMock(),
        )
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "smartclaw.server.routes.event.publish_event",
            AsyncMock(),
        )
