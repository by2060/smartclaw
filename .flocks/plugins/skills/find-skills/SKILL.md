...
This skill helps you discover skills within the current Flocks authorization scope.

## Authorization Boundary

Before answering any question about "skills", "available skills", "skill sources", or "what skills can Rex use", decide which catalog is authorized:

- Normal `user` sessions: only use the skills allowed by the current agent's `agent.yaml` `skills` list. For Rex, answer from Rex's allowlist unless the runtime explicitly says otherwise.
...
Only search external registries when the user explicitly asks for installable candidates or wants to extend the system. Do not use external search to answer "what can Rex currently use?".
