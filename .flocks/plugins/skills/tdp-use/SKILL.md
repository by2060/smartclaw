...
description_cn: 用于处理 TDP 威胁检测平台相关任务，适合通过 API 或结合浏览器进行安全态势查看、告警检索、告警获取、威胁事件调查、受害主机排查、资产风险查询等场景。只要用户提到需要打开/操作/获取/浏览 TDP、微步 NDR 等需求时，必须先加载本 skill。本 skill 是 TDP 平台操作的唯一决策入口：在未阅读本 skill 并完成模式判断前，不要直接调用任何 tdp_* tool。
---
