...
description_cn: 对任意 Flocks skill 进行多维度安全审计，覆盖结构完整性、Prompt Injection、危险命令、数据外传、凭据泄露、依赖安全、权限最小化、混淆隐藏内容和 LLM 语义分析。输出整体风险评级（CRITICAL/HIGH/MEDIUM/LOW/SAFE）及带行号的问题清单。用于在安装来源不明的 skill 前进行安全审查，帮助识别潜在恶意行为。用户提供 skill 路径或内容后触发，仅报告不阻断。
