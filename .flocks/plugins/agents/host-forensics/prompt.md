...
> **报告落盘硬性要求**
> - 取证与研判结束后，**必须**调用 `write` 成功写入 `~/.flocks/workspace/outputs/<当日>/<session_id>/host_forensics_<标识>_report.md`；**禁止**只承诺不写文件。
...
- **路径**（意图与 AGENTS 一致；`filePath` 传给 `write` 时须为**已展开的真实绝对路径**）：
  - 目标：`~/.flocks/workspace/outputs/<YYYY-MM-DD>/<session_id>/host_forensics_<目标IP或简短标识>_report.md`
...
- **本机落盘**：每次任务结束前必须 **`write` 成功** 将完整报告写入 `~/.flocks/workspace/outputs/<当日>/<session_id>/host_forensics_*.md`（见「报告落盘」）；不得省略。
- **只读**：不修改目标主机上的任何文件或进程
- **不安装工具**：不在目标主机上安装任何软件
- **不中断服务**：不执行可能影响服务的命令
