...
describe('apiProxy helpers', () => {
  it('uses the configured proxy target when present', () => {
    expect(getApiProxyTarget({ SMARTCLAW_API_PROXY_TARGET: 'http://127.0.0.1:9000' })).toBe('http://127.0.0.1:9000');
  });

  it('does not rewrite the configured host', () => {
    expect(getApiProxyTarget({ SMARTCLAW_API_PROXY_TARGET: 'http://10.0.0.8:9000' })).toBe('http://10.0.0.8:9000');
  });
