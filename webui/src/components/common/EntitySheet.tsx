 ...
 *
 * 集成「表单」「Titan 对话」「测试」三种交互模式：
 * - 表单模式：传统字段填写，预填已有数据（编辑模式）
 * - Titan 对话模式：与 Titan 自然语言对话，可一键将建议提取并应用到表单
 * - 测试模式：直接向实体（如 Agent）发送消息验证效果
 *
 * 使用方式：
 * 1. 表单内组件可通过 useEntitySheet() 获取 openTitan(msg?) / openTest() 方法
 * 2. 传入 onExtractFromTitan 启用「从 Titan 提取配置」功能
...
interface EntitySheetCtx {
  /** Switch to the Titan tab, optionally sending an initial message */
  openTitan: (prefillMessage?: string) => void;
  /** Switch to the Test tab */
  openTest: () => void;
}

const EntitySheetContext = createContext<EntitySheetCtx>({ openTitan: () => {}, openTest: () => {} });

/** Consume inside form content to get access to openTitan() / openTest() */
...
export interface EntitySheetProps {
  ...
  /** e.g. "Agent", "任务", "工作流" */
  entityType: string;
  /** Displayed in title for edit mode, e.g. "titan" */
  entityName?: string;
  /** Small icon in the header */
  icon?: React.ReactNode;
  /** Form content. Use useEntitySheet() inside to call openTitan(). */
  children: React.ReactNode;
  /** System context injected at session start (noReply) */
  titanSystemContext: string;
  /** Titan's opening message (mockReply) */
  titanWelcomeMessage: string;
  ...
  /**
   * If provided, a "从 Titan 提取配置" button appears in the Titan tab.
   ...
   */
  onExtractFromTitan?: (sessionId: string) => Promise<void>;
  /**
  ...
  /** Hide Titan tab (form-only sheet) */
  hideTitan?: boolean;
  /** Hide Test tab */
  hideTest?: boolean;
  /** Hide form tab (titan-only sheet, e.g. API 创建只能从 AI 编辑开始) */
  hideForm?: boolean;
  /** Initial tab when open (e.g. "form" to show 详情 first when creating) */
  initialTab?: 'form' | 'titan';
...
export default function EntitySheet({
  ...
  const showTabs = !(hideTitan && hideTest);
  ...
  const getDefaultTab = (): Tab => {
    ...
    if (hideForm) return 'titan';
    if (initialTab === 'form' || initialTab === 'titan') return initialTab;
    if (mode === 'create' && !hideTitan) return 'titan';
    return 'form';
  };
  ...
  const [titanInitialMessage, setTitanInitialMessage] = useState<string | null>(null);
  const [drawerWidth, setDrawerWidth] = useState(initialWidth ?? 560);
  const [isDragging, setIsDragging] = useState(false);
  const dragStartX = useRef(0);
  const dragStartWidth = useRef(560);

  // ── Titan session via unified hook ──────────────────────────────────────────
  const {
    sessionId,
    loading: sessionLoading,
    error: sessionError,
    create: createTitanSession,
    createAndSend: createAndSendTitan,
    retry: retryTitanSession,
    reset: resetTitanSession,
  } = useSessionChat({
    title: `${title} — ${t('entity.titanAssist')}`,
    category: 'entity-config',
    contextMessage: titanSystemContext,
    welcomeMessage: titanWelcomeMessage,
  });
  ...
  }, [open, mode, defaultTestPrompt, resetTitanSession, initialWidth, showTabs, hideTitan, hideForm, initialTab]);
  ...
  // ── openTitan (exposed via context) ─────────────────────────────────────────

  const openTitan = useCallback(
    (msg?: string) => {
      setActiveTab('titan');
      if (sessionId && msg) {
        ...
        createAndSendTitan({ text: msg }).catch(() => {});
      }
    },
    [sessionId, createAndSendTitan],
  );
  ...
  const handleExtract = async () => {
    if (!sessionId || !onExtractFromTitan) return;
    setExtracting(true);
    setExtractError(null);
    try {
      await onExtractFromTitan(sessionId);
      setActiveTab('form');
    } catch (err: unknown) {
  ...
  return (
    <EntitySheetContext.Provider value={{ openTitan, openTest }}>
      {/* Backdrop */}
      ...
      >
            ...
            <div className="h-full flex flex-col">
              {testSessionId ? (
                <SessionChat
                  ...
                  hideInput
                  className="flex-1"
                  emptyText={t('entity.titanThinking')}
                />
              ) : (
            ...
            <div className="h-full flex flex-col">
              ...
              {!sessionError && (
                <SessionChat
                  sessionId={sessionId}
                  live={!!sessionId}
                  placeholder={t('entity.titanInputPlaceholder')}
                  className="flex-1"
                  emptyText={t('entity.titanReady')}
                  initialMessage={titanInitialMessage}
                  supportsVision={supportsVision}
                  onEnsureSession={!sessionId ? createTitanSession : undefined}
                  onCreateAndSend={!sessionId ? (text, imageParts) => createAndSendTitan({ text, imageParts }) : undefined}
                  welcomeContent={!sessionId ? (
                    <div className="text-center max-w-md">
                      <MessageSquare className="w-10 h-10 text-red-500 mx-auto mb-3" />
                      <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('entity.titanAssist')}</h3>
                      <p className="text-sm text-gray-500">{t('entity.titanReady')}</p>
                    </div>
                  ) : undefined}
                />
              )}
        ...
        {/* ── Titan tab footer: extract / switch actions only ── */}
        {activeTab === 'titan' && (
          <div className="flex-shrink-0 border-t border-gray-200 bg-white px-6 py-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                {onExtractFromTitan && sessionId ? (
                  ...
                  >
                    ...
                    {extracting ? t('entity.extracting') : t('entity.extractFromTitan')}
              ...
              {/* In create mode, show Done button directly in the Titan tab so the user
