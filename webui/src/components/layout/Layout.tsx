...
function buildUpdateNotification(info: VersionInfo | null, language: string): UserNotification | null {
  ...
  return {
    ...
    kind: 'whats_new',
    title: isZh ? `SmartClaw v${version} 更新内容` : `What's new in SmartClaw v${version}`,
...
export default function Layout() {
  ...
  useEffect(() => {
    window.addEventListener('smartclaw:open-onboarding', handleOpenOnboarding);
    return () => window.removeEventListener('smartclaw:open-onboarding', handleOpenOnboarding);
  ...
  const navigation = [
    {
      name: t('home'),
      items: [
        { name: t('smartclawHome'), href: '/', icon: Home },
      ],
    },
    {
      name: t('aiWorkbench'),
            ...
            {collapsed ? (
              ...
              <>
                <div className="flex items-center flex-1 min-w-0">
                  <span className="text-xl font-bold text-gray-900 whitespace-nowrap">SmartClaw</span>
                </div>
                <button
                  onClick={() => setSidebarOpen(false)}
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-medium text-gray-500 group-hover:text-gray-700 transition-colors">
                        SmartClaw {currentVersion ? `v${currentVersion}` : '...'}
                      </span>
                    </div>
                    <div className="mt-0.5 text-xs text-gray-400">AI Native SecOps Platform</div>
                  </button>
