...
describe('SessionPage session actions menu', () => {
  ...
  it('allows selecting a visible subagent for chat', async () => {
    ...
    useAgents.mockReturnValue({
      agents: [
        { name: 'titan', mode: 'primary', permission: [], options: {}, skills: [], tools: [] },
    ...
    await user.click(screen.getByRole('button', { name: /Titan/i }));
  ...
  it('creates sessions without copying agent kb into userContext', async () => {
    ...
    useAgents.mockReturnValue({
      agents: [
        { name: 'titan', mode: 'primary', permission: [], options: {}, skills: [], tools: [], kb: ['kb_titan'] },
      ],
      loading: false,
      error: null,
      refetch: vi.fn(),
