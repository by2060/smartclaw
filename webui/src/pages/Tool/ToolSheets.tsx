...
const MCP_TITAN_CONTEXT = `你是 MCP 服务接入专家，帮助用户将 MCP（Model Context Protocol）服务接入系统。
...
const MCP_TITAN_WELCOME = `你好！我来帮你接入一个 MCP 服务。
...
export function MCPSheet({ onClose, onSaved, onRefresh }: MCPSheetProps) {
  ...
  return (
    <EntitySheet
      ...
      titanSystemContext={MCP_TITAN_CONTEXT}
      titanWelcomeMessage={MCP_TITAN_WELCOME}
...
const API_TITAN_CONTEXT = `你是 API 工具接入专家，帮助用户将外部 API 接入系统作为工具使用。
...
const API_TITAN_WELCOME = `你好！我来帮你接入一个 API 服务作为工具。
...
export function APISheet({ onClose }: APISheetProps) {
  ...
  return (
    <EntitySheet
      ...
      titanSystemContext={API_TITAN_CONTEXT}
      titanWelcomeMessage={API_TITAN_WELCOME}
      submitDisabled
      submitLabel={t('button.submitToTitan')}
      onClose={onClose}
      onSubmit={handleSubmit}
      hideForm
      initialTab="titan"
...
const GENERATE_TITAN_CONTEXT = `你是工具生成专家，帮助用户通过自然语言描述创建自定义工具。
...
const GENERATE_TITAN_WELCOME = `你好！我来帮你生成一个自定义工具。
...
export function GenerateToolSheet({ onClose }: GenerateToolSheetProps) {
  ...
  return (
    <EntitySheet
      ...
      titanSystemContext={GENERATE_TITAN_CONTEXT}
      titanWelcomeMessage={GENERATE_TITAN_WELCOME}
