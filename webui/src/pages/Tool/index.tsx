...
export default function ToolPage() {
  ...
  const filterOptions = useMemo(() => {
    ...
    tools.forEach((tool) => {
      ...
      sourceNames.add(tool.source_name || 'SmartClaw');
    });
    return {
  ...
  const processedTools = useMemo(() => {
    ...
    if (filters.source_name.size > 0) {
      result = result.filter((tool) => filters.source_name.has(tool.source_name || 'SmartClaw'));
    }
    ...
    result.sort((a, b) => {
      ...
      switch (sort.field) {
        ...
        case 'source':
          ...
          break;
        case 'source_name':
          cmp = (a.source_name || 'SmartClaw').localeCompare(b.source_name || 'SmartClaw', 'zh');
          break;
        case 'enabled':
...
function LegacyAPITabContent({
  ...
  const handleInstallNoAuth = async (entry: MCPCatalogEntry) => {
    ...
    try {
      setInstalling(entry.id);
      // Ensure the entry is in smartclaw.json (idempotent if auto_setup already ran)
      installRes = await mcpAPI.catalogInstall(entry.id);
      shouldConnect = installRes.data?.config?.enabled !== false;
      onConfiguredChange(entry.id);
    } catch (err: any) {
              ...
              return (
                <tr key={tool.name} className="hover:bg-gray-50 cursor-pointer" onClick={() => onSelect(tool)}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-gray-700">{tool.source_name || 'SmartClaw'}</span>
                  </td>
                  {/* tool name */}
                  <td className="px-6 py-4 whitespace-nowrap">
