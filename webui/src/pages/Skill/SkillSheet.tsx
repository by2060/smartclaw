 ...
 * - Titan chat mode (natural language → extract config into form)
 */
...
function buildTitanContext(isEdit: boolean, formData: SkillFormData): string {
  if (!isEdit) {
    return `你是 Skill 创建助手。用户希望通过对话来创建一个新的 Skill。

**Skill 文件规范（必须严格遵守）：**
- 每个 Skill 是一个独立目录，目录名即 skill 名称（kebab-case，如 \`my-skill\`）
- 文件路径：\`~/.smartclaw/plugins/skills/<skill-name>/SKILL.md\`（优先写到用户全局目录）
  - 若需写到项目目录则使用：\`./.smartclaw/plugins/skills/<skill-name>/SKILL.md\`
  - **注意：不要写到 \`.smartclaw/skills/\`（那是内置 skill 目录，不应由用户创建）**
...
3. 用 \`write\` 工具写入文件（路径：\`~/.smartclaw/plugins/skills/<name>/SKILL.md\`）`;
  }
...
function buildTitanWelcome(isEdit: boolean, skillName?: string): string {
  if (!isEdit) {
...
export default function SkillSheet({ skill, onClose, onSaved, onDeleted }: SkillSheetProps) {
  ...
  // user source:    ~/.smartclaw/plugins/skills/baidu-news-fetch  (home dir → ~)
  // project source: .smartclaw/plugins/skills/my-skill            (extract from .smartclaw/ onwards)
  const relativePath = (() => {
    ...
    // For project/smartclaw/claude: extract from .smartclaw/ onwards
    return loc.match(/\.smartclaw\/.+/)?.[0] ?? loc;
  })();
  ...
  return (
    <EntitySheet
      ...
      titanSystemContext={buildTitanContext(isEdit, formData)}
      titanWelcomeMessage={buildTitanWelcome(isEdit, skill?.name)}
