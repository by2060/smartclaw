...
export default function TaskSheet({ task, defaultScheduleKind = 'recurring', onClose, onSaved }: TaskSheetProps) {
  ...
  const [formData, setFormData] = useState<TaskFormData>(() => {
    if (task) {
      ...
      return {
        ...
        agentName: task.agentName ?? 'titan',
  ...
  // ── Titan: extract config from conversation ─────────────────────────────────

  const handleExtractFromTitan = async (sessionId: string) => {
...
\`\`\``;
  ...
  return (
    <EntitySheet
      ...
      icon={icon}
      titanSystemContext={buildTitanContext(formData, isEdit)}
      titanWelcomeMessage={buildTitanWelcome(isEdit, task?.title)}
      ...
      onExtractFromTitan={handleExtractFromTitan}
    >
      <TaskFormContent
        task={task ?? undefined}
        formData={formData}
...
function TaskFormContent({
  ...
  const { openTitan } = useEntitySheet();
      ...
      <div className="space-y-2.5">
        ...
        {formData.executionMode === 'agent' ? (
          <div className="flex items-center gap-3">
            ...
            >
              {agents.length === 0 && (
                <option value={formData.agentName}>{formData.agentName || 'titan'}</option>
              )}
              {agents.map((a) => {
...
// ─── Titan context builders ─────────────────────────────────────────────────────

function buildTitanContext(formData: TaskFormData, isEdit: boolean): string {
  ...
  return [
    ...
    `- 执行模式：${formData.executionMode === 'agent' ? `Agent（${formData.agentName || 'titan'}）` : `Workflow（${formData.workflowID || '未指定'}）`}`,
...
function buildTitanWelcome(isEdit: boolean, taskTitle?: string): string {
  if (isEdit) {
...
告诉我你的需求，配置好后点击「从 Titan 提取配置」填入表单。`;
  }
...
描述清楚后，我帮你整理成任务配置。完成后点击「从 Titan 提取配置」自动填入表单。`;
}
