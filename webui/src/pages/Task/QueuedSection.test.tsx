  ...
  it('workflow 结果支持 JSON format 和复制', async () => {
    ...
    const workflowTask = buildExecution(
      ...
      'completed',
      {
        executionMode: 'workflow',
        workflowID: 'keyword-search-summary',
        resultSummary: "{'result': {'keywords': 'smartclaw agent', 'search_success': True, 'result_count': 8}}",
        executionInputSnapshot: {
          context: {
            keywords: 'smartclaw agent',
          },
        },
      },
    );
    ...
    expect(mocks.copyText).toHaveBeenCalledWith(expect.stringContaining('"keywords": "smartclaw agent"'));
