...
\`\`\``;
  ...
  return (
    <EntitySheet
      ...
      titanWelcomeMessage={buildTitanWelcome(isEdit, agent?.name)}
      ...
      onExtractFromTitan={isEdit ? handleExtractFromTitan : undefined}
...
function AgentFormContent({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* System Prompt with Titan assist */}
      <div>
...
function buildTitanContext(formData: AgentFormData, isEdit: boolean): string {
  if (!isEdit) {
    return `你是 Agent 创建助手。用户希望通过对话来创建一个新的子 Agent。

请使用 agent-builder skill 根据用户的需求生成子 Agent 配置文件（YAML + prompt 文件），保存到 ~/.smartclaw/plugins/agents/ 目录。
...
- mode 固定为 subagent
- 文件必须写入 ~/.smartclaw/plugins/agents/
- 不要与内置 Agent 名称冲突
...
请先引导用户描述需求，如果信息不够清晰可适当追问，然后一次性生成所有文件。`;
  ...
  return [
    ...
    `配置完成后，用户可以点击「从 Titan 提取配置」按钮，将配置自动填入表单。`,
    `届时你会被要求以 JSON 格式输出配置摘要，请确保 JSON 格式正确。`,
  ].join('\n');
}

function buildTitanWelcome(isEdit: boolean, agentName?: string): string {
  if (isEdit) {
...
描述你的需求，我来帮你完善配置。配置好后，点击底部「从 Titan 提取配置」即可自动填入表单。`;
  }
