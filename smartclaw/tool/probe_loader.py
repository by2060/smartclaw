...
The leading underscore makes :mod:`smartclaw.plugin.loader` skip the file, so a
...
See ``.smartclaw/plugins/tools/api/_TEST_YAML_DESIGN.md`` for the full schema.
"""
...
class ConnectivitySpec:
    ...
    The probe passes iff :attr:`smartclaw.tool.registry.ToolResult.success` is
...
def clear_cache() -> None:
    ...
    Does not touch :mod:`smartclaw.config.api_versioning`'s descriptor cache;
...
def get_tool_fixtures_by_tool_name(tool_name: str) -> list[Fixture]:
    ...
    1. Look up the tool in :class:`~smartclaw.tool.registry.ToolRegistry`. If
