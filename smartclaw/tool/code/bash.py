...
from smartclaw.workspace import WorkspaceManager

if TYPE_CHECKING:
    from smartclaw.sandbox.types import BashSandboxConfig

from smartclaw.tool.registry import ToolRegistry, ToolCategory, ToolParameter, ParameterType, ToolResult, ToolContext
...
When Python is executed via Bash, SMARTCLAW_OUTPUTS_DIR already points to the final session output directory. Use it directly and do not append YYYY-MM-DD or session_id again.
...
def _bash_output_env(ctx: ToolContext) -> dict[str, str]:
    ...
    return {
        "SMARTCLAW_WORKSPACE_DIR": str(manager.get_user_workspace_dir()),
        "SMARTCLAW_OUTPUTS_DIR": str(output_dir),
        "SMARTCLAW_ARTIFACTS_DIR": str(output_dir / "artifacts"),
    }
...
def _migrate_misrouted_project_workspace_outputs(
    ...
    manager = WorkspaceManager.get_instance()
    source_root = Path(source_dir).expanduser() / ".smartclaw" / "workspace" / "outputs"
...
def _migrate_nested_session_outputs(ctx: ToolContext) -> list[dict[str, str]]:
    """Move files back when a script appends date/session to SMARTCLAW_OUTPUTS_DIR."""
    from smartclaw.workspace.manager import WorkspaceManager
...
def _is_smartclaw_plugin_write_path(path: str, base_dir: str) -> bool:
    """Return True when a shell write targets a project SmartClaw plugin definition."""
    normalized = str(path).replace("\\", "/").strip("'\"")
    if normalized.startswith((".smartclaw/plugins/", "./.smartclaw/plugins/")):
        return True
    ...
    roots = [Path(base_dir).expanduser() / ".smartclaw" / "plugins", Path.cwd() / ".smartclaw" / "plugins"]
    for root in roots:
        try:
...
def _is_user_smartclaw_plugin_write_path(path: str) -> bool:
    """Return True when a shell write targets ~/.smartclaw/plugins."""
    candidate = Path(path).expanduser()
    if not candidate.is_absolute():
        return False
    root = Path.home() / ".smartclaw" / "plugins"
    try:
...
def _sandbox_project_plugins_host_root(sandbox: "BashSandboxConfig") -> Optional[Path]:
    ...
    if sandbox.agent_workspace_dir:
        return Path(sandbox.agent_workspace_dir).expanduser() / ".smartclaw" / "plugins"
    return None
...
def _sandbox_project_plugins_container_root(sandbox: "BashSandboxConfig") -> str:
    ...
    return f"{container_root}/.smartclaw/plugins"
...
async def bash_tool(
    ...
    if (
        blocked_script_path
        and not _is_smartclaw_plugin_write_path(blocked_script_path, cwd)
    ...
    if blocked_output_path and _is_user_smartclaw_plugin_write_path(blocked_output_path):
        expected_plugin_dir = Path(cwd) / ".smartclaw" / "plugins"
        return ToolResult(
            success=False,
            error=(
                "SmartClaw plugin definitions must be written to the project-level "
                f"plugin directory, not ~/.smartclaw/plugins. Detected attempted path: {blocked_output_path}. "
    ...
    if blocked_output_path and not _is_smartclaw_plugin_write_path(blocked_output_path, cwd):
        from smartclaw.workspace.manager import WorkspaceManager
...
async def _execute_sandboxed(
    ...
    """
    import uuid
    from smartclaw.sandbox.docker import build_docker_exec_args, build_sandbox_env, kill_sandbox_exec
