...
Compatible with SmartClaw's TypeScript Tool system.
"""
...
class ToolInfo(BaseModel):
    ...
    native: bool = Field(False, description=(
        ...
        "plugin tools (<cwd>/.smartclaw/plugins/tools/). False for user-level plugin tools "
        "(~/.smartclaw/plugins/tools/). Determined by loading context, not declared in YAML."
    ))
...
class ToolContext:
    ...
    Compatible with SmartClaw's Tool.Context interface.
    """
    ...
    async def execute(self, ctx: ToolContext, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if result.success and not result.truncated:
                from smartclaw.tool.truncation import truncate_output
...
class ToolRegistry:
    ...
    Compatible with SmartClaw's Tool Registry pattern.
    """
    ...
    def register(cls, tool: Tool) -> None:
        """Register a tool"""
        try:
            from smartclaw.tool.catalog import apply_tool_catalog_defaults
        ...
        **kwargs
        ...
        try:
            from smartclaw.agent.controls import (
                agent_allowed_tools,
                agent_allows_tool,
                titan_session_uses_full_tool_catalog,
            )
            from smartclaw.tool.catalog import get_always_load_tool_names

            use_full_catalog = await titan_session_uses_full_tool_catalog(
                ctx.session_id,
                ctx.agent,
    ...
    def _load_plugin_tools(cls) -> None:
        ...
        Scans both:
        - ``~/.smartclaw/plugins/tools/`` (user-level)
        - ``<cwd>/.smartclaw/plugins/tools/`` (project-level)
        ...
        try:
            from smartclaw.plugin import PluginLoader
    ...
    def _bootstrap_user_api_services(cls) -> None:
        """Auto-enable newly loaded user-level API tools in smartclaw.json.
        ...
        Ensures the corresponding api_services entry in smartclaw.json has
    ...
    def _sync_api_service_states(cls) -> None:
        """Disable tools whose API service is disabled in smartclaw.json.
    ...
    def _apply_tool_settings(cls) -> None:
        """Apply user-level ``tool_settings`` from smartclaw.json on top of YAML defaults.

        The YAML file is treated as the factory default; the overlay in
        ``smartclaw.json`` (``tool_settings[<tool_name>]``) is the user's
        ...
        for name, entry in settings.items():
            ...
            if desired and tool.info.provider:
                ...
                if not svc.get("enabled", False):
                    ...
                    # The overlay entry stays in smartclaw.json so that re-enabling
    ...
    def _register_plugin_extension_point(cls) -> None:
        ...
        """
        from smartclaw.plugin import ExtensionPoint, PluginLoader
        from smartclaw.tool.tool_loader import yaml_to_tool

        # User-level plugin root: ~/.smartclaw/plugins/
        _user_plugin_root = Path.home() / ".smartclaw" / "plugins"
        ...
        def _is_native_source(source: str) -> bool:
            ...
            try:
                ...
                return False  # Under ~/.smartclaw/plugins/ → user-level → not native
            except ValueError:
                return True   # Under <cwd>/.smartclaw/plugins/ or elsewhere → project-level → native
    ...
    def _register_builtin_tools(cls) -> None:
        ...
        _tool_groups = [
            ...
            ("smartclaw.tool.file", ["read", "write", "edit", "multiedit", "apply_patch", "glob", "list_tool", "file_search", "doc_parser"]),
            # code/ — code analysis + terminal
            ("smartclaw.tool.code", ["bash", "grep", "codesearch", "lsp_tool"]),
            # web/ — internet access
            ("smartclaw.tool.web", ["webfetch", "websearch"]),
            # agent/ — agent delegation/coordination
            ("smartclaw.tool.agent", ["delegate_task", "call_omo_agent"]),
            # task/ — task/workflow
            ("smartclaw.tool.task", ["task", "task_center", "todo", "plan", "run_workflow", "run_workflow_node"]),
            # security/ — SSH forensics + threat intelligence (optional: asyncssh)
            ("smartclaw.tool.security", ["ssh_host_cmd", "ssh_run_script"]),
            # system/ — background tasks, questions, model config, memory, skill, batch, session management, slash commands
            ("smartclaw.tool.system", ["background_output", "background_cancel", "question", "model_config", "memory", "skill", "batch", "session_manage", "slash_command", "tool_search", "dify_kb_search"]),
            # skill/ — skill management (search, install, status, deps, remove)
            ("smartclaw.tool.skill", ["smartclaw_skills"]),
            # channel/ — IM platform messaging
            ("smartclaw.tool.channel", ["channel_message"]),
            # wecom/ — 企业微信 MCP（文档、智能表格）
            ("smartclaw.tool.wecom", ["wecom_mcp"]),
        ]
    ...
    def _generated_tools_dirs(cls) -> List[Path]:
        ...
        Checks (in order):
        1. ``~/.smartclaw/plugins/tools/generated/``  (external plugin path)
        2. ``smartclaw/tool/security/``  (built-in threat intelligence tools)
        """
    ...
    def _discover_dynamic_modules(cls) -> Dict[str, Path]:
        ...
        for gen_dir in cls._generated_tools_dirs():
            for path in gen_dir.glob("*.py"):
                ...
                module_name = f"_smartclaw_gen_{gen_dir.parent.name}_{path.stem}"
                if "security" in str(gen_dir) and "plugins" not in str(gen_dir):
                    module_name = f"smartclaw.tool.security.{path.stem}"
...
class ToolFileWatcher:
    ...
    def _collect_watch_dirs(self) -> Set[str]:
        ...
        try:
            ...
            tools_root = Path.home() / ".smartclaw" / "plugins" / "tools"
        ...
        try:
            project_tools_root = Path.cwd() / ".smartclaw" / "plugins" / "tools"
            for subdir in self._WATCH_SUBDIRS:
