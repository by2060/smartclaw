...
def _get_api_token() -> str | None:
    ...
    Reuses ``API_TOKEN_SECRET_ID`` from ``smartclaw.server.auth`` so that the
...
async def _http_session_send(
    ...
    """Send a message via the running smartclaw server's /api/channel/session-send endpoint,
...
async def channel_message(ctx: ToolContext, **kwargs) -> ToolResult:
    ...
    # Prefer the HTTP endpoint of the running smartclaw server to reuse its WS connection.
    try:
