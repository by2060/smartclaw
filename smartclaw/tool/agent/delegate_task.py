"""
delegate_task tool - category or subagent-based delegation (Oh-My-SmartClaw parity).
"""
...
from smartclaw.tool.delegate_task_constants import (
    DEFAULT_CATEGORIES,
    CATEGORY_PROMPT_APPENDS,
    CATEGORY_DESCRIPTIONS,
)
from smartclaw.task.background import get_background_manager, LaunchInput, ResumeInput
...
# 使用轻量级元数据查询，避免循环依赖
from smartclaw.agent.registry import Agent, is_delegatable
from smartclaw.agent.controls import (
    agent_allowed_skills,
    agent_allows_skill,
    titan_session_allowed_subagents_text,
...
from smartclaw.tool.subagent_result import format_sync_subagent_result
...
async def _find_completed_delegate(
    ...
    try:
        from smartclaw.session.message import ToolPart
...
async def delegate_task_tool(
    ...
    # apart from explicit registry aliases so typos cannot fall through to Titan.
    ...
    if subagent_type:
        ...
        if not target_agent.delegatable or not is_delegatable(target_agent.name):
            if target_agent.name in {"sisyphus-junior", "titan-junior"}:
                return ToolResult(
                    success=False,
    ...
    # skills权限控制新增
    full_skill_catalog = await titan_session_uses_full_skill_catalog(
        ctx.session_id,
        ctx.agent,
    ...
    if session_id:
        ...
        if target_agent != ctx.agent and not await titan_session_allows_subagent(ctx.session_id, ctx.agent, target_agent):
            allowed_text = await titan_session_allowed_subagents_text(ctx.session_id, ctx.agent)
            return ToolResult(
                success=False,
                error=(
    ...
    # subagents权限控制新增
    if agent_to_use and not await titan_session_allows_subagent(
        ctx.session_id,
        ctx.agent,
        agent_to_use,
        getattr(ctx, "extra", None),
    ...
    from smartclaw.session.features.activity_forwarder import ActivityForwarder
