...
# Default ignore patterns matching SmartClaw
IGNORE_PATTERNS = [
    "node_modules/",
    "__pycache__/",
    ".git/",
...
# Description matching SmartClaw' ls.txt
...
DESCRIPTION_CN = """列出给定路径中的文件和目录。path 参数必须是绝对路径；省略时使用当前工作区目录。可以通过 ignore 参数提供要忽略的 glob 模式数组。如果已经知道要搜索的目录，通常应优先使用 Glob 和 Grep 工具。"""
