...
from smartclaw.tool.file.sandbox_paths import (
    is_project_plugin_path,
    is_session_output_path,
    is_upload_read_only,
    resolve_sandbox_path,
)
from smartclaw.workflow.artifact_service import (
    WorkflowArtifactError,
    WorkflowArtifactFiles,
    write_workflow_artifact,
)
from smartclaw.workflow.skill_guard import is_workflow_artifact_path, require_workflow_builder_for_artifact
...
def _is_user_workspace_output_path(filepath: str, session_id: Optional[str]) -> bool:
    """Return True for the canonical ~/.smartclaw/workspace/outputs session path."""
...
def _project_plugins_root(base_dir: str) -> Path:
    return Path(base_dir).expanduser() / ".smartclaw" / "plugins"


def _rewrite_user_plugin_path_to_project(filepath: str, base_dir: str) -> tuple[str, Optional[str]]:
    """Route ~/.smartclaw/plugins writes to the project plugin directory."""
    path = Path(filepath).expanduser()
    if not path.is_absolute():
        return filepath, None
...
def _is_smartclaw_plugin_path(filepath: str, base_dir: str) -> bool:
...
def _is_sandbox_container_plugin_path(filepath: str, sandbox: Optional[dict]) -> bool:
    ...
    if rel is None:
        return False
    parts = rel.split("/")
    return len(parts) >= 3 and parts[0] == ".smartclaw" and parts[1] == "plugins"


def _is_smartclaw_plugin_or_container_path(
    filepath: str,
    base_dir: str,
    sandbox: Optional[dict],
) -> bool:
    return _is_smartclaw_plugin_path(filepath, base_dir) or _is_sandbox_container_plugin_path(
        filepath,
        sandbox,
    )
...
def _rewrite_workflow_json_path(
    ...
    if Path(filepath).name != "workflow.json":
        return filepath, None
    if _is_smartclaw_plugin_or_container_path(filepath, base_dir, sandbox):
        return filepath, None
...
def _map_sandbox_container_path_to_host(
    ...
    if len(parts) >= 2 and parts[0] == ".smartclaw" and parts[1] == "plugins":
...
def _workflow_workspace_from_artifact_path(path: Path) -> Optional[Path]:
    if (
        ...
        and path.parent.parent.parent.parent.name == ".smartclaw"
    ):
...
async def write_tool(
    ...
    is_smartclaw_plugin_path = _is_smartclaw_plugin_or_container_path(filepath, base_dir, sandbox)
    if rewritten_from is None and is_smartclaw_plugin_path:
        ...
        is_smartclaw_plugin_path = _is_smartclaw_plugin_or_container_path(filepath, base_dir, sandbox)
    if rewritten_from is None and not is_user_workspace_output and not is_smartclaw_plugin_path:
        filepath, container_mapped_from = _map_sandbox_container_path_to_host(filepath, sandbox)
        if container_mapped_from is not None:
            rewritten_from = container_mapped_from
