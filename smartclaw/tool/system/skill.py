...
from smartclaw.workflow.skill_guard import mark_workflow_builder_loaded
...
#
# Why 500?
#   Empirically, the descriptions in `smartclaw/.smartclaw/plugins/skills/*/SKILL.md`
...
def build_description(skills: List[SkillInfo]) -> str:
    ...
    # Match SmartClaw's format: space-separated, no newlines
    parts = [
    ...
    # Join with space like SmartClaw does: .join(" ")
    return " ".join(parts)
...
async def list_skills_for_agent_context(
    *,
    ...
    if await titan_session_uses_full_skill_catalog(session_id, agent_name, extra):
        return skill_list
...
async def skill_tool_impl(
    ...
    if not await titan_session_uses_full_skill_catalog(
        ctx.session_id,
        ctx.agent,
