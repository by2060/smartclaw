"""
Session management tools — 查询、创建、更新、删除 SmartClaw Session 的元数据。
...
@ToolRegistry.register_function(
    name="session_create",
    description="创建一个新的 SmartClaw Session。",
    description_cn="创建一个新的 SmartClaw Session。",
    category=ToolCategory.SYSTEM,
    parameters=[
        ToolParameter(
            name="title",
        ...
        ToolParameter(
            ...
            type=ParameterType.STRING,
            required=False,
            description="指定 agent 类型，如 hephaestus、titan、build、plan 等",
        ),
        ToolParameter(
            name="parent_id",
            type=ParameterType.STRING,
