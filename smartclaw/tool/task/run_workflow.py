"""
Run Workflow Tool - Execute workflows using smartclaw-workflow runtime
...
# Lazy import to avoid circular import (smartclaw.tool <-> smartclaw.workflow)
...
def _get_workflow_runtime():
    ...
    try:
        # Prefer in-repo integration (smartclaw.workflow). Fallback to external package if installed.
        from smartclaw.workflow import RequirementsInstaller as _ReqInstaller, run_workflow as _run
        from smartclaw.workflow.runner import RunWorkflowResult as _Result
        ...
        try:
            from smartclaw_workflow import (  # pyright: ignore[reportMissingImports]
...
_BASE_DESCRIPTION = """Execute a workflow definition using the smartclaw-workflow runtime.
...
_BASE_DESCRIPTION_CN = """使用 smartclaw-workflow 运行时执行工作流定义。
...
async def run_workflow_tool(
    ...
    """
    Execute a workflow using smartclaw-workflow runtime
    ...
    if _run_workflow_fn is None or RunWorkflowResultCls is None:
        return ToolResult(
            success=False,
            error="smartclaw-workflow package is not available. Please check code"
        )
