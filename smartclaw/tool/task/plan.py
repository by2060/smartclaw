...
from smartclaw.tool.system.question import set_question_handler, QuestionRejectedError
...
async def _ask_user(ctx: ToolContext, question: str, header: str, options: list) -> str:
    ...
    """
    from smartclaw.tool.system.question import _question_handler, default_question_handler, _current_message_id
...
async def plan_exit_tool(
    ...
    return ToolResult(
        success=True,
        output="User approved switching to titan agent. Wait for further instructions.",
        title="Switching to titan agent",
        metadata={}
    )
