...
Model resolution priority for child sessions:
  ...
  2. Agent-specific model from AgentInfo.model (set in smartclaw.json agent config)
...
from smartclaw.agent.registry import is_delegatable
...
async def task_tool(
    ...
    if not await titan_session_allows_subagent(
        ctx.session_id,
        ctx.agent,
        normalized,
    ...
    if session_id:
        ...
        from smartclaw.session.session_loop import LoopCallbacks as _LoopCbs
