...
in :mod:`smartclaw.plugin.loader`.
"""
...
from smartclaw.tool.auth_crypto import decrypt_sm4_value as _decrypt_sm4_value
...
from smartclaw.tool.smart_auth import SmartAuthBinding, SmartAuthError, build_smart_auth_binding
...
def _project_tools_root() -> Path:
    ...
    return Path(project_dir) / ".smartclaw" / "plugins" / "tools"
...
def _resolve_secrets(value: str) -> str:
    ...
    def _replacer(match: re.Match) -> str:
        ...
        try:
            from smartclaw.security import get_secret_manager, resolve_secret_value
...
def _build_script_handler(cfg: dict, yaml_path: Path) -> ToolHandler:
    ...
    project_plugins_root = (Path.cwd() / ".smartclaw" / "plugins").resolve()
    ...
    spec = importlib.util.spec_from_file_location(
        f"_smartclaw_tool_handler_{script_path.stem}",
        str(script_path),
    )
...
def yaml_to_tool(raw: dict, yaml_path: Path) -> Tool:
    ...
    # ``info.provider`` is the lookup key in ``smartclaw.json`` ``api_services``.
    ...
    # (see :mod:`smartclaw.config.api_versioning`). Without a version
    # the storage key collapses back to ``service_id`` for full back-compat.
    if service_id and provider_version:
        from smartclaw.config.api_versioning import derive_storage_key
...
def _yaml_tool_search_roots() -> List[Path]:
    ...
    Bundled smartclawhub directories are intentionally NOT included — bundled
    ...
    '''roots = [
        _TOOLS_SUBDIR,
        Path.cwd() / ".smartclaw" / "plugins" / "tools",
    ]'''
    # 新增
    roots = [_project_tools_root(), _TOOLS_SUBDIR, Path.cwd() / ".smartclaw" / "plugins" / "tools"]
    result: List[Path] = []
    seen: set[str] = set()
    for root in roots:
...
def create_yaml_tool(
    ...
    The resulting path is::
        ...
        <project>/.smartclaw/plugins/tools/{tool_type}/{provider?}/{name}.yaml
...
def _python_tool_dirs() -> List[Path]:
    ...
    Bundled smartclawhub directories are intentionally excluded; python
    ...
    '''dirs = [
        ...
        Path.cwd() / ".smartclaw" / "plugins" / "tools" / TOOL_TYPE_PYTHON,
    ]'''
    # 新增
...
def _infer_tool_type(yaml_path: Path) -> str:
    ...
    Checks both the user-level tools dir (~/.smartclaw/plugins/tools/) and
    the project-level tools dir (<cwd>/.smartclaw/plugins/tools/).
    """
    # 插件输出和执行结果输出修改
    # 删除
    # candidates = [_TOOLS_SUBDIR, Path.cwd() / ".smartclaw" / "plugins" / "tools"]
...
def list_yaml_tools() -> List[Dict[str, Any]]:
    ...
    Searches both user-level (~/.smartclaw/plugins/tools/) and project-level
    (<cwd>/.smartclaw/plugins/tools/) directories, as well as legacy flat/provider paths.
