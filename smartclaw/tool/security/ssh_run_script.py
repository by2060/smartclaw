...
Typical usage:
  ssh_run_script(host="10.0.0.1", script_path=".smartclaw/plugins/agents/host-forensics/scripts/triage.sh")
  ssh_run_script(host="10.0.0.1", script_path=".smartclaw/plugins/agents/host-forensics/scripts/deep_scan.sh", timeout=300)
"""
...
from smartclaw.tool.security.ssh_utils import audit_log, execute_ssh_command, resolve_ssh_credentials
...
@ToolRegistry.register_function(
    ...
    description=(
        ...
        "(e.g. .smartclaw/plugins/agents/host-forensics/scripts/triage.sh). "
    ...
    description_cn=(
        ...
        "适用场景：执行 skill 或插件 agent 定义的取证调查脚本，例如 .smartclaw/plugins/agents/host-forensics/scripts/triage.sh。"
        "脚本会作为单个 SSH 会话运行，适合批量数据收集。\n\n"
        "输出：结构化文本，使用 ### SECTION_NAME ### 标记不同类别的数据。"
        "如果脚本中检测到危险操作（rm、chmod、写重定向等），执行会被阻止并报告违规项。\n\n"
        "脚本路径：优先使用相对于当前工作目录（workspace root）的路径，例如 '.smartclaw/plugins/agents/host-forensics/scripts/triage.sh'。"
        "也接受绝对路径和 '~' home 目录路径。"
    ),
    ...
    parameters=[
        ...
        ToolParameter(
            ...
            description=(
                ...
                "Example: .smartclaw/plugins/agents/host-forensics/scripts/triage.sh"
            ),
            required=True,
        ),
        ToolParameter(
    ...
    if not resolved_path.exists():
        return ToolResult(
            ...
            error=(
                ...
                ".smartclaw/plugins/agents/host-forensics/scripts/triage.sh"
            ),
        )
