"""Logging configuration for smartclaw.workflow.

Workflow logs go to stderr and, when file logging is enabled, to
~/.smartclaw/logs/workflow.log (or SMARTCLAW_LOG_DIR/workflow.log).
"""
...
from smartclaw.utils.log import get_log_dir
...
def setup_workflow_logging(
    ...
    """配置 smartclaw.workflow 的日志输出（控制台 + 可选文件）。

    Args:
        level: 日志级别，默认为 INFO
        format_string: 日志格式字符串，如果为 None 则使用默认格式
        stream: 输出流，默认为 sys.stderr
        file: 是否同时写入 ~/.smartclaw/logs/workflow.log（与主 Log 同目录）

    Example:
        >>> from smartclaw.workflow import setup_workflow_logging
    ...
    logger = logging.getLogger("smartclaw.workflow")
    ...
    # File (same directory as smartclaw.utils.log)
    if file:
        try:
            log_dir = get_log_dir()
