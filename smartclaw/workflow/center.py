...
Service driver selection (env var SMARTCLAW_WORKFLOW_SERVICE_DRIVER):
...
from smartclaw.sandbox.docker import docker_container_state, exec_docker
...
GLOBAL_WORKFLOW_ROOT: Path = Path.home() / ".smartclaw" / "workflow"


def _resolve_global_workflow_root() -> Path:
    """~/.smartclaw/plugins/workflows/ — canonical global (user-level) workflow storage."""
    return Path.home() / ".smartclaw" / "plugins" / "workflows"
...
def resolve_project_workflow_roots(base_dir: Optional[Path] = None) -> list[Path]:
    ...
    smartclaw = root / ".smartclaw"
    return [
        smartclaw / "plugins" / "workflow",   # legacy compat (read-only)
        smartclaw / "workflow",               # legacy compat (read-only)
        smartclaw / "plugins" / "workflows",  # new canonical (read + write)
    ]


def _resolve_project_workflow_root(base_dir: Optional[Path] = None) -> Path:
    """<cwd>/.smartclaw/plugins/workflows/ — canonical project-level workflow storage."""
    root = base_dir or Path.cwd()
    return root / ".smartclaw" / "plugins" / "workflows"
...
async def _allocate_port() -> int:
    start = int(os.getenv("SMARTCLAW_WORKFLOW_SERVICE_PORT_START", str(_DEFAULT_PORT_START)))
    end = int(os.getenv("SMARTCLAW_WORKFLOW_SERVICE_PORT_END", str(_DEFAULT_PORT_END)))
    if start > end:
...
async def _scan_workflow_dir(
    ...
    for workflow_path in sorted(workflow_root.glob("*/workflow.json")):
        ...
        entry = {
            ...
            # global (~/.smartclaw/plugins/workflows/) are user-level customizations.
...
async def scan_skill_workflows(base_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    resolve_project_workflow_roots:
      1. ~/.smartclaw/plugins/workflow/        (global legacy, sourceType="global")
      2. ~/.smartclaw/workflow/                (global compat, sourceType="global")
      3. ~/.smartclaw/plugins/workflows/       (global canonical, sourceType="global")
      4. <cwd>/.smartclaw/plugins/workflow/    (project legacy, sourceType="project")
      5. <cwd>/.smartclaw/workflow/            (project compat, sourceType="project")
      6. <cwd>/.smartclaw/plugins/workflows/   (project canonical, sourceType="project")
...
def _workflow_container_name(workflow_id: str, release_id: str) -> str:
    return f"smartclaw-wf-{workflow_id[:8]}-{release_id[:8]}"
...
async def publish_workflow_local(workflow_id: str) -> Dict[str, Any]:
    ...
    proc = await asyncio.create_subprocess_exec(
        sys.executable,
        "-m", "smartclaw.workflow.service_runtime",
    ...
    health_retries = int(os.getenv("SMARTCLAW_WORKFLOW_SERVICE_HEALTH_RETRIES", str(_DEFAULT_HEALTH_RETRIES)))
    health_interval_s = float(os.getenv("SMARTCLAW_WORKFLOW_SERVICE_HEALTH_INTERVAL_S", str(_DEFAULT_HEALTH_INTERVAL_S)))
...
def _service_driver() -> str:
    return os.getenv("SMARTCLAW_WORKFLOW_SERVICE_DRIVER", _DEFAULT_SERVICE_DRIVER).lower()
...
async def _publish_workflow_docker(workflow_id: str, image: Optional[str] = None) -> Dict[str, Any]:
    ...
    image_name = image or os.getenv("SMARTCLAW_WORKFLOW_SERVICE_IMAGE", _DEFAULT_IMAGE)
    runtime_install = os.getenv("SMARTCLAW_WORKFLOW_SERVICE_RUNTIME_INSTALL", "1").strip().lower() not in {
        "0",
        "false",
        "no",
    }
    health_interval_s = float(
        os.getenv("SMARTCLAW_WORKFLOW_SERVICE_HEALTH_INTERVAL_S", str(_DEFAULT_HEALTH_INTERVAL_S))
    )
    default_retries = (
        _DEFAULT_RUNTIME_INSTALL_HEALTH_RETRIES if runtime_install else _DEFAULT_HEALTH_RETRIES
    )
    health_retries = int(
        os.getenv("SMARTCLAW_WORKFLOW_SERVICE_HEALTH_RETRIES", str(default_retries))
    )
    ...
    cmd = [
        ...
        "PYTHONPATH=/app",
        "-e",
        "SMARTCLAW_CONFIG_DIR=/runtime/.smartclaw-config",
        "-e",
        "SMARTCLAW_CONFIG=/runtime/.smartclaw-config/smartclaw.json",
        image_name,
    ]
    if user_config_dir.exists():
        cmd[cmd.index(image_name):cmd.index(image_name)] = [
            "-v",
            f"{user_config_dir}:/runtime/.smartclaw-config:ro",
        ]
    ...
    if runtime_install:
        ...
        service_cmd = (
            f"{install_cmd} && "
            "python -m smartclaw.workflow.service_runtime "
        ...
        cmd.extend(
            [
                "python",
                "-m",
                "smartclaw.workflow.service_runtime",
                "--workflow",
