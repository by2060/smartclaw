...
from smartclaw.provider.provider import ChatMessage, Provider, ProviderConfig
...
def _run_coro_sync(coro):
    ...
    outlive their owning loop. See ``smartclaw.workflow._async_runtime``.
    """
...
class LLMClient:
    """Minimal workflow LLM client backed by smartclaw Provider."""
    ...
    def __init__(
        ...
        *,
        ...
        # but uses smartclaw Provider (async) internally for consistency with the rest of smartclaw.
        # Configuration should come from smartclaw Provider + Config (Provider.apply_config).
    ...
    def _get_provider(self, provider_id: str) -> Any:
        ...
        if provider is None:
            raise ValueError(
                ...
                "Set provider config in ~/.smartclaw/config/smartclaw.json."
            )
        return provider
