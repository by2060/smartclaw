...
def _default_cache_dir() -> Path:
    raw = os.getenv("SMARTCLAW_WORKFLOW_REQUIREMENTS_CACHE_DIR")
    if raw:
        return Path(raw).expanduser().resolve()
    return (Path.home() / ".cache" / "smartclaw-workflow" / "requirements").resolve()
...
class SandboxRequirementsInstaller:
    ...
    marker_root: str = f"{os.getcwd()}/.smartclaw/workflow/requirements"
    site_packages_dir: str = f"{os.getcwd()}/.smartclaw/workflow/site-packages"
