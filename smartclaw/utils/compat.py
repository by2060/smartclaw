"""
SmartClaw configuration directory helpers.
...
def _get_cli_config_dir() -> Path:
    env_dir = os.environ.get("SMARTCLAW_CONFIG_DIR", "").strip()
    if env_dir:
    ...
    if system == "windows":
        ...
        cross_config = cross_dir / "smartclaw.json"
        if cross_config.exists():
            return cross_dir

        app_data = os.environ.get("APPDATA") or (home / "AppData" / "Roaming")
        app_dir = Path(app_data) / "opencode"
        app_config = app_dir / "smartclaw.json"
...
def get_smartclaw_config_dir(binary: str = "opencode", version: str | None = None, check_existing: bool = True) -> Path:
    """
    Resolve SmartClaw config dir for CLI or desktop.
    """
    if binary == "opencode":
    ...
    if check_existing:
        ...
        legacy_config = legacy_dir / "smartclaw.json"
        legacy_configc = legacy_dir / "smartclaw.jsonc"
