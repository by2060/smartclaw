...
Provides structured logging exactly matching SmartClaw' TypeScript Log namespace.
This ensures complete compatibility between Python and TypeScript services.
All logs can be written to ~/.smartclaw/logs (or SMARTCLAW_LOG_DIR); init is required
...
def _log_dir() -> Path:
    """Log directory: SMARTCLAW_LOG_DIR, or SMARTCLAW_ROOT/logs, or ~/.smartclaw/logs. Matches config."""
    raw = os.getenv("SMARTCLAW_LOG_DIR")
    if raw:
        return Path(raw)
    root = os.getenv("SMARTCLAW_ROOT")
    if root:
        return Path(root) / "logs"
    return Path.home() / ".smartclaw" / "logs"
...
class Log:
    ...
    Exactly matches SmartClaw's TypeScript Log namespace.
    """
    ...
    async def init(
        ...
        # Setup log directory (SMARTCLAW_LOG_DIR or ~/.smartclaw/logs)
        log_dir = _log_dir()
        log_dir.mkdir(parents=True, exist_ok=True)
    ...
    def file(cls) -> str:
        ...
        return str(_log_dir() / "smartclaw.log")
