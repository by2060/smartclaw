...
Blocks arbitrary file read without changing :meth:`smartclaw.utils.file.File.read`
used by internal callers (agent tools, memory, etc.).

- Used only from ``smartclaw.server.routes.file``.
- Reads ``allowReadPaths`` via :meth:`smartclaw.config.config.Config.get` (Pydantic alias).
- When no ``.smartclaw`` project root exists, does not fall back to cwd as a sandbox root.
"""
...
from smartclaw.utils.paths import find_smartclaw_project_root
...
def _initial_abs_path(user_path: str, project_root: Optional[Path]) -> str:
    ...
    if project_root is None:
        raise PermissionError("Relative paths require a SmartClaw project root (.smartclaw/)")
...
async def resolve_path_for_http_file_access(
    ...
    """
    project_root = find_smartclaw_project_root()
