...
Extension point:
  Built-in agents:         smartclaw/agent/agents/<name>/          native=True
  Project plugin agents:   <cwd>/.smartclaw/plugins/agents/<name>/ native=True
  User plugin agents:      ~/.smartclaw/plugins/agents/<name>/     native=False
...
from smartclaw.agent.agent import AgentInfo, AgentModel, AgentPromptMetadata, DelegationTrigger
from smartclaw.agent.controls import filter_agent_skills, filter_agent_subagents, filter_agent_tools
from smartclaw.agent.toolset import resolve_agent_initial_tools
...
try:
    ...
    _PLUGIN_AGENTS_DIR = Path.home() / ".smartclaw" / "plugins" / "agents"
...
def load_agent(agent_dir: Path, native: bool = False) -> Optional[AgentInfo]:
    ...
    if prompt_md.is_file():
        ...
        # Derive Python module path from file location, relative to smartclaw package root
        try:
...
def scan_and_load(dirs: Optional[List[Path]] = None) -> Dict[str, AgentInfo]:
    ...
    - ``<cwd>/.smartclaw/plugins/agents/`` → native=True  (project-level)
    - ``~/.smartclaw/plugins/agents/``     → native=False (user custom)
    ...
    project_agents_dir = Path.cwd() / ".smartclaw" / "plugins" / "agents"
...
def _project_agents_dir() -> Path:
    ...
    return Path(project_dir) / ".smartclaw" / "plugins" / "agents"
