...
Consolidates:
  - AgentInfo / AgentModel  (previously smartclaw.agent.core.agent)
  - AgentPromptMetadata / DelegationTrigger / AvailableAgent /
    AvailableTool / AvailableSkill / AvailableCategory
    (previously smartclaw.agent.prompts.builder.dynamic)
...
from smartclaw.permission import Ruleset
...
class AgentPromptMetadata:
    ...
    Used by Titan / Hephaestus prompt builders to generate the delegation table.
    """
    category: str
    cost: str
...
class AgentInfo(BaseModel):
    ...
    # "primary"  – top-level orchestrator shown to the user (e.g. Titan)
    ...
    # Delegation prompt metadata used by Titan / Hephaestus prompt builders.
