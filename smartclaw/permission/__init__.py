...
from smartclaw.permission.rule import (
    PermissionLevel,
    PermissionScope,
    PermissionRule,
    PermissionRequest,
...
from smartclaw.permission.manager import PermissionManager, Permission
from smartclaw.permission.helpers import Ruleset, from_config, merge
from smartclaw.permission.next import PermissionNext, PermissionRequestInfo, DeniedError
...
class PermissionNext:
    ...
    async def ask(
        ...
        if os.environ.get("SMARTCLAW_AUTO_APPROVE") == "true":
            log.debug("permission.auto_approved", {
                "permission": permission,
                "reason": "SMARTCLAW_AUTO_APPROVE=true"
            })
            return
    ...
    def _evaluate(
        ...
        Uses SmartClaw's "last matching rule wins" behavior.
        """
        matched_rule = None
