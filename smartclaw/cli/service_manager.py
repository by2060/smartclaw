"""
Service lifecycle helpers for local SmartClaw daemon commands.
"""
...
WEBUI_DIRECT_BACKEND_URLS_ENV = "SMARTCLAW_WEBUI_DIRECT_BACKEND_URLS"
...
def repo_root() -> Path:
    ...
    override = os.getenv("SMARTCLAW_REPO_ROOT")
    if override:
        return Path(override).expanduser().resolve()
    return Path(__file__).resolve().parents[2]


def smartclaw_root() -> Path:
    """Return the user-level SmartClaw state directory."""
    override = os.getenv("SMARTCLAW_ROOT")
    if override:
        return Path(override).expanduser().resolve()
...
def ensure_install_layout(root: Path | None = None) -> Path:
    ...
    if not (current / "webui" / "package.json").exists():
        raise ServiceError("未找到 WebUI 源码，请重新安装 SmartClaw，或设置 SMARTCLAW_REPO_ROOT 指向有效安装目录。")
    return current
...
def resolve_python_subprocess_command(
    root: Path | None = None,
    *,
    preferred_modules: Sequence[str] = ("uvicorn", "smartclaw"),
) -> list[str]:
...
def _smartclaw_executable_from_venv(venv_root: Path) -> str | None:
    """Return the smartclaw CLI entry point inside a virtual environment."""
    candidates = [
        venv_root / "Scripts" / "smartclaw.exe",
        venv_root / "Scripts" / "smartclaw.cmd",
        venv_root / "bin" / "smartclaw",
    ]
    for candidate in candidates:
        if candidate.exists():
            return str(candidate.resolve())
...
def resolve_smartclaw_cli_command(root: Path | None = None) -> list[str]:
    """Resolve a command prefix that launches the ``smartclaw`` CLI reliably.

    On Windows, always uses ``python.exe -m smartclaw.cli.main`` instead of
    ``smartclaw.exe`` to avoid locking the console-script entry point, which
    ...
    if sys.platform == "win32":
        venv_python = _python_executable_from_env_root(current_root / ".venv")
        if venv_python:
            return [venv_python, "-m", "smartclaw.cli.main"]
    else:
        venv_smartclaw = _smartclaw_executable_from_venv(current_root / ".venv")
        if venv_smartclaw:
            return [venv_smartclaw]

    launcher = which("smartclaw") or which("smartclaw.exe") or which("smartclaw.cmd")
    if launcher and not launcher.startswith("/mnt/"):
        return [launcher]

    return resolve_python_subprocess_command(root) + ["-m", "smartclaw.cli.main"]


def _bundled_node_install_dir() -> Path | None:
    """Return the bundled Node.js installation directory when available."""
    candidates: list[str] = []
    node_home = os.getenv("SMARTCLAW_NODE_HOME")
    if node_home:
        candidates.append(node_home)

    install_root = os.getenv("SMARTCLAW_INSTALL_ROOT")
    if install_root:
...
def _windows_identity_clauses(record: RuntimeRecord) -> list[tuple[str, ...]]:
    ...
    if "smartclaw.cli.main" in payload:
        clauses.append(("smartclaw.cli.main", "serve"))
    elif record.command and Path(record.command[0]).name.lower().startswith("smartclaw"):
        clauses.append(("smartclaw", "serve"))
...
def _read_upgrade_runtime_info(frontend_port: int | None = None) -> UpgradeRuntimeInfo:
    try:
        from smartclaw.updater import updater as updater_module
...
def _resolve_upgrade_runtime(console, *, frontend_port: int, attempt_recover: bool) -> dict[str, object]:
    ...
    _console_print(console, "[smartclaw] 检测到升级临时页残留，正在尝试恢复或清理...")
    ...
    if action == "recovered":
        _console_print(console, "[smartclaw] 已恢复未完成升级，正式 WebUI 将继续接管端口。")
    elif action != "noop":
        _console_print(console, "[smartclaw] 已清理升级临时页残留。")

    if isinstance(error, str) and error:
        _console_print(console, f"[smartclaw] 未完成升级的自动恢复失败，已清理临时升级页: {error}")
    return result
...
def start_backend(config: ServiceConfig, console) -> None:
    ...
    if listeners:
        if tracked_pid and tracked_pid in listeners:
            console.print(f"[smartclaw] 后端已在运行，PID={tracked_pid}")
            return
        raise ServiceError(
            f"后端端口 {config.backend_port} 已被占用 (PID: {_join_pids(listeners)})，"
            "与当前运行时记录不一致，请先执行 `smartclaw stop` 或手动清理残留进程。"
        )
    ...
    if runtime_record is not None and runtime_record_is_running(runtime_record):
        raise ServiceError(
            "后端运行记录仍存活，但端口未监听；请先执行 `smartclaw stop` 清理异常状态后重试。"
        )

    if runtime_record is not None:
        paths.backend_pid.unlink(missing_ok=True)

    command = resolve_smartclaw_cli_command(root) + [
        "serve",
        "--host",
        config.backend_host,
        "--port",
    ...
    backend_env["_SMARTCLAW_WEBUI_HOST"] = config.frontend_host
    backend_env["_SMARTCLAW_WEBUI_PORT"] = str(config.frontend_port)
    backend_env["PYTHONUNBUFFERED"] = "1"

    console.print("[smartclaw] 启动后端服务...")
    ...
    console.print(f"[smartclaw] 后端已启动，日志: {paths.backend_log}")
...
def start_frontend(config: ServiceConfig, console) -> None:
    ...
    if listeners:
        if tracked_pid and tracked_pid in listeners:
            console.print(f"[smartclaw] WebUI 已在运行，PID={tracked_pid}")
            return
    ...
    if runtime_record is not None and runtime_record_is_running(runtime_record):
        raise ServiceError(
            "WebUI 运行记录仍存活，但端口未监听；请先执行 `smartclaw stop` 清理异常状态后重试。"
        )
    ...
    if not config.skip_frontend_build:
        console.print("[smartclaw] 构建 WebUI...")
        completed = subprocess.run(
            [npm, "run", "build"],
            cwd=webui_dir,
            check=False,
    ...
    console.print("[smartclaw] 启动 WebUI...")
    process = _spawn_process(
        command,
        cwd=webui_dir,
    ...
    console.print(f"[smartclaw] WebUI 已启动，日志: {paths.frontend_log}")
...
def stop_one(port: int, pid_file: Path, name: str, console) -> None:
    ...
    if not target_pids and not group_running:
        ...
        pid_file.unlink(missing_ok=True)
        console.print(f"[smartclaw] {name} 未运行。")
        return
    ...
    console.print(f"[smartclaw] 停止 {name}（端口 {port}，PID: {details}）...")
    ...
    if sys.platform == "win32":
        ...
        for _ in range(10):
            ...
            if _tracked_processes_stopped(port, runtime_record, current_targets):
                pid_file.unlink(missing_ok=True)
                console.print(f"[smartclaw] {name} 已停止。")
                return
            time.sleep(1)

        console.print(f"[smartclaw] {name} 未在预期时间内退出，强制终止...")
...
def service_lock(paths: RuntimePaths):
    ...
    try:
        try:
            ...
            raise ServiceError("另一个 smartclaw 命令正在执行，请稍后重试。") from error
        yield
    finally:
        try:
...
def build_status_lines(paths: RuntimePaths | None = None) -> list[str]:
    ...
    if backend_listeners:
        lines.append(
            f"[smartclaw] 后端运行中: PID={_join_pids(backend_listeners)} URL=http://{backend_host}:{backend_port}"
        )
    elif backend_in_use:
        lines.append(f"[smartclaw] 后端运行中: PID=unknown URL=http://{backend_host}:{backend_port}")
    elif pid_is_running(backend_pid):
        lines.append(f"[smartclaw] 后端主进程仍在运行，但端口 {backend_port} 未监听: PID={backend_pid}")
    elif process_group_is_running(backend_record.pgid if backend_record else None):
        lines.append(f"[smartclaw] 后端进程组仍在运行，但端口 {backend_port} 未监听: PGID={backend_record.pgid}")
    else:
        lines.append("[smartclaw] 后端未运行")

    if upgrade_info.page_active:
        lines.append(
            f"[smartclaw] WebUI 临时升级页运行中: PID={_join_pids(upgrade_info.listener_pids)} URL=http://{frontend_host}:{frontend_port}"
        )
    elif frontend_listeners:
        lines.append(
            f"[smartclaw] WebUI 运行中: PID={_join_pids(frontend_listeners)} URL=http://{frontend_host}:{frontend_port}"
        )
    elif frontend_in_use:
        lines.append(f"[smartclaw] WebUI 运行中: PID=unknown URL=http://{frontend_host}:{frontend_port}")
    elif pid_is_running(frontend_pid):
        lines.append(f"[smartclaw] WebUI 主进程仍在运行，但端口 {frontend_port} 未监听: PID={frontend_pid}")
    elif process_group_is_running(frontend_record.pgid if frontend_record else None):
        lines.append(f"[smartclaw] WebUI 进程组仍在运行，但端口 {frontend_port} 未监听: PGID={frontend_record.pgid}")
    else:
        lines.append("[smartclaw] WebUI 未运行")

    if upgrade_info.payload_present:
        lines.append("[smartclaw] 检测到未完成的升级恢复状态")

    lines.append(f"[smartclaw] 后端日志: {current.backend_log}")
    lines.append(f"[smartclaw] WebUI 日志: {current.frontend_log}")
    return lines
...
def show_start_summary(config: ServiceConfig, console) -> None:
    ...
    paths = ensure_runtime_dirs()
    console.print()
    console.print("[smartclaw] 日志:")
    console.print(f"[smartclaw]   后端: {paths.backend_log}")
    console.print(f"[smartclaw]   WebUI: {paths.frontend_log}")
    console.print()
    console.print("[smartclaw] 后端接口:")
    console.print(f"[smartclaw]   http://{_loopback_host(config.backend_host)}:{config.backend_port}")
    console.print()
    console.print("[smartclaw] 打开浏览器访问:")
    console.print(f"[smartclaw]   {config.frontend_url}")
...
def show_logs(
    ...
    *,
    ...
    console.print("[smartclaw] 按 Ctrl+C 退出日志跟随。")
    handles = {}
    try:
        for path in selections:
...
def _emit_service_log_tail(console, log_path: Path, service_label: str, lines: int = 10) -> None:
    ...
    if not log_path.exists():
        console.print(
            f"[dim][smartclaw] {service_label} 日志文件尚不存在（{log_path}），"
            "子进程可能启动即退出。[/dim]",
        )
        return
    try:
        ...
        console.print(f"[dim][smartclaw] 无法读取 {service_label} 日志: {exc}[/dim]")
        return
    if not excerpt:
        return
    console.print(f"[yellow][smartclaw] {service_label} 近期日志（最后 {len(excerpt)} 行）:[/yellow]")
    for line in excerpt:
...
def open_default_browser(url: str, console) -> None:
    ...
    try:
        if webbrowser.open(url):
            console.print(f"[smartclaw] 已使用默认浏览器打开: {url}")
            return
    except Exception:
        pass
    console.print(f"[smartclaw] 未检测到可用的浏览器打开命令，请手动访问: {url}")
...
def build_frontend_env(config: ServiceConfig) -> dict[str, str]:
    ...
    env["SMARTCLAW_API_PROXY_TARGET"] = backend_url
