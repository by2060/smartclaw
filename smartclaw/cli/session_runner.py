...
from smartclaw.session.runner import SessionRunner, RunnerCallbacks, ToolResult
...
from smartclaw.project.project import Project
...
class CLISessionRunner:
    ...
    def __init__(
        ...
        # Streaming display state (SmartClaw style: accumulate and flush)
    ...
    async def start(
        ...
        os.environ["SMARTCLAW_CLI_RUN_MODE"] = "true"
        os.environ["SMARTCLAW_CLI_RUN_MODE"] = "true"
        ...
        # Load custom providers from smartclaw.json (same as server mode)
        try:
    ...
    async def _process_message(
        ...
        *,
        ...
        if dispatch_commands and stripped.startswith("/"):
            ...
            from smartclaw.input.output import CliOutputSink
        ...
        # Following SmartClaw's philosophy: minimal terminal updates during streaming
    ...
    def _flush_content(self) -> None:
        """
        Flush and finalize accumulated content (SmartClaw style).
    ...
    async def _on_text_delta(self, delta: str) -> None:
        """
        Handle text delta from LLM - SmartClaw style (最优性能).
