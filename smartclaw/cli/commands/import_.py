...
SHARE_URL_PATTERN = re.compile(r"https?://(?:opncd\.ai|smartclaw\.ai)/share/([a-zA-Z0-9_-]+)")
...
def import_session(
    ...
    Supports:
    - Local JSON files exported with 'smartclaw export'
    - Share URLs (https://opncd.ai/share/<slug> or https://smartclaw.ai/share/<slug>)
    """
...
async def _import_session(file_or_url: str, project_id: Optional[str]):
    ...
    if is_url:
        ...
        if not match:
            console.print(f"[red]Invalid URL format. Expected: https://opncd.ai/share/<slug> or https://smartclaw.ai/share/<slug>[/red]")
            raise typer.Exit(1)
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    # Try smartclaw.ai
                    response = await client.get(f"https://smartclaw.ai/api/share/{slug}")
