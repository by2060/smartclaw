...
Provides skill management commands:
  smartclaw skills list             – list all discovered skills
  smartclaw skills status           – show eligibility status (deps check)
  smartclaw skills find <query>     – search installable skills (clawhub + GitHub)
  smartclaw skills install <source> – install a skill from URL/GitHub/clawhub/local
  smartclaw skills remove <name>    – uninstall a user-managed skill
  smartclaw skills install-deps     – install a skill's declared tool dependencies
"""
...
from smartclaw.skill.installer import SkillInstaller
        ...
        for s in not_ready:
            ...
            if s.install_specs:
                ...
                if spec.kind == "brew" and spec.formula:
                    install_hint = f"smartclaw skills install-deps {s.name}"
...
def find_skills(
    ...
    console.print("[dim]Install with: smartclaw skills install <source>[/dim]")
...
def install_skill(
    ...
    scope: str = typer.Option(
        "project",
        "--scope",
        help="Install scope; skills are always installed to project .smartclaw/plugins/skills/.",
    ),
):
        ...
        if installed_skill and installed_skill.requires:
            ...
            if not checked.eligible:
                ...
                for m in checked.missing or []:
                    console.print(f"  • {m}")
                console.print(
                    f"\n  Run [bold]smartclaw skills install-deps {installed_skill.name}[/bold] to install them."
                )
    else:
