"""
smartclaw update  — self-update CLI command
"""
...
def update_command(
    ...
    """
    Check for and upgrade SmartClaw to the latest version.

    Downloads the latest Release source archive from GitHub, backs up the
    current version to ~/.smartclaw/version/, extracts and replaces source files,
...
async def _update(check: bool, yes: bool, force: bool = False, region: str | None = None) -> None:
    from smartclaw.updater import build_updated_frontend, check_update, perform_update, detect_deploy_mode
    ...
    if detect_deploy_mode() == "docker":
        console.print(
            ...
            "  [bold]docker pull ghcr.io/agentsmartclaw/smartclaw:latest[/bold]\n"
            "  [bold]docker restart <container>[/bold][/yellow]"
        )
        return

    if check:
        command = "smartclaw update --force" if force else "smartclaw update"
    ...
    from smartclaw.cli.service_manager import ServiceError, stop_all

    try:
        stop_all(console)
    except ServiceError as error:
        console.print(f"[red]执行 smartclaw stop 失败：{error}[/red]")
        raise typer.Exit(1)

    append_upgrade_text_log(f"OK cli_update_stopped_services_before_upgrade version={version_to_apply}")
    console.print("[green]✓ 已执行 smartclaw stop[/green]")
    console.print()
    stage_labels = {
        "fetching":    "下载最新源码包",
        "backing_up":  "备份当前版本",
    ...
    console.print("[dim]如需恢复服务，请执行 [bold]smartclaw start[/bold][/dim]")
