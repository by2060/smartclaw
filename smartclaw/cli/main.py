...
Provides command-line interface for SmartClaw
"""
...
from smartclaw.cli.commands import (
    admin_app,
...
from smartclaw.cli.commands.update import update_command
from smartclaw.cli.service_manager import (
    ServiceConfig,
    ServiceError,
    read_runtime_record,
...
app = typer.Typer(
    name="smartclaw",
    help="SmartClaw - AI-Native SecOps tool",
    add_completion=True,
...
def _ensure_server_api_token() -> bool:
    """Ensure local non-browser clients such as `smartclaw tui` can authenticate."""
    from smartclaw.security import get_secret_manager
    from smartclaw.server.auth import API_TOKEN_SECRET_ID
...
def version_callback(value: bool):
    ...
    if value:
        console.print(f"SmartClaw version {__version__}")
        raise typer.Exit()
...
def main_callback(
    ...
    """
    SmartClaw - AI-powered development tool

    SmartClaw Python implementation
    """
    # Initialize logging
...
def _handle_service_error(error: Exception) -> None:
    ...
    console.print(f"[red][smartclaw] error: {error}[/red]")
...
def serve(
    ...
    """
    Start the SmartClaw API server
    """
    import uvicorn
...
def tui(
    ...
    """
    Start SmartClaw with TUI interface

    This command starts the SmartClaw API server in the background and launches
    ...
    # Determine paths
    smartclaw_dir = Path(__file__).parent.parent.parent
    tui_dir = smartclaw_dir / "tui"
    ...
    console.print(Panel(logo(), border_style="cyan"))
    console.print("[cyan]Starting SmartClaw TUI...[/cyan]")
    ...
    server_process = None
    try:
        console.print("[yellow]Starting SmartClaw server...[/yellow]")
        ...
        if auto_approve:
            env["SMARTCLAW_AUTO_APPROVE"] = "true"
        ...
        # Build TUI command - use smartclaw tui
        tui_cmd = [
            "bun",
            "run",
            "--conditions=browser",
