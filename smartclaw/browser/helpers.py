...
def _set_storage_entries(storage_name: str, entries: list[dict[str, Any]]) -> int:
    ...
    # Merge keys instead of clearing storage: smartclaw/browser reuses the user's real profile.
    applied = js(
        f"""(() => {{
...
def _load_agent_helpers() -> None:
    ...
    if not helper_path.exists():
        return
    spec = importlib.util.spec_from_file_location("smartclaw_browser_agent_helpers", helper_path)
