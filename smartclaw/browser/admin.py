"""Administrative helpers for ``smartclaw browser``."""
...
VERSION_CACHE = Path(tempfile.gettempdir()) / "smartclaw-browser-version-cache.json"
...
def ensure_daemon(
    ...
    for attempt in (0, 1):
        ...
        proc = subprocess.Popen(
            [sys.executable, "-m", "smartclaw.browser.daemon"],
            env=merged_env,
...
def run_setup() -> int:
    ...
    if not _chrome_running():
        print("no Chrome/Chromium/Edge process detected. please start your browser and rerun `smartclaw browser --setup`.")
        return 1
    try:
    ...
    print("run `smartclaw browser --doctor` for diagnostics.", file=sys.stderr)
    return 1
...
def run_doctor() -> int:
    ...
    row(
        "browser running",
        browser_running,
        "" if browser_running else "start Chrome, Chromium, or Edge and rerun `smartclaw browser --setup`",
    )
    row("daemon alive", daemon, "" if daemon else "run `smartclaw browser --setup` to attach")
