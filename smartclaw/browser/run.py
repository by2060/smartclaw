"""CLI entrypoint for ``smartclaw browser``."""
...
Commands:
  smartclaw browser --version        print the current SmartClaw version
  smartclaw browser --doctor         diagnose install, daemon, and browser state
  smartclaw browser --setup          interactively attach to your running browser
  smartclaw browser --reload         stop the daemon so the next call starts fresh
  smartclaw browser state save PATH [--url URL]
  smartclaw browser state load PATH [--url URL] [--no-reload]
  smartclaw browser state show PATH
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args or args[0] != "-c":
        sys.exit('Usage: smartclaw browser -c "print(page_info())"')
    if len(args) < 2:
        sys.exit('Usage: smartclaw browser -c "print(page_info())"')
    print_update_banner()
    ensure_daemon()
