...
async def read_container_config_hash(container_name: str) -> Optional[str]:
    ...
    stdout, _, code = await exec_docker(
        [
            "inspect",
            "-f",
            '{{ index .Config.Labels "smartclaw.configHash" }}',
            container_name,
        ],
        allow_failure=True,
    )
...
def build_sandbox_create_args(
    ...
    # Labels
    args.extend(["--label", "smartclaw.sandbox=1"])
    args.extend(["--label", f"smartclaw.sessionKey={scope_key}"])
    args.extend(["--label", f"smartclaw.createdAtMs={created_at_ms}"])
    if config_hash:
        args.extend(["--label", f"smartclaw.configHash={config_hash}"])
    ...
    if has_container:
        ...
        if hash_mismatch:
            ...
            workspace_artifact_mount_changed = any(
                ...
                or "/.smartclaw/plugins" in str(bind).replace("\\", "/")
...
def build_docker_exec_args(
    ...
    exec_id: 若提供，则将容器内 shell 的 PID 写入 /tmp/.smartclaw_exec_<exec_id>，
    ...
    if has_custom_path:
        args.extend(["-e", f"SMARTCLAW_PREPEND_PATH={env['PATH']}"])

    path_export = ""
    if has_custom_path:
        path_export = (
            'export PATH="${SMARTCLAW_PREPEND_PATH}:$PATH"; '
            "unset SMARTCLAW_PREPEND_PATH; "
        )

    if exec_id:
        pid_file = f"/tmp/.smartclaw_exec_{exec_id}"
