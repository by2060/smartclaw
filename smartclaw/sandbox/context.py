...
def _resolve_project_plugins_dir(agent_workspace_dir: str) -> Path:
    ...
    for root in _iter_candidate_project_roots(agent_workspace_dir):
        current = root
        while True:
            smartclaw_dir = current / ".smartclaw"
            plugins_dir = smartclaw_dir / "plugins"
            if plugins_dir.exists() or smartclaw_dir.exists():
                return plugins_dir
            if current.parent == current:
                break
            current = current.parent
    return Path(agent_workspace_dir).expanduser() / ".smartclaw" / "plugins"
...
async def resolve_sandbox_context(
    ...
    if effective_workspace_dir == sandbox_workspace_dir:
        ...
        os.makedirs(
            os.path.join(effective_workspace_dir, ".smartclaw", "workspace"),
            exist_ok=True,
        )
    else:
    ...
    if artifact_binds:
        ...
        env["SMARTCLAW_WORKSPACE_DIR"] = effective_workspace_dir
        env["SMARTCLAW_OUTPUTS_DIR"] = container_output_dir
        env["SMARTCLAW_ARTIFACTS_DIR"] = container_artifacts_dir
        env["SMARTCLAW_PROJECT_PLUGINS_DIR"] = f"{effective_workspace_dir}/.smartclaw/plugins"
        env["SMARTCLAW_SESSION_ID"] = output_session_key
