...
Loads command templates from SmartClaw and Claude-compatible locations.
"""
...
from smartclaw.command.command import CommandInfo
...
def discover_commands() -> Dict[str, CommandInfo]:
    """
    Discover commands from supported locations with SmartClaw-compatible precedence.
    """
    ...
    try:
        from smartclaw.utils.compat import get_smartclaw_config_dir
        opencode_global_dir = str(get_smartclaw_config_dir(binary="opencode") / "command")
        sources.append(("smartclaw-global", smartclaw_global_dir, "**/*.md"))
    except Exception as e:
        log.warn("command.smartclaw_dir.error", {"error": str(e)})

    for claude_dir in project_claude_dirs:
        sources.append(("claude-project", os.path.join(claude_dir, "commands"), "**/*.md"))

    smartclaw_project_dir = os.path.join(current_dir, ".smartclaw", "command")
    sources.append(("smartclaw-project", smartclaw_project_dir, "**/*.md"))
