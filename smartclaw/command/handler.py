...
from smartclaw.command.command import API_SURFACES, Command, CommandSurface
    ...
    *,
    ...
    if name == "tools":
        ...
        if args.startswith("create"):
            ...
            if not skill:
                ...
                return True
            if agent_name:
                from smartclaw.agent.controls import agent_allows_skill
    ...
    if name == "skills":
        if not args or args == "list":
            ...
            if agent_name:
                from smartclaw.agent.controls import filter_agent_skills
    ...
    if name == "workflows":
        from smartclaw.agent.controls import filter_workflow_entries_for_agent

        effective_agent = str(agent_name or "titan").strip() or "titan"
        from smartclaw.workflow.center import format_workflow_entries, scan_skill_workflows
        try:
        ...
        if not entries:
            await send_text(
                "No workflows found.\n"
                "Create a workflow.json in .smartclaw/plugins/workflows/<name>/ to get started."
            )
            return True
    ...
    if name == "mcp":
        from smartclaw.mcp import MCP, McpStatus
            ...
            try:
                ...
                if not status:
                    lines = [
                        ...
                        "",
                        "To add servers, configure them in ~/.smartclaw/config/smartclaw.json under the 'mcp' key.",
                        "Example:",
                        '  "mcp": {',
                        '    "my_server": {',
                        '      "type": "remote",',
