...
Manages ~/.smartclaw/workspace/ - the user-facing file storage for:
- outputs/     : agent-generated task artifacts
- knowledge/   : user-curated knowledge base (future: vector indexing)

Memory files stay in ~/.smartclaw/data/memory/ (agent-managed, not migrated).
...
def _looks_like_session_component(value: str) -> bool:
    """Return True for SmartClaw session directory names."""
...
def _user_workspace_dir() -> Path:
    return _user_home_dir() / ".smartclaw" / "workspace"
...
def _looks_like_source_dir(path: Path) -> bool:
    ...
    def has_source_markers(base: Path) -> bool:
        ...
        return (
            ...
            or ((base / "smartclaw").is_dir() and (base / ".smartclaw").is_dir())
        )

    for current in (candidate, *candidate.parents):
        if current == cwd or current.name == "smartclaw" or has_source_markers(current):
            return True
...
def _get_workspace_dir() -> Path:
    ...
    Priority:
    1. SMARTCLAW_WORKSPACE_DIR environment variable
    2. ~/.smartclaw/workspace (default, adjacent to data/ logs/ plugins/)
    """
    override = os.getenv("SMARTCLAW_WORKSPACE_DIR")
    # 输出按会话隔离修改
    # 删除
    '''
        if override:
    ...
    # data_dir is ~/.smartclaw/data; workspace is sibling of data/
    # 输出按会话隔离修改
    # 删除
...
class WorkspaceManager:
    ...
    def get_outputs_dir(
        ...
        *,
        ...
        ``~/.smartclaw/workspace/outputs/<YYYY-MM-DD>/<session_id>/``.
        """
        # direct_override = os.getenv("SMARTCLAW_OUTPUTS_DIR")
        # if direct_override:
    ...
    def rewrite_legacy_output_path(
        ...
        if ".smartclaw_outputs" in raw_parts:
            idx = raw_parts.index(".smartclaw_outputs")
    ...
    def resolve_user_workspace_path(self, rel_path: str) -> Path:
        ...
        This normally ignores SMARTCLAW_WORKSPACE_DIR so chat uploads and generated
        artifacts cannot be redirected into the project checkout. Sandbox and
        tool subprocesses may set both SMARTCLAW_WORKSPACE_DIR and SMARTCLAW_OUTPUTS_DIR
