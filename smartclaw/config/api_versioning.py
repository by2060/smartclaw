...
coexist in ``smartclaw.json`` under distinct ``<service_id>_v<version>`` keys.
...
storage_key   Composite key used in ``smartclaw.json``
...
Startup ordering invariant:
    ...
    ``api_services`` layout. The lifespan in ``smartclaw/server/app.py``
...
def derive_storage_key(service_id: str, version: Optional[str]) -> str:
    """Compose the ``smartclaw.json`` key from ``service_id`` + optional ``version``.
...
def discover_api_service_descriptors(*, refresh: bool = False) -> List[ApiServiceDescriptor]:
    ...
    Project-level discoveries (``<cwd>/.smartclaw/plugins/tools/api/``) win
...
def migrate_api_services(*, backup: bool = True) -> Dict[str, str]:
    ...
    * ``no-source``  — neither key present; nothing to do.
    ...
    A timestamped backup of ``smartclaw.json`` is created beside the
...
def _api_plugin_roots() -> List[Path]:
    ...
    needing to reimport :mod:`smartclaw.plugin.loader`.
    """
    project_tools_api = Path.cwd() / ".smartclaw" / "plugins" / "tools" / "api"
    user_tools_api = Path.home() / ".smartclaw" / "plugins" / "tools" / "api"
...
def _atomic_write_json(path: Path, data: Dict[str, Any]) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(
        dir=str(path.parent), prefix=".smartclaw_", suffix=".tmp",
    )
    try:
