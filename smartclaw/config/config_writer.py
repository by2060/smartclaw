"""
Atomic CRUD operations for the provider section of smartclaw.json.

All writes follow: read raw JSON → modify in-memory → atomic write (tmp + rename) → clear Config cache.

This module is the single entry point for programmatic writes to smartclaw.json provider config.
...
def _get_example_config_dir() -> Path:
    ...
    return Path(__file__).resolve().parents[2] / ".smartclaw"
...
def ensure_config_files() -> None:
    ...
    smartclaw.json, .secret.json, and mcp_list.json exist. If they don't,
    ...
    for target_name, example_name in [
        ("smartclaw.json", "smartclaw.json.example"),
...
class ConfigWriter:
    """Atomic read-modify-write operations on the provider section of smartclaw.json."""

    @classmethod
    def _get_config_path(cls) -> Path:
        """Return the unified user smartclaw.json path."""
        config_dir = Config.get_config_path()
        jsonc_path = config_dir / "smartclaw.jsonc"
    ...
    def _read_raw(cls) -> Dict[str, Any]:
        """Read smartclaw.json as raw dict (no secret resolution)."""
    ...
    def _write_raw(cls, data: Dict[str, Any]) -> None:
        ...
        fd, tmp_path = tempfile.mkstemp(
            dir=str(path.parent), suffix=".tmp", prefix=".smartclaw_"
        )
        try:
    ...
    def get_provider_raw(cls, provider_id: str) -> Optional[Dict[str, Any]]:
        """Read a single provider entry from smartclaw.json (raw, secrets unresolved).
    ...
    def list_provider_ids(cls) -> list[str]:
        """Return all provider IDs present in smartclaw.json."""
        data = cls._read_raw()
        providers = data.get("provider", {})
    ...
    def get_all_providers(cls) -> Dict[str, Any]:
        """Get all provider configurations from smartclaw.json (raw, secrets unresolved).
    ...
    def add_provider(cls, provider_id: str, provider_config: Dict[str, Any]) -> None:
        """Add or replace a provider entry in smartclaw.json.
    ...
    def remove_provider(cls, provider_id: str) -> bool:
        """Remove a provider entry from smartclaw.json.
    ...
    def get_model_setting(cls, provider_id: str, model_id: str) -> Optional[Dict[str, Any]]:
        """Get setting for a specific model from smartclaw.json model_settings section."""
        data = cls._read_raw()
    ...
    def set_model_setting(
        ...
        """Set or update model setting in smartclaw.json model_settings section."""
        data = cls._read_raw()
    ...
    def remove_model_setting(cls, provider_id: str, model_id: str) -> bool:
        """Remove a model setting from smartclaw.json."""
        data = cls._read_raw()
        settings = data.get("model_settings", {})
        key = f"{provider_id}/{model_id}"
    ...
    def get_mcp_server(cls, name: str) -> Optional[Dict[str, Any]]:
        """Get a single MCP server config from smartclaw.json (raw, secrets unresolved)."""
        data = cls._read_raw()
        return data.get("mcp", {}).get(name)

    @classmethod
    def list_mcp_servers(cls) -> Dict[str, Any]:
        """Return all MCP server configs from smartclaw.json (raw, secrets unresolved)."""
        data = cls._read_raw()
        return data.get("mcp", {})

    @classmethod
    def add_mcp_server(cls, name: str, server_config: Dict[str, Any]) -> None:
        """Add or replace an MCP server entry in smartclaw.json.
    ...
    def remove_mcp_server(cls, name: str) -> bool:
        """Remove an MCP server entry from smartclaw.json.
    ...
    def get_api_service_raw(cls, service_id: str) -> Optional[Dict[str, Any]]:
        ...
        :func:`smartclaw.config.api_versioning.resolve_api_service`.
        ...
        try:
            from smartclaw.config.api_versioning import resolve_api_service
    ...
    def list_api_services_raw(cls) -> Dict[str, Any]:
        """Return all raw api_services entries from smartclaw.json."""
        data = cls._read_raw()
        return data.get("api_services", {})

    @classmethod
    def set_api_service(cls, service_id: str, service_config: Dict[str, Any]) -> None:
        """Create or replace an api_services entry in smartclaw.json.
        ...
        :mod:`smartclaw.config.api_versioning`. Writes that target a
        ...
        try:
            from smartclaw.config.api_versioning import warn_if_shadowing_legacy
    ...
    def remove_api_service(cls, service_id: str) -> bool:
        """Remove an api_services entry from smartclaw.json.
    ...
    # ``<project>/.smartclaw/plugins/tools/`` are tracked by git and may be
    ...
    def delete_tool_setting(cls, tool_name: str) -> bool:
        ...
        removed so smartclaw.json doesn't accumulate empty container objects
    ...
    def clear_default_models_for_provider(cls, provider_id: str) -> List[str]:
        ...
        model references remain in smartclaw.json.
    ...
    def build_provider_config(
        ...
        *,
        ...
        """Build a standard provider config dict for smartclaw.json.
