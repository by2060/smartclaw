...
class BashToolConfig(BaseModel):
    """Bash tool runtime policy configuration."""
    model_config = {"extra": "allow", "populate_by_name": True}

    command_black_list: Optional[List[str]] = Field(
        None,
        validation_alias=AliasChoices("command_black_list", "commandBlacklist", "command_blacklist"),
        description="Global command-name blacklist enforced before bash execution.",
    )
    block_message: Optional[str] = Field(
        None,
        validation_alias=AliasChoices("block_message", "blockMessage"),
        description="Message template used when a command is blocked. Supports {command}.",
    )
...
class UpdaterConfig(BaseModel):
    ...
    backup_retain_count: int = Field(
        1,
        description="Number of old version backups to retain under ~/.smartclaw/version/. Oldest are purged first.",
    )
...
class ChannelConfig(BaseModel):
    """Per-channel configuration block in smartclaw.json.
    ...
    workspace_dir: Optional[str] = Field(
        ...
        alias="workspaceDir",
        description=(
            "Working directory used when this channel auto-creates a SmartClaw session. "
            ...
            "x-smartclaw-directory request header)."
        ),
    )
...
class ConfigInfo(BaseModel):
    ...
    agent_logic: Optional[Literal["base", "titan"]] = Field(None, alias="agentLogic")
    enterprise: Optional[EnterpriseConfig] = None
    bash: Optional[BashToolConfig] = None
    ...
    allow_read_paths: Optional[List[str]] = Field(
        ...
        description=(
            ...
            "workspace; the SmartClaw config directory and ~/.ssh are never allowed."
        ),
    )
    ...
    def post_process(self):
        ...
        if self.memory is not None and isinstance(self.memory, dict):
            try:
                from smartclaw.memory.config import MemoryConfig
...
def _get_smartclaw_root() -> Path:
    """
    Get smartclaw root directory
    
    Priority:
    1. SMARTCLAW_ROOT environment variable
    2. ~/.smartclaw

    Returns the root without /config suffix.
    """
    # Priority 1: Explicit override
    root = os.getenv("SMARTCLAW_ROOT")
    if root:
        return Path(root)
...
def _get_config_dir() -> Path:
    ...
    Priority:
    1. SMARTCLAW_CONFIG_DIR environment variable
    2. Use smartclaw root/config
    """
    config_dir = os.getenv("SMARTCLAW_CONFIG_DIR")
    if config_dir:
        return Path(config_dir)
    return _get_smartclaw_root() / "config"


def _get_data_dir() -> Path:
    """
    Get data directory (under smartclaw root)
    
    Priority:
    1. SMARTCLAW_DATA_DIR environment variable
    2. XDG_DATA_HOME/smartclaw (if XDG_DATA_HOME is set)
    3. smartclaw_root/data (default)
    
    Can be overridden by SMARTCLAW_DATA_DIR environment variable.
    """
    data_dir = os.getenv("SMARTCLAW_DATA_DIR")
    if data_dir:
    ...
    if xdg_data:
        return Path(xdg_data) / "smartclaw"
    
    return _get_smartclaw_root() / "data"


def _get_log_dir() -> Path:
    """
    Get log directory (under smartclaw root)
    
    Priority:
    1. SMARTCLAW_LOG_DIR environment variable
    2. XDG_STATE_HOME/smartclaw/logs (if XDG_STATE_HOME is set)
    3. smartclaw_root/logs (default)
    
    Can be overridden by SMARTCLAW_LOG_DIR environment variable.
    """
    log_dir = os.getenv("SMARTCLAW_LOG_DIR")
    if log_dir:
        return Path(log_dir)
    ...
    if xdg_state:
        return Path(xdg_state) / "smartclaw" / "logs"
    
    return _get_smartclaw_root() / "logs"
...
class GlobalConfig(BaseSettings):
    ...
    All data unified under ~/.smartclaw directory.
    """
    model_config = {"env_prefix": "SMARTCLAW_", "case_sensitive": False}
    
    # Paths - now using ~/.smartclaw by default
    ...
    # Config overrides
    config_path: Optional[str] = None  # SMARTCLAW_CONFIG
    config_content: Optional[str] = None  # SMARTCLAW_CONFIG_CONTENT
    config_dir_override: Optional[str] = None  # SMARTCLAW_CONFIG_DIR
    
    # Permission overrides
    permission: Optional[str] = None  # SMARTCLAW_PERMISSION (JSON)
...
class Config:
    ...
    def replace_secret_refs(cls, text: str) -> str:
        ...
        try:
            from smartclaw.security import resolve_secret_value, get_secret_manager
    ...
    async def load_global_config(cls) -> ConfigInfo:
        ...
        for filename in ["smartclaw.json", "smartclaw.jsonc", "config.json"]:
    ...
    async def get(cls) -> ConfigInfo:
        ...
        1. User config (~/.smartclaw/config)
        2. Custom config path (SMARTCLAW_CONFIG env var)
        3. Inline config (SMARTCLAW_CONFIG_CONTENT env var)
        ...
        if global_cfg.config_content:
            try:
                ...
                from smartclaw.utils.log import Log
                log = Log.create(service="config")
                log.error("Failed to parse SMARTCLAW_CONFIG_CONTENT", {"error": str(e)})
    ...
    async def resolve_default_llm(cls) -> Optional[Dict[str, str]]:
        ...
        Priority:
        1. default_models.llm in smartclaw.json (structured, preferred)
        2. config.model in smartclaw.json (legacy "provider/model" string, fallback)
    ...
    def get_config_file(cls) -> Path:
        """Get the primary smartclaw.json file path."""
        return cls.get_config_path() / "smartclaw.json"
