"""Path helpers for the bundled SmartClaw Hub."""
...
def _candidate_roots() -> list[Path]:
    roots: list[Path] = []
    env_root = os.getenv("SMARTCLAW_HUB_ROOT")
    if env_root:
        roots.append(Path(env_root).expanduser())

    project_dir = Instance.get_directory()
    if project_dir:
        roots.append(Path(project_dir) / ".smartclaw" / "smartclawhub")

    cwd = Path.cwd().resolve()
    roots.extend(candidate / ".smartclaw" / "smartclawhub" for candidate in [cwd, *cwd.parents])

    package_root = Path(__file__).resolve().parents[2]
    roots.append(package_root / ".smartclaw" / "smartclawhub")
    roots.append(package_root.parent / ".smartclaw" / "smartclawhub")
    return roots
...
def get_bundled_hub_root() -> Path:
    for root in _candidate_roots():
        if root.is_dir():
            return root
    return Path.cwd().resolve() / ".smartclaw" / "smartclawhub"
...
def bundled_plugin_subdir(subdir: str) -> list[Path]:
    """Bundled smartclawhub plugin subdirectory roots for *subdir*.
    ...
    ``SMARTCLAW_HUB_ROOT`` between runs do not see stale paths.
    """
    if not subdir:
        return []
...
def bundled_tool_plugin_roots() -> list[Path]:
    """Tool-plugin search roots bundled inside smartclawhub.

    Returns directories shaped exactly like ``~/.smartclaw/plugins/tools/``
    (i.e. with ``api/``, ``python/`` etc. as immediate children). Used
    by the SmartClawHub catalog to surface bundled tool plugins as
