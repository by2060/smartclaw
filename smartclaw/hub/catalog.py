...
from smartclaw.hub.models import HubCatalogEntry, HubIndex, HubIndexEntry, HubPluginManifest, HubTaxonomy, PluginType
from smartclaw.hub.paths import get_bundled_hub_root
...
def _base_manifest(
    *,
    ...
    return HubPluginManifest(
        ...
        compatibility={"smartclaw": ">=0.8.0", "os": ["darwin", "linux", "windows"]},
...
def _bundled_tool_roots() -> dict[tuple[PluginType, str], Path]:
    """Tool plugin directories shipped pre-bundled inside smartclawhub.

    Returns ``(plugin_type, plugin_id) -> Path`` for every directory
    under ``<smartclawhub>/plugins/tools/`` that carries an actual tool
    payload (``_provider.yaml`` or any non-underscore YAML/PY file).

    These bundled tools are surfaced in the SmartClawHub UI as ``available``
    ...
    """
    from smartclaw.hub.paths import bundled_tool_plugin_roots
...
def system_plugin_root(plugin_type: PluginType, plugin_id: str) -> Optional[Path]:
    ...
    Lookup order (first hit wins):
      ...
      2. Bundled smartclawhub tool plugins (``_bundled_tool_roots``)

    The bundled fallback makes ``plugin_root``/``file_tree``/``read_file_content``
    work for SmartClawHub-bundled tools even when no copy has been installed
...
def list_catalog(
    *,
    ...
    # SmartClawHub-bundled tool plugins. The runtime ``ToolRegistry`` only
