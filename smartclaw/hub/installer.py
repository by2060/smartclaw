...
from smartclaw.hub import local
from smartclaw.hub.catalog import load_manifest
from smartclaw.hub.files import plugin_root
from smartclaw.hub.models import InstalledPluginRecord, PluginType
from smartclaw.hub.security import SKIP_NAMES, validate_package
...
def _copytree_skip_caches(src: Path, dst: Path) -> None:
    ...
    Bundled smartclawhub trees can carry leftover ``__pycache__``/VCS dirs
...
def _resolve_install_destination(
    ...
    (whether bundled in smartclawhub or living under a project's
    ``.smartclaw/plugins/tools/api/<id>/`` tree). Dropping that prefix
    silently breaks :mod:`smartclaw.config.api_versioning`'s
    ...
    is the bundled smartclawhub copy or an existing project-level install
...
def _collect_storage_keys(install_path: Path) -> list[str]:
    ...
    :mod:`smartclaw.config.api_versioning` uses, so callers can target
    ...
    """
    from smartclaw.config.api_versioning import _descriptor_for_plugin_dir
...
async def uninstall_plugin(plugin_type: PluginType, plugin_id: str) -> bool:
    ...
    if ".smartclaw/plugins" not in install_path.as_posix():
