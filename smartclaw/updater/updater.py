...
from smartclaw.updater.models import UpdateProgress, VersionInfo
from smartclaw.utils.log import Log

_DEFAULT_REPO = "AgentSmartClaw/SmartClaw"
_BACKUP_DIR = Path.home() / ".smartclaw" / "version"
_UPGRADE_PAGE_MARKER = "smartclaw-upgrade-in-progress"
...
def _record_update_journal(message: str) -> None:
    ...
    from smartclaw.utils.log import append_upgrade_text_log
...
def _running_from_legacy_uv_tool_install() -> bool:
    ...
    return "/uv/tools/smartclaw/" in executable
...
def _get_repo_root() -> Path:
    """
    Return the root of the smartclaw installation (the directory that owns
    ...
    for attempt in range(max_attempts):
        try:
            code, _, err = await _run_async(
                [str(python_exe), "-c", "import smartclaw; import uvicorn"],
                cwd=install_root,
                timeout=timeout,
            )
...
def _upgrade_run_dir() -> Path:
    return _smartclaw_root() / "run"


def _upgrade_log_dir() -> Path:
    return _smartclaw_root() / "logs"
...
<body data-upgrade-marker="{_UPGRADE_PAGE_MARKER}">
  <main class="panel">
    <div class="badge">SmartClaw 正在升级</div>
    <h1>系统升级中，请稍候</h1>
...
async def _get_updater_config():
    """Return UpdaterConfig from smartclaw.json, or defaults."""
    try:
        from smartclaw.config.config import Config, UpdaterConfig

        cfg = await Config.get()
        return cfg.updater or UpdaterConfig()
    except Exception:
        from smartclaw.config.config import UpdaterConfig
...
async def _fetch_gitlab_release(
    ...
    raise ValueError(
        ...
        "Verify the repo path or configure a token in smartclaw.json."
    )
...
def _backup_current_version(
    ...
    try:
        with tarfile.open(backup_path, "w:gz") as tar:
            tar.add(str(install_root), arcname="smartclaw", filter=_filter)
    except Exception as exc:
...
def _cleanup_old_backups(retain_count: int) -> None:
    ...
    backups = sorted(
        [p for p in _BACKUP_DIR.iterdir() if p.name.startswith("smartclaw-") and p.is_file()],
...
def _restore_backup_archive(backup_path: Path, install_root: Path) -> None:
    restore_dir = Path(tempfile.mkdtemp(prefix="smartclaw-rollback-"))
    try:
...
def cleanup_replaced_files(root: Path | None = None) -> None:
    ...
    leftovers = sorted(
        (path for path in install_root.rglob("*") if ".smartclaw_old_" in path.name),
...
def _renamed_lock_path(target: Path) -> Path:
    base = target.with_name(f"{target.name}.smartclaw_old_{os.getpid()}")
    candidate = base
    counter = 1
    while candidate.exists():
        candidate = target.with_name(f"{target.name}.smartclaw_old_{os.getpid()}_{counter}")
        counter += 1
    return candidate
...
def get_current_version() -> str:
    ...
    Compare the persisted marker at ~/.smartclaw/version/.current_version
...
async def check_update(*, locale: str | None = None, region: str | None = None) -> VersionInfo:
    ...
    from smartclaw.updater.deploy import detect_deploy_mode
...
async def perform_update(
    ...
    *,
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="smartclaw-update-"))
    if sys.platform == "win32":
        tmp_dir = _resolve_windows_long_path(tmp_dir)
    archive_filename = f"smartclaw-{latest_tag}.{fmt}"
    try:
...
def _refresh_global_cli_entry(install_root: Path) -> None:
    """Ensure the global ``smartclaw`` command points to the project ``.venv``.
    ...
    if sys.platform == "win32":
        ...
        if not venv_python.exists():
            return
        wrapper = link_dir / "smartclaw.cmd"
        content = f'@echo off\r\n"{venv_python}" -m smartclaw.cli.main %*'
        try:
            wrapper.write_text(content, encoding="oem")
        except LookupError:
            wrapper.write_text(content, encoding="utf-8")
        stale_exe = link_dir / "smartclaw.exe"
        if stale_exe.exists():
            try:
        ...
        target = install_root / ".venv" / "bin" / "smartclaw"
        if not target.exists():
            return
        link = link_dir / "smartclaw"
    ...
    if uv:
        try:
            ...
            if result.returncode == 0 and re.search(r"^smartclaw ", result.stdout, re.MULTILINE):
                subprocess.run(
                    [uv, "tool", "uninstall", "smartclaw"],
                    capture_output=True,
                    timeout=30,
                )
        except Exception:
...
def _build_restart_argv(install_root: Path | None = None) -> list[str]:
    ...
    return [str(venv_python), "-m", "smartclaw.cli.main"] + clean_rest
