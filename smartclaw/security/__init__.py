"""
Security utilities for SmartClaw

Provides secret management and credential handling.

Secret resolution:
- In ~/.smartclaw/config/smartclaw.json, use {secret:SECRET_ID} to reference .secret.json values
- Use {env:VAR_NAME} to reference environment variables (handled by Config)
- Secrets are stored in ~/.smartclaw/config/.secret.json as flat {secret_id: secret_value} KV pairs
"""
