...
from smartclaw.hooks.types import HookEvent
from smartclaw.hooks.registry import register_hook
...
class SessionMemoryHook:
    ...
    async def _save_session_to_memory(
        ...
        5. Write to memory file (in ~/.smartclaw/data/memory/)
        """
    ...
    async def _generate_slug(
        ...
        """
        from smartclaw.hooks.builtin.slug_generator import generate_slug_via_llm
    ...
    async def _write_to_memory(
        ...
        File path: ~/.smartclaw/data/memory/YYYY-MM-DD-slug.md
        
        Note: SmartClaw stores memory under the global data directory. When
