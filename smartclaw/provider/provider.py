...
class ProviderType(str, Enum):
    ...
    GATEWAY = "gateway"
    GITLAB = "gitlab"
    # Added in Batch 6 - Missing providers from SmartClaw comparison
...
class ModelInfo(BaseModel):
    ...
    """Field names explicitly present in smartclaw.json (not defaults)."""
...
class StreamChunk(BaseModel):
    ...
    Extended to support SmartClaw-style event types for richer streaming.
    """
    ...
    # Extended fields for SmartClaw-style events
...
class Provider:
    ...
    Similar to SmartClaw's Provider namespace
    """
    ...
    async def init(cls) -> None:
        ...
        # (all built-ins + dynamic providers from smartclaw.json) is registered.
        # Previously this only loaded 3 providers, which caused dynamic providers
        # like minimax/siliconflow (defined in smartclaw.json) to be silently missing
    ...
    def _ensure_initialized(cls):
        ...
        if not cls._initialized:
            ...
            providers_to_register = [
                ("openai", "smartclaw.provider.sdk.openai", "OpenAIProvider"),
                ("anthropic", "smartclaw.provider.sdk.anthropic", "AnthropicProvider"),
                ("google", "smartclaw.provider.sdk.google", "GoogleProvider"),
                ("azure", "smartclaw.provider.sdk.azure", "AzureProvider"),
                ("openai-compatible", "smartclaw.provider.sdk.openai_compatible", "OpenAICompatibleProvider"),
                ("mistral", "smartclaw.provider.sdk.mistral", "MistralProvider"),
                ("groq", "smartclaw.provider.sdk.groq", "GroqProvider"),
                ("cohere", "smartclaw.provider.sdk.cohere", "CohereProvider"),
                ("together", "smartclaw.provider.sdk.together", "TogetherProvider"),
                # Added in Batch 3
                ("xai", "smartclaw.provider.sdk.xai", "XAIProvider"),
                ("deepinfra", "smartclaw.provider.sdk.deepinfra", "DeepInfraProvider"),
                ("cerebras", "smartclaw.provider.sdk.cerebras", "CerebrasProvider"),
                ("perplexity", "smartclaw.provider.sdk.perplexity", "PerplexityProvider"),
                ("openrouter", "smartclaw.provider.sdk.openrouter", "OpenRouterProvider"),
                ("amazon-bedrock", "smartclaw.provider.sdk.bedrock", "BedrockProvider"),
                ("google-vertex", "smartclaw.provider.sdk.vertex", "VertexProvider"),
                ("local", "smartclaw.provider.sdk.local", "LocalProvider"),
                # Added in Batch 5
                ("gateway", "smartclaw.provider.sdk.gateway", "GatewayProvider"),
                ("gitlab", "smartclaw.provider.sdk.gitlab", "GitLabProvider"),
                # Added in Batch 6 - Missing providers from SmartClaw comparison
                ("github-copilot", "smartclaw.provider.sdk.github_copilot", "GitHubCopilotProvider"),
                ("github-copilot-enterprise", "smartclaw.provider.sdk.github_copilot", "GitHubCopilotEnterpriseProvider"),
                ("vercel", "smartclaw.provider.sdk.vercel", "VercelProvider"),
                ("opencode", "smartclaw.provider.sdk.opencode", "SmartClawCompatProvider"),
                ("sap-ai-core", "smartclaw.provider.sdk.sap_ai_core", "SAPAICoreProvider"),
                ("cloudflare-ai-gateway", "smartclaw.provider.sdk.cloudflare_gateway", "CloudflareGatewayProvider"),
                # Added in Batch 7 - Final providers
                ("google-vertex-anthropic", "smartclaw.provider.sdk.vertex_anthropic", "VertexAnthropicProvider"),
                ("azure-cognitive-services", "smartclaw.provider.sdk.azure_cognitive", "AzureCognitiveServicesProvider"),
                ("zenmux", "smartclaw.provider.sdk.zenmux", "ZenMuxProvider"),
                # Chinese providers
                ("deepseek", "smartclaw.provider.sdk.deepseek", "DeepSeekProvider"),
                ("volcengine", "smartclaw.provider.sdk.volcengine", "VolcengineProvider"),
                ("alibaba", "smartclaw.provider.sdk.alibaba", "AlibabaProvider"),
                ("tencent", "smartclaw.provider.sdk.tencent", "TencentProvider"),
                ("siliconflow", "smartclaw.provider.sdk.siliconflow", "SiliconFlowProvider"),
                ("threatbook-cn-llm", "smartclaw.provider.sdk.threatbook", "ThreatBookCnLLMProvider"),
                ("threatbook-io-llm", "smartclaw.provider.sdk.threatbook", "ThreatBookIoLLMProvider"),
                ("ollama", "smartclaw.provider.sdk.ollama", "OllamaProvider"),
                # Client-side tool calling (for backends without --enable-auto-tool-choice)
                ("cherry", "smartclaw.provider.sdk.cherry", "CherryProvider"),
            ]
            ...
            # Load dynamic providers from smartclaw.json
    ...
    def _load_dynamic_providers(cls):
        ...
        try:
            ...
            # Get all provider configs from smartclaw.json
            ...
            for provider_id, config in providers_config.items():
                ...
                if npm_package == "@ai-sdk/openai-compatible":
                        ...
                        class DynamicOpenAIProvider(OpenAIBaseProvider):
                            ...
                            def __init__(self):
                                ...
                                if not self._api_key:
                                    try:
                                        from smartclaw.provider.credential import get_api_key
                    ...
                    try:
                        from smartclaw.provider.sdk.cherry import CherryProvider
                    ...
                    # provider_id from smartclaw.json (e.g. "azure-openai").
                    try:
    ...
    def get(cls, provider_id: str) -> Optional["BaseProvider"]:
        ...
        with cls._smg_lock:
            ...
            if proxy is None:
                from smartclaw.provider.smg_provider import SMGProvider
    ...
    def resolve_model_info(cls, provider_id: str, model_id: str) -> tuple:
        ...
        Lookup order:
          1. provider._config_models (models loaded from smartclaw.json)
        ...
        try:
            ...
            # 1. Check provider._config_models first (smartclaw.json models)
    ...
    async def apply_config(cls, config: Optional[Any] = None, provider_id: Optional[str] = None) -> None:
        ...
        for pid, pconfig in provider_configs.items():
            ...
            # Update provider display name from smartclaw.json only for providers
                ...
                if isinstance(models_config, dict):
                    for model_id, model_data in models_config.items():
                        try:
                            ...
                            # Track which fields the user explicitly set in smartclaw.json
...
class BaseProvider:
    ...
    def is_configured(self) -> bool:
        ...
        {env:VAR} references in smartclaw.json, then applied via configure().
        """
    ...
    def get_meta(self) -> "ProviderMeta":
        ...
        """
        from smartclaw.provider.types import (
            AuthMethod,
            ConfigurateMethod,
    ...
    def _apply_config_overrides(self, catalog_def: "ModelDefinition", model: "ModelInfo") -> "ModelDefinition":
        """Apply user overrides from smartclaw.json (_config_models) on top of catalog definition.

        Only fields explicitly present in smartclaw.json are overridden; fields the user
        ...
        """
        from smartclaw.provider.types import PriceConfig

        overridden = catalog_def.model_copy(deep=True)
        keys = getattr(model, "_explicit_keys", set())

        # Display name — always override (every smartclaw.json entry has "name")
        if "name" in keys:
            overridden.name = model.name

        # Capabilities — only override fields explicitly stored in smartclaw.json
    ...
    def get_model_definitions(self) -> List["ModelDefinition"]:
        """Return model definitions for models in smartclaw.json (_config_models).
        ...
        release_date, etc.); all others fall back to the config data in smartclaw.json.
        smartclaw.json is the single source of truth for *which* models are listed.
        User-edited values in smartclaw.json always override catalog defaults.
        """
        catalog_by_id: dict = {}
        if self.CATALOG_ID:
            try:
                from smartclaw.provider.model_catalog import get_provider_model_definitions
        ...
        for model in getattr(self, "_config_models", []):
            if model.id in catalog_by_id:
                ...
                # but user edits stored in smartclaw.json always take precedence.
    ...
    async def validate_credential(
        ...
        """
        import time
        from smartclaw.provider.types import CredentialValidateResult
