...
def _resolve_reasoning_enabled(provider_id: str, model_id: str) -> Optional[bool]:
    """Read model-level default reasoning settings from smartclaw.json."""
    try:
...
def build_provider_options(
    ...
    *,
    ...
    if "claude" in model_lower:
        ...
        # the catalog entry (catalog takes priority over smartclaw.json overrides
...
def _get_catalog_model_max_tokens(model_id: str) -> Optional[int]:
    ...
    catalog entries taking priority over smartclaw.json overrides (see
    ``AnthropicProvider.get_model_definitions``), so the value returned here
    is the real Anthropic API limit, not the conservative smartclaw.json value.
    """
    try:
