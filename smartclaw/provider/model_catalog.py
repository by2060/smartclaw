...
# so `from smartclaw.provider.model_catalog import _CATALOG` still works.
class _CatalogProxy:
...
def sync_catalog_models_to_config() -> int:
    """Sync new models from catalog.json into smartclaw.json for existing providers.

    When catalog.json gains new models (e.g. after a code update), providers
    already configured in smartclaw.json won't see them because smartclaw.json only
    ...
    NOTE: If a user manually removes a catalog model from smartclaw.json, it will
