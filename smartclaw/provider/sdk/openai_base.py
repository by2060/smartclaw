...
#
# Recognised input block schema (SmartClaw-internal):
#   {"type": "text",  "text": "..."}
...
# SmartClaw-internal block types that the translation logic knows about. Used to
# distinguish "known type with missing/invalid fields" from "genuinely unknown
# type" when logging dropped blocks.
_SMARTCLAW_INTERNAL_BLOCK_TYPES = frozenset({"text", "image"})
...
def format_openai_content(content: Any) -> Any:
    """Translate SmartClaw-internal content blocks to OpenAI chat.completions schema.
    ...
    * Unknown block types are dropped and logged at DEBUG level to avoid
    ...
    for block in content:
        ...
        elif block_type in _SMARTCLAW_INTERNAL_BLOCK_TYPES:
...
class OpenAIBaseProvider(BaseProvider):
    ...
    def _get_client(self):
        ...
        if self._client is None:
            ...
            # box. Operators can opt out globally via SMARTCLAW_HTTP_TRUST_ENV=0
            # or per-provider via ``custom_settings.trust_env``.
            trust_env = _coerce_bool(
                os.getenv("SMARTCLAW_HTTP_TRUST_ENV"), True
            )
    ...
    def get_models(self) -> List[ModelInfo]:
        """Return models from smartclaw.json (_config_models) only.

        catalog.json is not consulted at runtime; it is only used when
        credentials are first saved to pre-populate smartclaw.json.
        """
