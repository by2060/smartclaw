...
from smartclaw.provider.provider import BaseProvider, ChatMessage, ChatResponse, ProviderConfig, StreamChunk
...
class SMGProvider(OpenAIBaseProvider):
    ...
    async def _log_http_attempt(self, request: httpx.Request) -> None:
        request.extensions["smartclaw_smg_started_at"] = time.perf_counter()
    ...
    async def _log_http_response(self, response: httpx.Response) -> None:
        started_at = response.request.extensions.get("smartclaw_smg_started_at")
        duration_ms = (
