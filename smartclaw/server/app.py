"""
FastAPI application for SmartClaw server
...
from smartclaw.utils.log import Log, LogLevel
from smartclaw.config.config import Config
from smartclaw.storage.storage import Storage
from smartclaw.utils.langfuse import initialize as init_observability, shutdown as shutdown_observability
from smartclaw.auth.service import AuthService
from smartclaw.server.auth import apply_auth_for_request, clear_auth_context
...
async def lifespan(app: FastAPI):
    ...
    try:
        from smartclaw.updater.updater import cleanup_replaced_files
    ...
    try:
        from smartclaw.updater.updater import _get_repo_root, _refresh_global_cli_entry
    ...
    try:
        from smartclaw.config.config_writer import ensure_config_files
        await _run_startup_phase(
            log,
            "config.ensure_files",
            ensure_config_files,
    ...
    try:
        from smartclaw.config.api_versioning import migrate_api_services
    ...
    from smartclaw.tool.question_handler import setup_api_question_handler
    await _run_startup_phase(
        log,
    ...
    try:
        ...
        if config.memory.enabled:
            from smartclaw.hooks.builtin import register_builtin_hooks
            await _run_startup_phase(
                log,
    ...
    try:
        from smartclaw.provider.credential import migrate_env_credentials
    ...
    # Sync new catalog models into smartclaw.json for existing providers (idempotent)
    try:
        from smartclaw.provider.model_catalog import sync_catalog_models_to_config
    ...
    # Load custom providers from smartclaw.json into runtime
    try:
        from smartclaw.server.routes.custom_provider import load_custom_providers_on_startup
        await _run_startup_phase(
            log,
    ...
    try:
        from smartclaw.server.routes.workflow import sync_workflows_from_filesystem
    ...
    # Seed built-in scheduled tasks from .smartclaw/plugins/tasks/*.json (idempotent)
    try:
        from smartclaw.task.plugin import seed_tasks_from_plugin
    ...
    try:
        from smartclaw.channel.gateway.manager import default_manager
    ...
    try:
        from smartclaw.updater.updater import recover_upgrade_state
    ...
    try:
        from smartclaw.server.routes.event import EventBroadcaster
    ...
    try:
        from smartclaw.session.core.status import SessionStatus
        grace_seconds = 5
...
#      ``_SMARTCLAW_WEBUI_*`` origin inferred from the current CLI launch.
#   2. Explicit ``server.cors`` in smartclaw.json → append user-configured
...
def _read_cors_config() -> tuple[list[str], Optional[str]]:
    ...
    Reads ``server.cors`` directly from ``smartclaw.json`` using synchronous
    ...
    _append_origin(
        origins,
        os.environ.get("_SMARTCLAW_WEBUI_HOST", ""),
        os.environ.get("_SMARTCLAW_WEBUI_PORT", ""),
    )
...
async def instance_context_middleware(request: Request, call_next):
    ...
    2. Header 'x-smartclaw-directory'
    3. Falls back to current working directory
    """
    import os
    from urllib.parse import unquote
    from smartclaw.project.instance import Instance
    from smartclaw.project.bootstrap import instance_bootstrap
    ...
    # Support both x-smartclaw-directory (native) and x-smartclaw-directory (TUI compatibility)
    directory = request.query_params.get("directory")
    if not directory:
        directory = request.headers.get("x-smartclaw-directory")
    if not directory:
        directory = request.headers.get("x-smartclaw-directory")
    if not directory:
        directory = os.getcwd()
...
from smartclaw.server.routes.health import router as health_router
from smartclaw.server.routes.session import router as session_router
from smartclaw.server.routes.provider import router as provider_router
from smartclaw.server.routes.config import router as config_router
from smartclaw.server.routes.project import router as project_router
from smartclaw.server.routes.file import router as file_router
from smartclaw.server.routes.message import router as message_router
from smartclaw.server.routes.agent import router as agent_router
from smartclaw.server.routes.model import router as model_router
# Added in Batch 3
from smartclaw.server.routes.tool import router as tool_router
from smartclaw.server.routes.pty import router as pty_router
# Added in Batch 4
from smartclaw.server.routes.lsp import router as lsp_router
# Added in Batch 5
from smartclaw.server.routes.mcp import router as mcp_router
# Added for TUI compatibility
from smartclaw.server.routes.event import router as event_router
from smartclaw.server.routes.global_ import router as global_router
from smartclaw.server.routes.path import router as path_router
from smartclaw.server.routes.vcs import router as vcs_router
from smartclaw.server.routes.find import router as find_router
from smartclaw.server.routes.misc import router as misc_router
# P1: Permission and Question routes for SmartClaw TUI
from smartclaw.server.routes.permission import router as permission_router
from smartclaw.server.routes.question import router as question_router
# P3: TUI control routes for remote TUI control
from smartclaw.server.routes.tui import router as tui_router
# WebUI: Workflow routes
from smartclaw.server.routes.workflow import router as workflow_router
# WebUI: Skill & Command routes
from smartclaw.server.routes.skill import router as skill_router
from smartclaw.server.routes.hub import router as hub_router
# WebUI: Hook management routes
from smartclaw.server.routes.hooks import router as hooks_router
# Model management: Default model, Usage routes
from smartclaw.server.routes.default_model import router as default_model_router
from smartclaw.server.routes.usage import router as usage_router
from smartclaw.server.routes.custom_provider import router as custom_provider_router
# Onboarding routes
from smartclaw.server.routes.onboarding import router as onboarding_router
# Task Center routes
from smartclaw.server.routes.task_entities import router as task_entities_router
# Background Task routes (agent-spawned async tasks)
from smartclaw.server.routes.background_task import router as background_task_router
# Channel routes (webhook + status)
from smartclaw.server.routes.channel import router as channel_router
# Workspace routes (file manager)
from smartclaw.server.routes.workspace import router as workspace_router
# Update (self-upgrade)
from smartclaw.server.routes.update import router as update_router
# Log viewing
from smartclaw.server.routes.logs import router as logs_router
from smartclaw.server.routes.auth import router as auth_router
from smartclaw.server.routes.admin_users import router as admin_users_router
from smartclaw.server.routes.notifications import router as notifications_router
