...
from smartclaw.provider.model_manager import get_model_manager
...
def _connected_provider_ids() -> set[str]:
    """Return provider ids explicitly connected in smartclaw.json."""
...
@router.delete(
    ...
    description="Delete a model from a provider (removes from smartclaw.json and runtime)",
)
async def delete_model_definition(
    ...
    Removes the model entry from smartclaw.json and the runtime cache.
    smartclaw.json is the single source of truth for the model list, so a
