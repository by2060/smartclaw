...
from smartclaw.auth.context import get_current_auth_user
from smartclaw.server.routes._timing import log_route_timing
from smartclaw.session.session import Session, SessionInfo as SessionModel
from smartclaw.session.policy import SessionPolicy
from smartclaw.sandbox.uploads import UPLOADS_CHAT_PREFIX
from smartclaw.workflow.skill_guard import workflow_session_metadata
from smartclaw.utils.log import Log
from smartclaw.utils.json_repair import parse_json_robust, repair_truncated_json
from smartclaw.server.auth import require_user
...
from smartclaw.utils.monitor import get_monitor
...
class SessionCreateRequest(BaseModel):
    ...
    Schema follows standard Session.create format (matches SmartClaw):
    - parentID: optional parent session ID
    - title: optional session title
    - permission: optional permission ruleset
    
    Note: Model is not stored at session level (matches SmartClaw).
...
class SessionResponse(BaseModel):
    """
    Session response - SmartClaw compatible
    
    Matches SmartClaw Session.Info format exactly.
...
async def get_session_status() -> Dict[str, Any]:
    ...
    SmartClaw compatible endpoint.
    """
    from smartclaw.session.core.status import SessionStatus
    from smartclaw.session.core.turn_state import get_turn_state, get_context_state
...
async def create_session(http_request: Request, request: Optional[SessionCreateRequest] = None) -> SessionResponse:
    ...
    if request.parentID:
        try:
            from smartclaw.hooks import trigger_hook, create_command_event
    ...
    if request.permission:
        from smartclaw.session.session import PermissionRule as SessionPermRule
        permission = [
            SessionPermRule(
    ...
    if effective_user_context:
        from smartclaw.session.user_context import set_session_user_context
...
async def delete_session(sessionID: str, request: Request) -> bool:
    ...
    from smartclaw.session.user_context import clear_session_user_context
...
async def update_session(
    ...
    if request.user_context is not None:
        from smartclaw.session.user_context import clear_session_user_context, set_session_user_context
        ...
        if updated:
            session = updated
        try:
            from smartclaw.tool.system.memory import evict_session_memory
...
async def abort_session(sessionID: str) -> bool:
    ...
    from smartclaw.server.routes.question import reject_session_questions
    ...
    bg_cancelled = 0
    try:
        from smartclaw.task.background import get_background_manager
...
async def get_session_diff(
    ...
    """Get session diff"""
    from smartclaw.storage.storage import Storage
    from smartclaw.session.lifecycle.summary import SessionSummary
...
async def summarize_session(sessionID: str, request: SummarizeRequest) -> bool:
    ...
    # This matches SmartClaw logic in session.ts:520-528
...
async def revert_session(sessionID: str, request: RevertRequest) -> SessionResponse:
    """Revert session"""
    from smartclaw.session.lifecycle.revert import SessionRevert
...
class UserMessageInfo(BaseModel):
    """
    User message info - SmartClaw TUI compatible format.
...
class AssistantMessageInfo(BaseModel):
    """
    Assistant message info - SmartClaw TUI compatible format.
...
async def _prepare_replay_runtime(
    ...
    from smartclaw.provider.provider import Provider
...
async def send_session_message(sessionID: str, request: PromptRequest):
    ...
    from smartclaw.server.routes.event import publish_event
    from smartclaw.utils.id import Identifier
    from smartclaw.tool.registry import ToolRegistry, ToolContext
    from smartclaw.agent.registry import Agent
    from smartclaw.project.instance import Instance
    import time
    import json
...
async def _run_session_compaction(
    ...
    *,
    ...
    """
    from smartclaw.session.lifecycle.compaction import run_compaction
    from smartclaw.session.lifecycle.compaction.compaction import (
...
# JSON repair utilities — delegated to smartclaw.utils.json_repair
...
async def _process_session_message(
    ...
    from smartclaw.provider.provider import Provider
    from smartclaw.session.session_loop import SessionLoop, LoopCallbacks
    from smartclaw.session.runner import RunnerCallbacks
    import time
    import os
    
    # Clean up revert state before processing (SmartClaw compatibility)
    await SessionRevert.cleanup(session)
    ...
    # ------------------------------------------------------------------
    from smartclaw.session.message import FilePart

    def _materialize_data_url_to_disk(
        data_url: str, mime_hint: str, filename_hint: Optional[str]
    ) -> str:
        """Decode a ``data:`` URL to ``~/.smartclaw/workspace/uploads/chat/<session>/...``.
        ...
        try:
            ...
            # the project checkout when SMARTCLAW_WORKSPACE_DIR is misconfigured.
    ...
    try:
        from smartclaw.session.lifecycle.title import SessionTitle
        import asyncio
...
async def _dispatch_sse_input(sessionID: str, session, event, working_directory: str) -> None:
    ...
    from smartclaw.input.dispatcher import dispatch_user_input
    from smartclaw.input.output import SSEOutputSink
    from smartclaw.server.routes.event import publish_event
    from smartclaw.session.message import Message, MessageRole
    from smartclaw.utils.id import Identifier

    agent_name = event.agent or "titan"
...
async def send_session_message_async(
    ...
    import os
    from smartclaw.input.events import UserInputEvent
