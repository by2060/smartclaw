...
SmartClaw TUI expects Config format:
{
    "$schema": string,
    "theme": string,
...
from smartclaw.config.config import Config, GlobalConfig, ConfigInfo as ConfigInfoModel
...
async def get_config() -> Dict[str, Any]:
    ...
    Retrieve the current SmartClaw configuration settings and preferences.
    SmartClaw TUI expects the merged Config object directly.
    """
    try:
        # Get complete configuration (includes global + project + env)
        complete_config = await Config.get()
        
        # Return merged config in SmartClaw format
...
async def update_config(config_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Update SmartClaw configuration settings and preferences.
    Returns the updated Config in SmartClaw format.
    ...
    smartclaw.json, so that plaintext secrets never land in that file.
    """
    try:
        ...
        if "channels" in config_data and isinstance(config_data.get("channels"), dict):
            from smartclaw.security.channel_secrets import extract_channel_secrets
...
async def get_providers():
    ...
    Note: SmartClaw TUI expects models as Dict[modelID, Model], not List[Model].
    """
    try:
        with log.time("providers"):
            ...
            for provider_id in provider_ids:
                try:
                    ...
                    # SmartClaw TUI expects models as Dict[modelID, Model]
                    models_dict = {}
                    first_model_id = None
                    ...
                    for model in models:
                        ...
                        # Build model dict in SmartClaw format
