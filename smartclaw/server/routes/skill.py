...
from smartclaw.skill.skill import Skill, SkillInfo
from smartclaw.skill.installer import SkillInstaller, SkillInstallResult, DepInstallResult
from smartclaw.command.command import API_SURFACES, Command, CommandInfo
from smartclaw.storage.storage import Storage
from smartclaw.utils.log import Log


def _user_skills_root() -> Path:
    """Return the canonical user-level skills directory (~/.smartclaw/plugins/skills/)."""
...
def _is_user_managed_skill(skill: SkillInfo) -> bool:
    ...
    deleted through the API.  Skills discovered from project .smartclaw/ directories
...
async def _agent_allowed_skill_or_403(
    ...
    if await titan_session_uses_full_skill_catalog(session_id, agent, extra):
        return
...
async def _filter_skills_for_agent(
    ...
    from smartclaw.tool.system.skill import list_skills_for_agent_context
...
class SkillInstallRequest(BaseModel):
    ...
    scope: str = Field(
        default="project",
        description="Install scope. Skills are always installed to project .smartclaw/plugins/skills/.",
    )
...
async def create_skill(
    ...
    (<project>/.smartclaw/plugins/skills/<name>/SKILL.md).
    """
    try:
...
async def update_skill(
    ...
    (~/.smartclaw/plugins/skills/) can be renamed; creates new directory,
        ...
        if is_rename:
            if skill.source == 'project':
                raise HTTPException(
                    status_code=400,
                    # # skill输出到项目级目录下修改
                    detail="Project skills (.smartclaw/plugins/skills/) can be updated in place but not renamed",
                )
            # # skill输出到项目级目录下修改
...
async def delete_skill(
    ...
    (~/.smartclaw/plugins/skills/) can be deleted via the API.
    """
    try:
        await _agent_allowed_skill_or_403(agent, name, session_id, category)
        skill = await Skill.get(name)
        ...
        if skill.source == 'project' and not SkillInstaller.is_deletable_project_skill(skill):
            raise HTTPException(
                status_code=403,
                detail="Built-in project skills (.smartclaw/plugins/skills/) cannot be deleted",
            )
        ...
        if skill.source == 'project':
            try:
                ...
                raise HTTPException(
                    status_code=403,
                    detail="Refusing to delete project skill outside .smartclaw/plugins/skills/",
                )
        if skill_dir.exists():
