...
from smartclaw.channel.registry import default_registry
...
async def channel_send(req: SendMessageRequest):
    ...
    from smartclaw.channel.base import OutboundContext
...
async def bind_session(channel_id: str, req: BindSessionRequest):
    ...
    of-process bridges (e.g. DingTalk's ``runner.ts``) create their SmartClaw
    ...
    """
    from smartclaw.channel.base import ChatType
    from smartclaw.channel.inbound.session_binding import SessionBindingService
...
async def weixin_qr_login_start(req: WeixinQrStartRequest):
    ...
    """
    from smartclaw.channel.builtin.weixin.config import ILINK_BASE_URL
    from smartclaw.channel.builtin.weixin.qr_login import start_qr_login
...
async def weixin_qr_login_status(qrcode: str, baseUrl: Optional[str] = None):
    ...
    """
    from smartclaw.channel.builtin.weixin.config import ILINK_BASE_URL
    from smartclaw.channel.builtin.weixin.qr_login import poll_qr_status
...
def _append_telegram_allow_from(user_id: str) -> None:
    """Atomically add *user_id* to channels.telegram.allowFrom in smartclaw.json.
    ...
    """
    from smartclaw.config.config_writer import ConfigWriter  # local import — avoids circular deps
...
async def telegram_pair(req: TelegramPairRequest):
    ...
    in smartclaw.json, so no manual save is required in the UI.
    """
    ...
    # Persist user_id to smartclaw.json immediately
    config_saved = False
    try:
        import asyncio
