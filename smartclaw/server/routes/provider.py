...
SmartClaw TUI expects models as Dict[modelID, Model], not List[Model].
...
from smartclaw.provider.provider import Provider, ModelInfo as ProviderModelInfo
from smartclaw.security.secrets import SecretManager
...
from smartclaw.tool.tool_loader import extract_provider_version
...
def _get_inline_provider_api_key(provider_id: str) -> Optional[str]:
    """Return an inline apiKey from smartclaw.json when present.

    WebUI credential flows primarily store API keys in .secret.json, but some
    users still configure custom providers directly in smartclaw.json. The test
...
def _model_to_api_format(model: ProviderModelInfo) -> Dict[str, Any]:
    """Convert internal ModelInfo to SmartClaw-compatible dict format.
...
def _get_npm_package(provider_id: str) -> str:
    ...
    from smartclaw.provider.model_catalog import get_provider_npm
...
# Response Models - SmartClaw compatible format
...
class ModelApi(BaseModel):
    """Model API configuration - SmartClaw compatible"""
...
class ModelInfo(BaseModel):
    """AI Model information - SmartClaw compatible format"""
...
class ProviderInfo(BaseModel):
    """Provider information - SmartClaw compatible format"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
    ...
    # SmartClaw expects models as Dict[modelID, Model], not List[Model]
...
async def list_providers() -> ProviderListResponse:
    ...
    try:
        ...
        # Only show providers the user has explicitly connected in smartclaw.json.
        ...
        for provider_id in provider_ids:
            ...
            # {secret:xxx} or {env:xxx} in smartclaw.json. Provider.apply_config()
            ...
            # Convert to response format - SmartClaw expects Dict[modelID, Model]
            models_dict: Dict[str, Dict[str, Any]] = {}
            first_model_id = None
...
async def get_provider_catalog():
    ...
    """
    from smartclaw.provider.model_catalog import get_raw_catalog, get_catalog
...
async def get_all_provider_auth() -> Dict[str, List[Any]]:
    ...
    SmartClaw compatible: GET /provider/auth
    """
    # TODO: Implement actual auth methods retrieval
    # SmartClaw format: Record<string, ProviderAuth.Method[]>
    return {}


# OAuth authorize endpoint - SmartClaw compatible
@router.post(
...
async def oauth_authorize(
    ...
    SmartClaw compatible: POST /provider/:providerID/oauth/authorize
...
# OAuth callback endpoint - SmartClaw compatible
@router.post(
...
async def oauth_callback(
    ...
    SmartClaw compatible: POST /provider/:providerID/oauth/callback
...
async def get_provider(provider_id: str) -> ProviderInfo:
    ...
    try:
        ...
        # Credentials are resolved at config load time via {secret:xxx} in smartclaw.json.
        # Provider.apply_config() above already configures from resolved config.

        # Get models - Convert to Dict[modelID, Model] format for SmartClaw
        provider_models = Provider.list_models(provider_id)
        models_dict: Dict[str, Dict[str, Any]] = {}
...
async def list_models(provider_id: str) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of models in SmartClaw-compatible format
    """
    try:
        Provider._ensure_initialized()
...
async def configure_provider(provider_id: str, config: ProviderConfigRequest) -> ProviderInfo:
    ...
    try:
        ...
        from smartclaw.provider.provider import ProviderConfig
...
def _get_api_service_tool_infos(provider_id: str) -> List[Any]:
    ...
    from smartclaw.server.routes.tool import _get_tool_source
...
def _is_api_service_builtin(provider_id: str, tools: Optional[List[Any]] = None) -> bool:
    ...
    its tools have native=True (project-level .smartclaw/plugins/ or core code).
    User-level tools (~/.smartclaw/plugins/) have native=False.
...
def _set_api_service_tools_enabled(provider_id: str, enabled: bool) -> int:
    ...
    if matched_tools:
        from smartclaw.agent.registry import invalidate_agent_cache_after_tool_change
...
async def list_api_services() -> List[APIServiceSummary]:
    try:
        from smartclaw.tool.registry import ToolRegistry
        from smartclaw.config.api_versioning import shadowed_legacy_ids
...
def _find_user_installed_tool_plugin_for(storage_key: str) -> Optional[str]:
    ...
    ``~/.smartclaw/plugins/`` (project-level / built-in installs are kept
    intact — only user-level installs cascade).
    """
    from smartclaw.config.api_versioning import discover_api_service_descriptors
    from smartclaw.hub import local as hub_local

    user_root = (Path.home() / ".smartclaw" / "plugins" / "tools").resolve()
...
async def delete_api_service(provider_id: str) -> Dict[str, Any]:
    ...
    under ``~/.smartclaw/plugins/tools/``), the plugin itself is also
    uninstalled so the SmartClawHub catalog flips back to ``available`` —
    ...
    """
    from smartclaw.hub.installer import uninstall_plugin
    from smartclaw.security import get_secret_manager
...
async def get_provider_credentials(provider_id: str):
    ...
    try:
        ...
        # base_url from smartclaw.json raw (not resolved)
        base_url = None
        raw_provider = ConfigWriter.get_provider_raw(provider_id)
        if raw_provider:
            options = raw_provider.get("options", {})
...
async def set_provider_credentials(provider_id: str, request: ProviderCredentialRequest):
    ...
    - api_key → .secret.json
    - base_url → smartclaw.json provider.{id}.options.baseURL
    - Ensures provider entry exists in smartclaw.json
    ...
    try:
        ...
        # 2. Ensure provider entry exists in smartclaw.json and update base_url / name
        raw_provider = ConfigWriter.get_provider_raw(provider_id)
        if raw_provider:
            ...
            # Provider not yet in smartclaw.json — create a minimal entry
            ...
            if not effective_base_url:
                try:
                    from smartclaw.provider.model_catalog import get_raw_catalog
                    raw_catalog = get_raw_catalog()
        ...
        provider = Provider._get_raw(provider_id)
        if provider:
            # Preserve the existing base_url from smartclaw.json when not explicitly provided,
...
async def delete_provider_credentials(provider_id: str):
    """Delete a provider: remove from smartclaw.json and .secret.json."""
    from smartclaw.security import get_secret_manager
    ...
    try:
        ...
        # 2. Remove provider entry from smartclaw.json
...
async def get_service_credentials(provider_id: str):
    ...
    Reads the secret_id from smartclaw.json api_services.{id}.secret first,
...
async def set_service_credentials(provider_id: str, request: ProviderCredentialRequest):
    ...
    Saves to .secret.json AND writes the secret reference into smartclaw.json
...
async def test_provider_credentials(provider_id: str, body: Optional[TestCredentialRequest] = None):
    ...
    try:
        ...
        if provider:
            from smartclaw.provider.provider import ProviderConfig, ChatMessage as ProviderChatMessage
            ...
            from smartclaw.tool.registry import ToolRegistry, ToolCategory, ToolInfo
            from smartclaw.server.routes.tool import _get_tool_source
            ...
            from smartclaw.tool.probe_loader import get_connectivity_spec
