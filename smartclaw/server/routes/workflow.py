...
from smartclaw.workflow.models import Workflow, Node, Edge
from smartclaw.workflow.runner import run_workflow, RunWorkflowResult
...
from smartclaw.workflow.compiler import compile_workflow
from smartclaw.workflow.fs_store import (
...
from smartclaw.workflow.execution_store import (
...
from smartclaw.workflow.io import load_workflow, dump_workflow
from smartclaw.workflow.tools import get_tool_registry
...
async def _workflow_visible_ids_for_request(
    ...
    from smartclaw.agent.controls import agent_visible_workflow_ids
...
def _workflow_dir(workflow_id: str) -> Path:
    ...
    return _find_workspace_root() / ".smartclaw" / "plugins" / "workflows" / workflow_id
    ...
    *,
    ...
    if effective_session_id:
        ...
        if not effective_agent:
            effective_agent = str(getattr(parent_session, "agent", None) or "titan")
        ...
        parent_session = await Session.create(
            ...
            agent=effective_agent or "titan",
            category="workflow",
...
def _write_workflow_to_fs(
    ...
    directory: ``<workspace>/.smartclaw/plugins/workflows/<id>/``.
    """
...
async def _migrate_storage_to_filesystem() -> None:
    ...
    Uses a marker file (.smartclaw/.storage_migrated) so the migration is safe
    across multiple workers / process restarts.
    """
    marker = _find_workspace_root() / ".smartclaw" / ".storage_migrated"
    if marker.exists():
        return
...
async def export_workflow(workflow_id: str):
    ...
    try:
        ...
        workflow_json["metadata"]["exportedFrom"] = "smartclaw"
