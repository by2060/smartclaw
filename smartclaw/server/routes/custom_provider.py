"""
Custom provider & model management — smartclaw.json backed.

Custom provider = OpenAI-compatible endpoint stored in smartclaw.json with npm="@ai-sdk/openai-compatible".
Custom model    = model entry in smartclaw.json under its provider's "models" dict.
...
from smartclaw.provider.provider import (
    BaseProvider,
    ModelCapabilities,
    ModelInfo,
    Provider,
...
class CustomProvider(OpenAICompatibleProvider):
    """A user-created custom provider backed by smartclaw.json.
...
async def list_providers():
    """List all custom providers from smartclaw.json."""
...
async def create_provider(body: CreateProviderReq):
    """Create a custom OpenAI-compatible provider in smartclaw.json."""
    ...
    # Build provider config for smartclaw.json
...
async def delete_provider(provider_id: str):
    """Delete a custom provider from smartclaw.json."""
...
async def list_models(provider_id: str):
    """List models for a custom provider from smartclaw.json."""
    raw = ConfigWriter.get_provider_raw(provider_id)
    if raw is None:
...
async def create_model(provider_id: str, body: CreateModelReq):
    """Add a model to a provider in smartclaw.json."""
    raw = ConfigWriter.get_provider_raw(provider_id)
    if raw is None:
        raise HTTPException(404, f"Provider '{provider_id}' not found")
    ...
    # Write to smartclaw.json (upsert — create or update)
...
async def delete_model(provider_id: str, model_id: str):
    """Remove a model from a provider in smartclaw.json."""
...
async def load_custom_providers_on_startup():
    ...
    with "custom-" from smartclaw.json, registers them in the Provider runtime.
    """
    from smartclaw.security import get_secret_manager
    ...
    for pid in provider_ids:
        ...
        # Load models from smartclaw.json
        models = raw.get("models", {})
        for model_id, mcfg in models.items():
