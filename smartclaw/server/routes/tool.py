...
from smartclaw.server.auth import require_admin
...
from smartclaw.permission.next import DeniedError, PermissionNext
from smartclaw.tool.api_tool_draft import (
    APIToolDraft,
...
from smartclaw.tool.api_tool_draft_sources import APIToolDraftSource, extract_and_merge_sources
...
class ToolInfoResponse(BaseModel):
    ...
    enabled_customized: bool = Field(False, description="True if a user setting is recorded in smartclaw.json tool_settings")
...
def _get_default_enabled(t: ToolInfo) -> bool:
    ...
    if snapshot is not None:
        return snapshot
    try:
        from smartclaw.tool.tool_loader import read_yaml_tool
...
def _get_effective_tool_enabled(tool_info: ToolInfo) -> bool:
    ...
    from smartclaw.server.routes.provider import _get_api_service_enabled
...
async def list_tools(
    ...
    effective_agent = str(agent or "titan").strip() or "titan"
    use_full_catalog = (
        effective_session_category == "workflow"
...
async def update_tool(tool_name: str, request: ToolUpdateRequest, _admin: object = Depends(require_admin)):
    ...
    ``smartclaw.json`` (``tool_settings.<tool_name>.enabled``) instead of
...
async def list_tool_fixtures(name: str) -> List[FixtureItemResponse]:
    ...
    """
    from smartclaw.tool.probe_loader import get_tool_fixtures_by_tool_name
...
async def _create_and_register_yaml_tool(
    ...
    *,
    ...
    from smartclaw.tool.tool_loader import (
        TOOL_TYPE_API,
...
def _reload_api_provider_tool_bindings(provider_id: str) -> None:
    ...
    from smartclaw.tool.tool_loader import _read_yaml_raw, list_api_provider_tools, yaml_to_tool
...
async def generate_api_tool_draft_route(
    ...
    try:
        from smartclaw.tool.api_tool_draft_llm import generate_api_tool_draft
...
async def confirm_api_tool_draft_route(
    ...
    from smartclaw.tool.smart_auth import SmartAuthConfigError, invalidate_smart_auth_provider
...
async def create_tool(request: CreateToolRequest, _admin: object = Depends(require_admin)):
    ...
    The tool is written to ``<project>/.smartclaw/plugins/tools/api/`` (or a
...
async def delete_tool(name: str, _admin: object = Depends(require_admin)):
    ...
    """
    from smartclaw.tool.tool_loader import delete_yaml_tool, delete_python_tool, find_yaml_tool
...
async def reload_tool(name: str, _admin: object = Depends(require_admin)):
    ...
    """
    from smartclaw.tool.tool_loader import find_yaml_tool, yaml_to_tool, _read_yaml_raw
...
async def list_plugin_tools():
    ...
    Returns tools discovered from ``~/.smartclaw/plugins/tools/`` including
    provider subdirectories.
    """
    from smartclaw.tool.tool_loader import list_yaml_tools
