...
Generic file-manager API for ~/.smartclaw/workspace/ plus a read-only view
of the agent memory directory (~/.smartclaw/data/memory/).
...
from smartclaw.workspace.models import WorkspaceNode, WorkspaceStats
from smartclaw.utils.log import Log
# 记忆按账号隔离新增
from smartclaw.auth.context import get_current_auth_user
from smartclaw.memory.manager import _safe_scope_segment
# -----------end-------------------
router = APIRouter()
...
def _max_upload_bytes() -> int:
    return int(os.getenv("SMARTCLAW_WORKSPACE_MAX_UPLOAD_MB", str(_DEFAULT_MAX_UPLOAD_MB))) * 1024 * 1024
...
def _sandbox_upload_path(relative_path: str) -> str | None:
    ...
    if len(parts) >= 4 and parts[0] == "uploads" and parts[1] == "chat":
        return "/".join((str(mgr.get_user_workspace_dir()), *parts))
    if len(parts) >= 3 and parts[0] == "uploads" and parts[1] == "task":
        return "/".join((str(mgr.get_user_workspace_dir()), *parts))
    return None
