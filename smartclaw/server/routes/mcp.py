...
from smartclaw.mcp.catalog import McpCatalog
from smartclaw.mcp.auth import McpAuth
from smartclaw.mcp.installer import preflight_install, preflight_uninstall
...
async def get_mcp_status():
    ...
    Merges in-memory runtime status with servers configured in smartclaw.json
    that have not been connected yet (shown as DISCONNECTED). This ensures
    that servers added by Titan via ConfigWriter appear in the UI immediately.
...
async def add_mcp_server(request: McpAddRequest):
    """
    Add a new MCP server and persist to both smartclaw.json and
    ``~/.smartclaw/plugins/tools/mcp/``.
    ...
    try:
        ...
        # plain-text credential is never written to smartclaw.json.
        ...
        # Persist to smartclaw.json (runtime config) — use clean_config so that
...
async def remove_mcp_server(name: str):
    ...
    server from smartclaw.json so it never reappears without a restart.
    ...
    try:
        ...
        # Clean up installed npm packages under ~/.smartclaw/mcp if this is a catalog entry
...
async def get_mcp_server_info(name: str):
    """Get info for a specific MCP server, with config from smartclaw.json when available."""
    try:
...
async def get_mcp_tools(agent: Optional[str] = None, scope: Optional[str] = Query(None)):
    ...
    try:
        from smartclaw.mcp import McpToolRegistry
...
@router.get(
    ...
    description="Return IDs of catalog servers already present in smartclaw.json.",
    operation_id="mcp.catalog.configured"
)
async def get_catalog_configured():
    """Return server IDs already configured in smartclaw.json mcp section."""
    try:
...
async def auto_setup_catalog():
    """Batch write all no-secret catalog entries to smartclaw.json with enabled=false."""
    try:
        catalog = McpCatalog.get()
        existing = ConfigWriter.list_mcp_servers()
        configured: List[str] = []
...
async def install_from_catalog(request: CatalogInstallRequest):
    """Install an MCP server from catalog into smartclaw.json.

    If credentials are provided, they are saved to .secret.json and the config
    uses {secret:key} references so that actual values stay out of smartclaw.json.
    """
    try:
        catalog = McpCatalog.get()
