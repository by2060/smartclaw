...
SESSION_COOKIE_NAME = "smartclaw_session"
...
def should_use_secure_cookie(request: Request) -> bool:
    forced = os.getenv("SMARTCLAW_COOKIE_SECURE", "").strip().lower()
...
def _read_api_token_from_request(request: HTTPConnection) -> Optional[str]:
    """
    Read API token from Authorization Bearer or x-smartclaw-api-token header.
    """
    ...
    alt = request.headers.get("x-smartclaw-api-token")
