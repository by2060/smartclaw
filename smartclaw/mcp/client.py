...
from smartclaw.mcp.types import McpResource, McpToolDef
from smartclaw.mcp.auth import McpAuth
from smartclaw.mcp.oauth2 import McpOAuth2ClientCredentials
from smartclaw.mcp.utils import build_mcp_url, resolve_env_var
...
class McpClient:
    ...
    def _smartclaw_mcp_prefix() -> str:
        ...
        """
        prefix = Path.home() / ".smartclaw" / "plugins" / "mcp"
    ...
    async def _connect_local(self, startup_future: asyncio.Future[None]) -> None:
        ...
        # For npx-based commands, set CWD to ~/.smartclaw/plugins/mcp so that
        ...
        if executable == "npx":
            mcp_prefix = self._smartclaw_mcp_prefix()
            cwd = mcp_prefix
