...
Matches SmartClaw' ported src/acp/agent.ts
"""
...
from smartclaw.acp.types import (
    ACPConfig,
    ACPSessionState,
    McpServer,
    McpServerLocal,
...
from smartclaw.acp.session import ACPSessionManager, RequestError
...
class ACPAgent:
    ...
    async def initialize(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        auth_method = {
            "id": "smartclaw-login",
            "name": "Login with SmartClaw",
            "description": "Run `smartclaw auth login` in the terminal",
        }
    ...
    async def _restart_session_state(self, session_id: str) -> None:
        ...
        from smartclaw.session.features.todo import Todo
    ...
    async def _get_default_model(self, cwd: str) -> Dict[str, str]:
        ...
        2. default_models.llm in smartclaw.json (structured)
        3. config.model in smartclaw.json (legacy string)
        4. Available providers (smartclaw -> first available)
        ...
        try:
            ...
            # Prefer smartclaw provider
            smartclaw_provider = next((p for p in providers if p.get("id") == "smartclaw"), None)
            if smartclaw_provider:
                models = smartclaw_provider.get("models", {})
                if "big-pickle" in models:
                    return {"providerID": "smartclaw", "modelID": "big-pickle"}
                
                if models:
                    model_id = next(iter(models.keys()))
                    return {"providerID": "smartclaw", "modelID": model_id}
