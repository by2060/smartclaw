...
Ported from SmartClaw' SessionProcessor pattern.
"""
...
from smartclaw.permission import PermissionNext
from smartclaw.agent.agent import AgentInfo
from smartclaw.utils.langfuse import span_scope
...
class StreamProcessor:
    ...
    Ported from SmartClaw' SessionProcessor behavior.
    """
    ...
    async def _handle_reasoning_start(self, event: ReasoningStartEvent) -> None:
        ...
        # Publish part created event (matches SmartClaw Session.updatePart)
    ...
    async def _handle_reasoning_end(self, event: ReasoningEndEvent) -> None:
        ...
        if event.id in self.reasoning_parts:
            ...
            # Publish final reasoning part (matches SmartClaw Session.updatePart)
            if self.event_publish_callback:
                await self.event_publish_callback("message.part.updated", {
                    "part": {
                        "id": part.id,
    ...
    async def _handle_tool_input_start(self, event: ToolInputStartEvent) -> None:
        ...
        # Create and store pending ToolPart (like SmartClaw's Session.updatePart)
        try:
            tool_part = ToolPart(
                id=part_id,
                sessionID=self.session_id,
            ...
            # Publish pending tool event (matches SmartClaw Session.updatePart)
            if self.event_publish_callback:
                await self.event_publish_callback("message.part.updated", {
                    "part": {
                        "id": part_id,
    ...
    async def _handle_tool_call(self, event: ToolCallEvent) -> None:
        ...
        # Update ToolPart to running state (like SmartClaw's Session.updatePart)
        # This matches SmartClaw's logic: update existing part from pending to running
        ...
        try:
            ...
            # Publish running tool event (matches SmartClaw Session.updatePart)
            if self.event_publish_callback:
                await self.event_publish_callback("message.part.updated", {
                    "part": {
                        "id": tool_state.part_id,
        ...
        try:
            ...
            try:
                ...
                # Publish completed/error tool event (matches SmartClaw Session.updatePart)
                if self.event_publish_callback:
                    state_dict = {
            ...
            try:
                ...
                # Publish error tool event (matches SmartClaw Session.updatePart)
                if self.event_publish_callback:
                    await self.event_publish_callback("message.part.updated", {
                        "part": {
                            "id": tool_state.part_id,
    ...
    async def _validate_tool_callable(self, tool_name: str) -> Optional[ToolResult]:
        ...
        if await titan_session_uses_full_tool_catalog(self.session_id, getattr(self.agent, "name", None)):
            return None
    ...
    async def _resolve_sandbox_meta(self, tool_name: str) -> Dict[str, Any]:
        ...
        try:
            from smartclaw.sandbox.context import resolve_sandbox_context
            ...
            if not is_tool_allowed(runtime.tool_policy, tool_name):
                ...
                result["error"] = (
                    ...
                    "Update sandbox.tools.allow/deny in ~/.smartclaw/config/smartclaw.json if needed."
                )
                return result
    ...
    async def _handle_text_end(self, event: TextEndEvent) -> None:
        ...
        if self.current_text_part:
            ...
            # Publish final text part (matches SmartClaw Session.updatePart)
            if self.event_publish_callback:
                await self.event_publish_callback("message.part.updated", {
                    "part": {
                        "id": self.current_text_part.id,
