...
from smartclaw.session.core.status import SessionStatus, SessionStatusBusy, SessionStatusIdle
from smartclaw.session.core.task_utils import fire_and_forget
from smartclaw.session.core.turn_state import (
    set_turn_state,
    set_context_state,
    clear_turn_state,
)
from smartclaw.session.lifecycle.compaction import (
    SessionCompaction,
    CompactionPolicy,
    build_compaction_policy,
    run_compaction,
)
from smartclaw.session.prompt import SessionPrompt
...
class SessionLoop:
    ...
    async def run(
        ...
        Main entry point matching SmartClaw' SessionPrompt.loop()
        ...
        from smartclaw.session.core.context import DefaultSessionContext
        ...
        ctx = LoopContext(
            ...
            model_id=model_id,
            agent_name=agent_name or session.agent or "titan",
        ...
        try:
            ...
            try:
                ...
                from smartclaw.bus.events import SessionError
            ...
            try:
                from smartclaw.bus.bus import Bus
                from smartclaw.bus.events import SessionIdle
    ...
    async def _resolve_model(
        ...
        *,
        ...
        # Priority 5: Parent session's model/provider (inherit from Titan etc.)
        if not resolved_provider or not resolved_model:
    ...
    async def _run_loop(
        ...
        while not ctx.should_abort():
            ...
            if ctx.step == 1 and ctx.session.memory_enabled and ctx.memory_bootstrap_data is None:
                try:
                    ...
                    user_context = (
                        ...
                        if isinstance(ctx.session.user_context, dict)
                        else {}
                    )
            ...
            if last_finished and not getattr(last_finished, 'summary', False):
                # Get model context limit from smartclaw.json / provider registry
                ...
                if model_context > 0:
                    ...
                    try:
                        ...
                        if is_overflow:
                            ...
                            # SessionCompaction.process() (matching SmartClaw).
            ...
            from smartclaw.session.runner import SessionRunner, RunnerCallbacks
    ...
    async def _check_reminders(
        ...
        """
        from smartclaw.session.features.reminders import SessionReminders, ReminderContext, ReminderConfig
    ...
    async def _execute_subtask(
        ...
        # Get agent
        agent = await Agent.get(agent_name) or await Agent.get("titan")
        ...
        from smartclaw.session.message import ToolPart, ToolStateRunning
        ...
        from smartclaw.session.message import ToolStateCompleted, ToolStateError
