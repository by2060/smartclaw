...
Default directory: ~/.smartclaw/data/records
Overrides:
- SMARTCLAW_RECORD_DIR: explicit record directory
- SMARTCLAW_ROOT: used indirectly via Config paths (if set)
"""
...
def _record_dir() -> Path:
    raw = os.getenv("SMARTCLAW_RECORD_DIR")
    if raw:
        return Path(raw)
