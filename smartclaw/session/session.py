...
Based on SmartClaw' ported src/session/index.ts
"""
...
from smartclaw.session.message import Message, MessageInfo, AssistantMessageInfo
...
class SessionInfo(BaseModel):
    ...
    # Agent and model
    agent: Optional[str] = Field("hephaestus", description="Agent type: hephaestus, build, plan, titan, …")
...
class Session:
    ...
    Mirrors original SmartClaw Session namespace from index.ts
    """
    ...
    async def create(
        ...
        **kwargs
        ...
        # Ensure sessions default to the configured primary agent (e.g., titan)
        ...
        try:
            ...
            from smartclaw.session.callable_state import initialize_session_callable_tools
        ...
        # SmartClaw compatibility: track main/sub sessions and publish event
        try:
            from smartclaw.session.core.session_state import set_main_session, add_subagent_session
            if parent_id:
        ...
        try:
            ...
            from smartclaw.bus.events import SessionCreated
    ...
    async def delete(cls, project_id: str, session_id: str) -> bool:
        ...
        try:
            from smartclaw.session.callable_state import clear_session_callable_tools
        ...
        # SmartClaw compatibility: clear state and publish event
        try:
            from smartclaw.session.core.session_state import get_main_session_id, set_main_session, remove_subagent_session
        ...
        try:
            from smartclaw.bus.bus import Bus
            from smartclaw.bus.events import SessionDeleted
