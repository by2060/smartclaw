...
Based on SmartClaw' ported src/session/message-v2.ts

Key features:
- Persistent storage using Storage module (SQLite)
- Thread-safe operations with asyncio locks
- SmartClaw compatible API
"""
...
from smartclaw.session.recorder import Recorder
...
class TokenUsage(BaseModel):
    """Token usage information - SmartClaw compatible"""
...
class TextPart(BaseModel):
    """Text message part - SmartClaw compatible"""
...
class FilePart(BaseModel):
    """File attachment part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class ToolPart(BaseModel):
    """Tool call/result part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class ReasoningPart(BaseModel):
    """Reasoning/thinking part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class SnapshotPart(BaseModel):
    """Snapshot part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class PatchPart(BaseModel):
    """File patch/edit part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class StepStartPart(BaseModel):
    """Step start marker - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class StepFinishPart(BaseModel):
    """Step finish marker - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class AgentPart(BaseModel):
    """Agent invocation part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class SubtaskPart(BaseModel):
    """Subtask/subagent part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class RetryPart(BaseModel):
    """Retry part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
class CompactionPart(BaseModel):
    """Compaction marker part - SmartClaw compatible"""
    model_config = ConfigDict(populate_by_name=True, by_alias=True)
...
# Union type for all parts - matches SmartClaw MessageV2.Part
PartType = Union[
    TextPart,
    SubtaskPart,
    ReasoningPart,
...
class UserMessageInfo(BaseModel):
    """
    User message information - SmartClaw compatible
...
class AssistantMessageInfo(BaseModel):
    """
    Assistant message information - SmartClaw compatible
...
class MessageWithParts(BaseModel):
    """
    Message with parts - SmartClaw compatible
...
class Message:
    ...
    def _cap_tool_part_output(cls, part: PartType) -> PartType:
        ...
        if not isinstance(output, str):
            ...
            try:
                ...
                output = str(output)
                state.output = output
        from smartclaw.tool.truncation import HARD_MAX_TOOL_RESULT_CHARS, truncate_tool_result_text
    ...
    async def to_llm_format(cls, messages: List[MessageInfo]) -> List[Dict[str, Any]]:
        ...
        for message in messages:
            ...
            # role is already a string in SmartClaw format
    ...
    def message_to_api(cls, message: MessageInfo) -> Dict[str, Any]:
        ...
        """
        from smartclaw.session.message import MessageSync
