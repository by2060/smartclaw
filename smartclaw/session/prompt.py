...
Based on SmartClaw' ported src/session/prompt.ts and src/session/system.ts
"""
...
OUTPUT_TOKEN_MAX = int(os.getenv("SMARTCLAW_OUTPUT_TOKEN_MAX", "32000"))


# Prompt template directory (same structure as SmartClaw)
PROMPT_DIR = Path(__file__).parent / "prompt"


def get_prompt_locale() -> str:
    from smartclaw.session.prompt_locale import get_prompt_locale as _get_prompt_locale

    return _get_prompt_locale()


def _localized_prompt_filename(filename: str, prompt_locale: Optional[str] = None) -> str:
    from smartclaw.session.prompt_locale import normalize_prompt_locale
...
# Lazy-loaded prompt templates (loaded from files like SmartClaw)
...
PROMPT_ANTHROPIC = """You are SmartClaw, an AI-Native SecOps Platform.
...
"""
PROMPT_GPT = """You are SmartClaw, a SecOps agent - please keep going until the user's security query is completely resolved.
...
PROMPT_GEMINI = """You are SmartClaw, an advanced AI SecOps agent specializing in cybersecurity operations.
...
PROMPT_DEFAULT = """You are SmartClaw, an AI-Native SecOps Platform.
...
class SystemPrompt:
    ...
    Mirrors original SmartClaw SystemPrompt namespace from system.ts
    ...
    def provider(cls, model_id: str) -> List[str]:
        ...
        Loads from template files (same as SmartClaw).
...
class SessionPrompt:
    ...
    Similar to SmartClaw's SessionPrompt namespace
    """
    ...
    def _get_tokenizer(cls):
        ...
        if cls._tokenizer is None:
            try:
                from smartclaw.utils.tiktoken_cache import ensure as _ensure_tiktoken
                _ensure_tiktoken()
                import tiktoken
