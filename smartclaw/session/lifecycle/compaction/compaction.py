...
#     override per-deployment via ``SMARTCLAW_COMPACTION_FLUSH_TIMEOUT``.
#
# Set ``SMARTCLAW_COMPACTION_FLUSH_BACKGROUND=0`` to fall back to the legacy
...
# ``SMARTCLAW_COMPACTION_FLUSH_TIMEOUT``.
...
def _flush_in_background_enabled() -> bool:
    raw = os.getenv("SMARTCLAW_COMPACTION_FLUSH_BACKGROUND", "1").strip().lower()
    return raw not in {"0", "false", "no", "off", ""}


def _flush_timeout_seconds() -> float:
    raw = os.getenv("SMARTCLAW_COMPACTION_FLUSH_TIMEOUT")
...
#   * ``SMARTCLAW_COMPACTION_PREEMPTIVE_CHUNK_RATIO`` (default 0.2):
#     fraction of ``target_chars`` above which we proactively chunk.
#   * ``SMARTCLAW_COMPACTION_TARGET_PARALLEL_CHUNKS`` (default 3):
#     desired number of chunks when preemptive chunking kicks in.
#   * ``SMARTCLAW_COMPACTION_MIN_CHUNK_CHARS`` (default 3000):
    ...
    *, total_chars: int, target_chars: int,
    ...
    if total_chars > target_chars * 2:
        ...
        ratio_hint = _env_float(
            "SMARTCLAW_COMPACTION_PREEMPTIVE_CHUNK_RATIO",
            _DEFAULT_PREEMPTIVE_CHUNK_RATIO,
        )
        target_parallel = _env_int(
            "SMARTCLAW_COMPACTION_TARGET_PARALLEL_CHUNKS",
            _DEFAULT_TARGET_PARALLEL_CHUNKS,
        )
        min_chunk = _env_int(
            "SMARTCLAW_COMPACTION_MIN_CHUNK_CHARS",
...
class SessionCompaction:
    ...
    async def _flush_memory_to_daily(
        ...
        continue to work.
        """
        from smartclaw.memory.flush import extract_and_save
    ...
    async def _dispatch_memory_flush(
        ...
        *,
        ...
        re-enabled by setting ``SMARTCLAW_COMPACTION_FLUSH_BACKGROUND=0``.
        ...
        2. **Hard timeout** — wraps the flush in ``asyncio.wait_for`` so a
           ...
           ``SMARTCLAW_COMPACTION_FLUSH_TIMEOUT`` (default
    ...
    async def process(
        ...
        try:
            from smartclaw.session.message import Message as MsgStore
    ...
    async def create(
        ...
        try:
            from smartclaw.session.message import CompactionPart, Message, MessageRole
        except ImportError:
    ...
    async def _build_post_compaction_context(
        ...
        try:
            ...
            if session_info and session_info.directory:
                ...
                for rules_file in ["AGENTS.md", ".smartclaw/rules/rules.md"]:
