...
# deployment via SMARTCLAW_COMPACTION_MERGE_TIMEOUT.
_DEFAULT_MERGE_TIMEOUT_SECONDS = 120


def _merge_timeout_seconds() -> float:
    raw = os.getenv("SMARTCLAW_COMPACTION_MERGE_TIMEOUT")
...
# ``SMARTCLAW_COMPACTION_CHUNK_CONCURRENCY`` environment variable.
def _chunk_concurrency_limit() -> int:
    raw = os.getenv("SMARTCLAW_COMPACTION_CHUNK_CONCURRENCY", "4")
    try:
        value = int(raw)
    except (TypeError, ValueError):
        return 4
