...
async def add_session_callable_tools(
    ...
    *,
    ...
    if agent_name:
        try:
            from smartclaw.agent.controls import agent_allows_tool, titan_session_uses_full_tool_catalog

            if not await titan_session_uses_full_tool_catalog(session_id, agent_name, extra):
                allowed = {
                    name
