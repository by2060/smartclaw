...
from smartclaw.session.message import Message, MessageInfo, MessageRole
from smartclaw.session.prompt import SystemPrompt, SessionPrompt, get_prompt_locale
from smartclaw.session.core.status import SessionStatus, SessionStatusRetry, SessionStatusBusy
from smartclaw.session.lifecycle.retry import SessionRetry
from smartclaw.session.lifecycle.compaction import SessionCompaction, CompactionPolicy
from smartclaw.session.streaming.stream_processor import StreamProcessor
from smartclaw.session.streaming.stream_events import (
    StartEvent,
    TextStartEvent,
    TextDeltaEvent,
    TextEndEvent,
...
from smartclaw.agent.toolset import agent_declares_tool
from smartclaw.provider.provider import Provider, ChatMessage
from smartclaw.hooks.pipeline import HookPipeline
from smartclaw.tool.catalog import get_tool_catalog_metadata, list_tool_catalog_infos
from smartclaw.tool.registry import ToolRegistry, ToolResult
from smartclaw.utils.langfuse import generation_scope, trace_scope
from smartclaw.session.utils.file_extractor import (
    read_file_part_bytes,
    is_text_extractable_mime,
    extract_file_text,
)
from smartclaw.sandbox.uploads import container_path_for_upload
...
from smartclaw.session.core.defaults import DOOM_LOOP_THRESHOLD
...
class SessionRunner:
    ...
    def __init__(
        ...
        from smartclaw.session.core.defaults import fallback_provider_id, fallback_model_id
        self.provider_id = provider_id or fallback_provider_id()
        self.model_id = model_id or fallback_model_id()
        self.agent_name = agent_name or "titan"
    ...
    # Provider IDs whose adapters are known to translate SmartClaw' internal
    ...
    def _model_supports_vision(self) -> bool:
        ...
        """
        try:
            from smartclaw.provider.provider import Provider as _Provider
    ...
    async def loop(cls, session_id: str) -> Optional['MessageInfo']:
        ...
        matching SmartClaw' SessionPrompt.loop() behavior.
        ...
        from smartclaw.session.session_loop import SessionLoop
    ...
    def cancel(cls, session_id: str) -> bool:
        ...
        # Set status to idle (SmartClaw compatibility)
        from smartclaw.session.core.status import SessionStatusIdle
    ...
    def cancel_children(cls, parent_session_id: str) -> int:
        ...
        from smartclaw.session.core.status import SessionStatus, SessionStatusIdle
    ...
    async def shell(
        ...
        from smartclaw.tool.code.bash_blacklist import check_bash_blacklist
        ...
        from smartclaw.tool.code.shell_risk import classify_shell_risk, high_risk_block_message
    ...
    async def _process_step(
        ...
        if not has_explicit_callbacks:
            try:
                from smartclaw.cli.session_runner import _get_cli_callbacks
        ...
        # Track session agent (SmartClaw compatibility)
        try:
            from smartclaw.session.core.session_state import set_session_agent
            set_session_agent(self.session.id, agent.name)
        except Exception as e:
            log.debug("runner.session_agent.error", {"error": str(e)})
        
        # Check if we've reached max steps (matching SmartClaw logic)
        ...
        if last_assistant_msg:
            ...
            if has_tool_result and not has_text:
                from smartclaw.session.prompt_strings import PROMPT_TOOL_RESULTS_AVAILABLE
            ...
            if has_tool_result and self._step > 2:
                ...
                if recent_tool_sigs:
                    ...
                    if repeated_sigs:
                        ...
                        from smartclaw.session.prompt_strings import PROMPT_REPEATED_TOOL_CALLS
        ...
        if chat_messages and tools and not self._should_use_text_tool_call_mode():
            ...
            if last_message.role == "assistant":
                ...
                from smartclaw.session.prompt_strings import PROMPT_SYNTHETIC_CONTINUE
        ...
        # Add reminder wrapping for queued user messages (matching SmartClaw logic)
        ...
        if self._step > 1:
            ...
            if last_finished:
                from smartclaw.session.prompt_strings import SYNTHETIC_MESSAGE_MARKERS
        ...
        # Add max steps warning if this is the last step (matching SmartClaw)
        if is_last_step:
            from smartclaw.session.prompt_strings import PROMPT_MAX_STEPS
        ...
        # Retry loop matching SmartClaw' SessionProcessor.process()
        ...
        while not self.is_aborted:
            try:
                ...
                # This matches SmartClaw's logic: check lastFinished.tokens at loop start
                ...
                if retry_message is not None and error_attempt <= MAX_ERROR_RETRIES:
                    ...
                    from smartclaw.session.lifecycle.retry import RETRY_MAX_DELAY_NO_HEADERS
    ...
    def _resolve_usage_pricing(self) -> Optional[Any]:
        ...
        from smartclaw.provider.usage_service import resolve_usage_pricing
    ...
    async def _record_usage_if_available(
        ...
        *,
        ...
        try:
            from smartclaw.provider.usage_service import RecordUsageRequest, record_usage
    ...
    async def _build_system_prompts(self, agent: AgentInfo) -> List[str]:
        ...
        from smartclaw.session.prompt_strings import PROMPT_CAPABILITY_SELF_CHECK
        ...
        if os.getenv("SMARTCLAW_PRINT_SYSTEM_PROMPT", "").lower() in ("1", "true", "yes"):
            header = (
    ...
    def _build_prompt_scope_cache_key(
        ...
        if not (isinstance(agent, AgentInfo) and agent.name == "titan"):
            return "default"
        payload = {
    ...
    async def _build_agent_identity_prompt(
        ...
        *,
        ...
        if not (isinstance(agent, AgentInfo) and agent.name == "titan"):
            return agent.prompt
        try:
            from smartclaw.agent.agents.titan.prompt_builder import build_session_dynamic_titan_prompt

            context = await Agent.prompt_builder_context()
            agents = context.get("agents", {})
            return build_session_dynamic_titan_prompt(
                titan_agent_info=agent,
                agent_lookup={
            ...
            log.warn("runner.titan_prompt.session_build_failed", {
                "session_id": self.session.id,
                "error": str(exc),
            })
            return agent.prompt
    ...
    async def _build_sandbox_prompt(self, agent: AgentInfo) -> Optional[str]:
        ...
        try:
            from smartclaw.config import Config
            from smartclaw.sandbox.system_prompt import build_sandbox_system_prompt
    ...
    async def _build_channel_context_prompt(self) -> Optional[str]:
        ...
        or DingTalk), this injects a one-liner so the agent knows:
          ...
          - the current SmartClaw session ID
        ...
        """
        try:
            from smartclaw.channel.inbound.session_binding import SessionBindingService
            ...
            if not bindings:
                return (
                    ...
                    f"This conversation originates from the **SmartClaw Web UI** (not an IM channel).\n"
    ...
    def _get_tool_instructions(self) -> str:
        from smartclaw.session.prompt_strings import PROMPT_TOOL_INSTRUCTIONS
    ...
    def _build_tool_catalog_prompt(self, agent: AgentInfo) -> Optional[str]:
        from smartclaw.tool.system.slash_command import format_tools_catalog_summary
    ...
    async def _build_callable_tool_schema(
        ...
        for tool_info in selected_tool_infos:
            ...
            if tool_info.name == "skill":
                ...
                from smartclaw.tool.system.skill import build_description, list_skills_for_agent_context
        ...
        for msg in messages:
            ...
            if msg.role == MessageRole.USER or (isinstance(msg.role, str) and msg.role == "user"):
                ...
                if is_latest_user_turn and get_prompt_locale() == "zh-CN":
                    from smartclaw.session.prompt_strings import PROMPT_ZH_THINKING_ANCHOR
                ...
                # Skip messages with errors (matching SmartClaw logic)
                # SmartClaw: skip if error exists, UNLESS it's AbortedError with useful content
                    ...
                    # Tool parts - use structured OpenAI function-calling format
                        ...
                        if part.state.status == "completed":
                            ...
                            if persisted_placeholder:
                                ...
                                from smartclaw.tool.truncation import truncate_tool_result_dynamic, HARD_MAX_TOOL_RESULT_CHARS
                                already_truncated = (
    ...
    async def _call_llm(
        ...
        Ported from SmartClaw' SessionProcessor.process() behavior.
        """
        ...
        try:
            from smartclaw.config import Config
            from smartclaw.session.core.session_state import get_main_session_id
        ...
        from smartclaw.provider.options import build_provider_options
        ...
        from smartclaw.session.streaming.tool_accumulator import ToolCallAccumulator
    ...
    async def _handle_permission(self, request) -> None:
        ...
        from smartclaw.permission.rule import PermissionRule, PermissionLevel
...
async def run_session(
    ...
    """
    from smartclaw.session.session_loop import SessionLoop, LoopCallbacks
