...
DEFAULT_PLUGIN_ROOT = Path.home() / ".smartclaw" / "plugins"
...
def get_project_plugin_root(project_dir: Optional[Path] = None) -> Path:
    ...
    return (project_dir or Path.cwd()) / ".smartclaw" / "plugins"
...
def load_module(source: str, base_dir: Path) -> ModuleType:
    ...
    if is_file:
        ...
        spec = importlib.util.spec_from_file_location(
            f"_smartclaw_plugin_{path.stem}",
            str(path),
        )
...
class PluginLoader:
    ...
    def load_all(
        ...
        1. Scan ``~/.smartclaw/plugins/{ext.subdir}/`` for ``.py`` files.
        2. Scan ``<project_dir>/.smartclaw/plugins/{ext.subdir}/`` for project-level plugins.
        3. Load *extra_sources* (from ``cfg.plugin``) and check for the attribute.
        4. Validate, dedup, and dispatch to the consumer.
        """
        project_dir = project_dir or Path.cwd()
        project_plugin_root = project_dir / ".smartclaw" / "plugins"

        for ext in cls._extension_points.values():
            ext._seen_keys = set()

            # 1. User-level plugin subdirectory (~/.smartclaw/plugins/{subdir}/)
            ...
            # 2. Project-level plugin subdirectory (<project>/.smartclaw/plugins/{subdir}/)
