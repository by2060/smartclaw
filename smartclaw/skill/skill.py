...
Discovers SKILL.md files from SmartClaw-compatible locations and provides
access to skill information. Mirrors original SmartClaw Skill namespace.
"""
...
class SkillInfo(BaseModel):
    ...
    native: bool = Field(default=False, description=(
        "True only for project-installed skills (<cwd>/.smartclaw/plugins/skills/). "
        "All other locations (.smartclaw/skills/, ~/.smartclaw/plugins/skills/, .claude/) "
        "are considered custom (user-defined). "
        "Derived from source; not declared in SKILL.md frontmatter."
    ))

    # Extended metadata (populated from frontmatter metadata.smartclaw / metadata.openclaw)
...
class Skill:
    ...
    - .smartclaw dirs   (global + project-level)
    - .claude dirs     (global ~/.claude + project-level)
    - ~/.smartclaw        (global user-level)
    - <project>/.smartclaw (project-level, wins on collision)
    """
    ...
    def _parse_skill_md(cls, filepath: str, source: Optional[str] = None) -> Optional[SkillInfo]:
        ...
        try:
            ...
            # Parse extended metadata — try metadata.smartclaw first, then metadata.openclaw
            ...
            if isinstance(raw_meta, dict):
                raw_smartclaw = raw_meta.get("smartclaw") or raw_meta.get("openclaw")
                if isinstance(raw_smartclaw, dict):
                    try:
                        skill_metadata = SkillMetadata.model_validate(raw_smartclaw)
            ...
            # native: only project-installed skills (<cwd>/.smartclaw/plugins/skills/) are built-in;
            # all other sources (smartclaw/skills/, ~/.smartclaw/plugins/, .claude/) are custom.
    ...
    def _discover(cls) -> Dict[str, SkillInfo]:
        ...
        Scan order (lowest → highest priority):
          ...
          2. ~/.smartclaw          (global user-level, overrides .claude)
          3. <project>/.smartclaw  (project-level, highest priority)

        Source labels:
          "smartclaw"   — built-in skills inside .smartclaw/skills/ directories
          "claude"   — skills discovered from .claude/ directories
          "user"     — user-installed skills under ~/.smartclaw/plugins/skills/
          "project"  — project-installed skills under <project>/.smartclaw/plugins/skills/
        """
        skills: Dict[str, SkillInfo] = {}
        ...
        global_smartclaw = os.path.join(home_dir, ".smartclaw")
        ...
        # 2) Global ~/.smartclaw — overrides .claude
        #    Built-in skills: source="smartclaw"; user-installed plugins: source="user"
        if os.path.isdir(global_smartclaw):
            for pattern in builtin_patterns:
                cls._scan_directory(global_smartclaw, pattern, skills, source="smartclaw")
            for pattern in plugin_patterns:
                cls._scan_directory(global_smartclaw, pattern, skills, source="user")

        # 3) Project-level .smartclaw — highest priority
        #    Built-in skills: source="smartclaw"; project-installed plugins: source="project"
        for smartclaw_dir in cls._find_dirs_up(".smartclaw", current_dir, worktree):
            if os.path.normpath(smartclaw_dir) == os.path.normpath(global_smartclaw):
                continue
            for pattern in builtin_patterns:
                cls._scan_directory(smartclaw_dir, pattern, skills, source="smartclaw")
            for pattern in plugin_patterns:
                cls._scan_directory(smartclaw_dir, pattern, skills, source="project")
...
class SkillFileWatcher:
    ...
    def _collect_watch_dirs(self) -> Set[str]:
        ...
        for target in (".smartclaw", ".claude"):
