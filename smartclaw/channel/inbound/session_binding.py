...
Maps platform conversations to SmartClaw Sessions.
...
async def _get_db() -> aiosqlite.Connection:
    ...
    shared with the rest of the SmartClaw storage subsystem.
    """
...
async def _migrate_legacy_binding_agent_ids(db: aiosqlite.Connection) -> None:
    ...
    only avoids breakage thanks to the ``Agent.get(name) or Agent.get("titan")``
    ...
    ``Agent.default_agent()`` → ``"titan"``) on the next inbound message. It is
...
class SessionBindingService:
    """Manages the platform-conversation ↔ SmartClaw-session mapping."""
    ...
    async def bind_session(
        ...
        *,
        ...
        create their SmartClaw session on their own and therefore bypass the
    ...
    async def _create_session(
        ...
        """Create a new SmartClaw Session and return its ID.
