"""
InboundDispatcher — the core router for platform → SmartClaw message flow.
...
from smartclaw.config.config import ChannelConfig
...
class ChannelDeliveryCallbacks:
    ...
    async def deliver_text(self, text: str) -> None:
        ...
        if not text:
            return
        from smartclaw.channel.outbound.deliver import OutboundDelivery
    ...
    def to_loop_callbacks(self, runner_callbacks=None):
        ...
        from smartclaw.session.session_loop import LoopCallbacks
...
class InboundDispatcher:
    """Central dispatcher: platform message → SmartClaw Session → Agent → reply.
    ...
    async def dispatch(self, msg: InboundMessage) -> None:
        ...
        # ``titan`` only via ``Agent.get(name) or Agent.get("titan")``, hiding the
        ...
        if not default_agent:
            try:
                from smartclaw.agent.registry import Agent as _Agent
        ...
        try:
            from smartclaw.channel.events import ChannelMessageReceived
            from smartclaw.bus.bus import Bus
        ...
        async with lock:
            ...
            if msg.channel_id == "feishu" and not msg.sender_name:
                try:
                    from smartclaw.channel.builtin.feishu.sender_name import resolve_sender_name
    ...
    def _resolve_feishu_context_limit(channel_config: ChannelConfig, chat_id: str) -> int:
        from smartclaw.channel.builtin.feishu.config import merge_group_overrides
    ...
    async def _handle_feishu_native_command(
        ...
        *,
        ...
        from smartclaw.input.output import ChannelOutputSink
    ...
    async def _handle_session_command(
        ...
        *,
        ...
        from smartclaw.session.session import Session
        from smartclaw.channel.inbound.session_binding import _build_title
    ...
    async def _trigger_command_hook(action: str, session_id: str, context: dict[str, Any]) -> None:
        try:
            from smartclaw.hooks import create_command_event, trigger_hook
    ...
    async def _run_agent_with_typing(
        ...
        try:
            from smartclaw.channel.builtin.feishu.typing import feishu_typing_indicator
    ...
    async def _run_agent_with_streaming(
        ...
        from smartclaw.channel.builtin.feishu.streaming_card import StreamingCard
    ...
    async def _append_user_message(
        ...
        from smartclaw.session.message import FilePart, Message, MessageRole
...
async def _resolve_session_model(session_id: str) -> Optional[dict]:
    ...
    """
    try:
        from smartclaw.session.session import Session as _Session
        from smartclaw.session.session_loop import SessionLoop
...
async def _expand_merge_forward(
    ...
    try:
        ...
        from smartclaw.channel.builtin.feishu.client import api_request_for_account
        from smartclaw.channel.builtin.feishu.monitor import _extract_content
