...
(``.smartclaw/plugins/channels/dingtalk/dingtalk.py``) with a pure-Python
...
* Outbound: enterprise app-robot OAPI via the existing
  :func:`smartclaw.channel.builtin.dingtalk.send.send_message_app`.
...
class DingTalkChannel(ChannelPlugin):
    ...
    async def send_text(self, ctx: OutboundContext) -> DeliveryResult:
        ...
        # ``smartclaw.channel.builtin.dingtalk.send_message_app`` (the public
        # surface) replace the same binding we look up here.
        from smartclaw.channel.builtin import dingtalk as _dingtalk_pkg
        from smartclaw.channel.builtin.dingtalk.client import DingTalkApiError
    ...
    async def stop(self) -> None:
        from smartclaw.channel.builtin.dingtalk.client import close_http_client
