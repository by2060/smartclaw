...
Connects SmartClaw to WeChat personal accounts via Tencent's iLink Bot API.
...
class WeixinChannel(ChannelPlugin):
    ...
    - ``dataDir``          — override path for storing sync_buf / context-token / media cache
                            (default: ~/.smartclaw/workspace/channels/weixin)
    """
    ...
    def config_schema(self) -> Optional[dict]:
        return {
            ...
            "properties": {
                ...
                "dataDir": {"type": "string", "description": "状态文件 / 媒体缓存存储目录（默认 ~/.smartclaw/workspace/channels/weixin）"},
            },
    ...
    async def send_text(self, ctx: OutboundContext) -> DeliveryResult:
        ...
        try:
            for idx, chunk in enumerate(chunks):
                client_id = f"smartclaw-weixin-{uuid.uuid4().hex}"
